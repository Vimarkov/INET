...
$req2="SELECT trame_travaileffectue.Id,Statut,Designation,DatePreparateur,DateValidation,StatutDelai,DescriptionModification,TempsPasse, ";
...
if($_SESSION['Langue']=="EN"){
	...
	$sheet->setCellValue('N1',utf8_encode("Further information"));
	$sheet->setCellValue('O1',utf8_encode("Deadline"));
	$sheet->setCellValue('P1',utf8_encode("Responsible of delay"));
	$sheet->setCellValue('Q1',utf8_encode("Cause of delay"));
	$sheet->setCellValue('R1',utf8_encode("Anomaly"));
	$sheet->setCellValue('S1',utf8_encode("Comment"));
	$sheet->setCellValue('T1',utf8_encode("Status"));
	$sheet->setCellValue('U1',utf8_encode("Time allocated"));
	if($nbResultaPlanning>0){
		$sheet->setCellValue('V1',utf8_encode("Time spent"));
	}
}
else{
	$sheet->setCellValue('A1',utf8_encode("Workpackage"));
	...
	$sheet->setCellValue('N1',utf8_encode("Informations complémentaires"));
	$sheet->setCellValue('O1',utf8_encode("Statut délais"));
	$sheet->setCellValue('P1',utf8_encode("Responsable délais"));
	$sheet->setCellValue('Q1',utf8_encode("Cause délais"));
	$sheet->setCellValue('R1',utf8_encode("Anomalie"));
	$sheet->setCellValue('S1',utf8_encode("Commentaire"));
	$sheet->setCellValue('T1',utf8_encode("Statut"));
	$sheet->setCellValue('U1',utf8_encode("Temps alloué"));
	if($nbResultaPlanning>0){
		$sheet->setCellValue('V1',utf8_encode("Temps passé"));
	}
}
if($nbResultaPlanning>0){
	$sheet->getStyle('A1:V1')->getBorders()->applyFromArray(array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN ,'color' => array('rgb' => '#000000'))));
	$sheet->getStyle('A1:V1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
	$sheet->getStyle('A1:V1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
	$sheet->getStyle('A1:V1')->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'f2f2f2'))));
	$sheet->getStyle('A1:V1')->getFont()->setBold(true);
	$sheet->getStyle('A1:V1')->getFont()->getColor()->setRGB('1f49a6');
}
else{
	$sheet->getStyle('A1:U1')->getBorders()->applyFromArray(array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN ,'color' => array('rgb' => '#000000'))));
	$sheet->getStyle('A1:U1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
	$sheet->getStyle('A1:U1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
	$sheet->getStyle('A1:U1')->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'f2f2f2'))));
	$sheet->getStyle('A1:U1')->getFont()->setBold(true);
	$sheet->getStyle('A1:U1')->getFont()->getColor()->setRGB('1f49a6');
}
...
while($row2=mysqli_fetch_array($result2)){
	...
	$sheet->setCellValue('I'.$ligne,utf8_encode(stripslashes(str_replace("\\","",$row2['Designation']))));
	...
	$sheet->setCellValue('N'.$ligne,utf8_encode(stripslashes(str_replace("\\","",$Infos))));
	$sheet->getStyle('N'.$ligne)->getAlignment()->setWrapText(true);
	$sheet->setCellValue('O'.$ligne,utf8_encode($row2['StatutDelai']));
	$sheet->setCellValue('P'.$ligne,utf8_encode($row2['ResponsableDelais']));
	$sheet->setCellValue('Q'.$ligne,utf8_encode($row2['CauseDelais']));
	$sheet->setCellValue('R'.$ligne,utf8_encode($anomalie));
	...
	$sheet->setCellValue('S'.$ligne,utf8_encode($description));
	$sheet->setCellValue('T'.$ligne,utf8_encode($statut));
	$sheet->setCellValue('U'.$ligne,utf8_encode($row2['TempsAlloue']));
	if($nbResultaPlanning>0){
		$sheet->setCellValue('V'.$ligne,utf8_encode($row2['TempsPasse']));
	}
