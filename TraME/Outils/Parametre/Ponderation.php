...
<head>
	...
	<script language="javascript">
		function OuvreFenetreAjout(){
			var w=window.open("Ajout_Ponderation.php?Mode=A&Id=0","PageUtilisateur","status=no,menubar=no,scrollbars=yes,width=600,height=120");
			w.focus();
			}
		function OuvreFenetreModif(Id){
			var w=window.open("Ajout_Ponderation.php?Mode=M&Id="+Id,"PageUtilisateur","status=no,menubar=no,scrollbars=yes,width=600,height=120");
			w.focus();
			}
		function OuvreFenetreSuppr(Id){
			if(window.confirm('Etes-vous sûr de vouloir supprimer ?')){
			var w=window.open("Ajout_Ponderation.php?Mode=S&Id="+Id,"PageUtilisateur","status=no,menubar=no,scrollbars=yes,,width=60,height=40");
			w.focus();
			}
		}
	</script>
...
$_SESSION['Formulaire']="Parametre/Ponderation.php";
...
<form class="test" method="POST" action="Ponderation.php">
	<tr>
		<td>
			<table width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td width="4"></td>
					<td class="TitrePage"><?php if($_SESSION['Langue']=="EN"){echo "Weighting list";}else{echo "Liste des pondérations";} ?></td>
				</tr>
			</table>
		</td>
	</tr>
	...
	<tr>
		<td align="left">
			<table align="left" style="width:30%;">
				<tr>
				<td align="center">
				<a style='text-decoration:none;' class='Bouton' href='javascript:OuvreFenetreAjout()'>&nbsp;<?php if($_SESSION['Langue']=="EN"){echo "Add a weighting";}else{echo "Ajouter une pondération";} ?>&nbsp;</a>
				</td>
				</tr>
			</table>
		</td>
	...
	<tr><td>
		<table cellpadding="0" cellspacing="0" align="left" class="GeneralInfo" style="width:40%;">
			<tr>
				...
				<td class="EnTeteTableauCompetences" width="25%" >&nbsp;<?php if($_SESSION['Langue']=="EN"){echo "Description";}else{echo "Description";} ?></td>
				<td class="EnTeteTableauCompetences" width="2%"></td>
				<td class="EnTeteTableauCompetences" width="2%"></td>
			</tr>
			<?php
				$req="SELECT Id,Libelle, Description FROM trame_ponderation WHERE Id_Prestation=".$_SESSION['Id_PrestationTR']." AND Supprime=false ORDER BY Libelle;";
