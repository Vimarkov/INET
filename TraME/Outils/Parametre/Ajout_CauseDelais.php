...
if($_POST){
	if($_POST['Mode']=="A"){
		$droit="";
		$requete="INSERT INTO trame_causedelais (Libelle,Id_Prestation) VALUES ('".addslashes($_POST['libelle'])."',".$_SESSION['Id_PrestationTR'].") ";
		$result=mysqli_query($bdd,$requete);
		echo "<script>FermerEtRecharger();</script>";
	}
	elseif($_POST['Mode']=="M"){
		$requete="UPDATE trame_causedelais SET ";
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="A" || $_GET['Mode']=="M"){
		if($_GET['Id']!='0')
		{
			$result=mysqli_query($bdd,"SELECT Id, Libelle FROM trame_causedelais WHERE Id=".$_GET['Id']);
			$Ligne=mysqli_fetch_array($result);
		}
?>

		<form id="formulaire" method="POST" action="Ajout_CauseDelais.php" onSubmit="return VerifChamps('<?php echo $_SESSION['Langue'];?>');">
