...
function AfficheDateFR2($Date)
{
	...
	else
	{
		...
		if($NavigOk ==1)
		{
			$tabDateTransfert = explode('-', $Date);
			$timestampTransfert = mktime(0, 0, 0, $tabDateTransfert[1], $tabDateTransfert[2], $tabDateTransfert[0]);
			$dateReq = date("m-d", $timestampTransfert);
		}
		else
		{
			$tabDateTransfert = explode('-', $Date);
...
{
	if($Date==Null || $Date=='' || $Date<='0001-01-01'){$dateReq="0001-01-01";}
	else
	{
		//Verifier si Google CHROME (true) ou Autre (fale)
		if (!empty($_SERVER['HTTP_USER_AGENT'])){$HTTP_USER_AGENT = $_SERVER['HTTP_USER_AGENT'];} 
 ...
 * ViderFiltreTri
 *
 * Cette fonction permet de vider les filtres/tris d'une page
 *
  * @param 	string 	$FiltreTri
 ...
 */
function ViderFiltreTri($Id_Prestation, $Id_Personne, $FiltreTri, $Page, $Variable = "")
{
	global $bdd;
	$req="UPDATE trame_parametrage
		SET Valeur='', Valeur2='' 
		WHERE Type='".$FiltreTri."'
		AND Id_Prestation=".$Id_Prestation." 
		AND Id_Personne=".$Id_Personne." 
		AND Page='".$Page."' ";
	if($Variable<>""){
		$req.="AND Variable='".$Variable."' ";
	}
 ...
 * RemplirFiltreTri
 *
 * Cette fonction permet de remplir les filtres/tris d'une page
 *
  * @param 	string 	$FiltreTri
 * @param 	int 	$Id_Prestation 
 * @param 	int 	$Id_Personne
 ...
 * @author Pauline FAUGE
 */
function RemplirFiltreTri($Id_Prestation, $Id_Personne, $FiltreTri, $Page, $Variable, $Valeur, $Valeur2="")
{
	global $bdd;
	$req="UPDATE trame_parametrage
		SET Valeur='".addslashes($Valeur)."', Valeur2='".addslashes($Valeur2)."' 
		WHERE Type='".$FiltreTri."'
		AND Id_Prestation=".$Id_Prestation." 
		AND Id_Personne=".$Id_Personne." 
		AND Page='".$Page."' 
