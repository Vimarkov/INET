...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		//Ajout du dossier
		$req="INSERT INTO sp_olwdossier (Id_Prestation,Id_Personne,MSN,Id_Client,Reference,DateDossier,HeureDossier,
				TypeACP,CaecACP,Priorite,Titre,DateCreation,DatePrevisionnelleIntervention,
				Ajusteur,Composite,Cosmetique,Hydraulique,Meca,Autre,Id_ZoneDeTravail,Id_Statut,Id_Retour,Programme,Id_StatutPREPA,Id_Poste)";
		
		$req.=" VALUES (316,".$_SESSION['Id_PersonneSP'].",".$_POST['msn'].",".$_POST['client'].",'".addslashes($_POST['numDossier'])."',
			'".$DateJour."','".$HeureJour."','".addslashes($_POST['typeDossier'])."','".$_POST['caec']."',
			".$_POST['priorite'].",'".addslashes($_POST['titre'])."','".$DateJour."','".TrsfDate_($_POST['datePrevisionnelleIntervention'])."',";
		if(isset($_POST['Ajustage'])){$req.="1,";}else{$req.="0,";}
		if(isset($_POST['Composite'])){$req.="1,";}else{$req.="0,";}
		if(isset($_POST['Cosmetique'])){$req.="1,";}else{$req.="0,";}
		if(isset($_POST['Hydraulique'])){$req.="1,";}else{$req.="0,";}
		if(isset($_POST['Meca'])){$req.="1,";}else{$req.="0,";}
		if(isset($_POST['Autre'])){$req.="1,";}else{$req.="0,";}
		$req.="".$_POST['localisation'].",";
		if(substr($_SESSION['DroitSP'],4,1)=='1'){
			if($_POST['statutQualite']<>"0"){$req.="'".$_POST['statutQualite']."',".$_POST['retourQualite'].",";}
			else{$req.="'".$_POST['statutProd']."',".$_POST['retourProd'].",";}
		}
		elseif(substr($_SESSION['DroitSP'],1,1)=='1'){
			$req.="'".$_POST['statutProd']."',".$_POST['retourProd'].",";
		}
		else{
			$req.="'',0,";
		}
		$req.="'".addslashes($_POST['programme'])."','".addslashes($_POST['statutPrepa'])."',".$_POST['poste'].")";
		...
		if($IdCree<>0){
			...
			$req.="Id_Dossier,NumFI,Id_Createur,DateCreation,PasDePS,PasDeECMEPROD,PasDeECMEQUALITE,PasDeIngredient,ValidationPSCE,ValidationPSIQ,
				DateIntervention,Vacation,TempsProd,Id_StatutPROD,Id_RetourPROD,DateCreationPROD,Id_PROD,CommentairePROD,DateTERA,
			...
			$req.=")
				VALUES (".$IdCree.",'".$_POST['numIC']."',".$_SESSION['Id_PersonneSP'].",'".$DateJour."',";
			...
			if(substr($_SESSION['DroitSP'],1,1)=='1'){
				$reqProd="'".$_POST['statutProd']."',".$_POST['retourProd']."";
			}
			
			$tempsProd=0;
			if($_POST['tempsProd']<>""){$tempsProd=$_POST['tempsProd'];}
			$req.= "'".TrsfDate_($_POST['dateIntervention'])."','".$_POST['vacation']."',".$tempsProd.",".$reqProd.",'".$DateJour."',
						...
						if($nbECME==0){
							...
							$ReqResponsablePostePrestation="
								...
								WHERE
									...
									AND new_competences_personne_poste_prestation.Id_Prestation IN (16,316)
							...
							$Objet="Nouvel ECME à mettre à jour, suivi production prestation TT350";
							$MessageMail="	<html>
											<head><title>Nouvel ECME à mettre à jour, suivi production prestation TT350</title></head>
											<body>
												Bonjour,
												<br><br>
												Attention, une référence d'ECME non identifiée dans la liste prédéfinie de votre outil de suivi production vient d'être saisie pour la prestation TT350<br>
				 ...
				 if($valeur<>""){
					...
					if($InfosProduit<>""){
						...
						if($nbProduit==0){
							...
							$Objet="Nouvel ingrédient à mettre à jour, suivi production prestation TT350";
							$MessageMail="	<html>
											<head><title>Nouvel ingrédient à mettre à jour, suivi production prestation TT350</title></head>
											<body>
												Bonjour,
												<br><br>
												Attention, un ingrédient non identifié dans la liste prédéfinie de votre outil de suivi production vient d'être saisi pour la prestation TT350<br>
				 ...
				 if($valeur<>""){
					...
					if($Qualif<>""){
						...
						if($nbQualif==0){
							...
							$Objet="Nouveau PS identifié, suivi production prestation TT350";
							$MessageMail="	<html>
											<head><title>Nouveau PS identifié, suivi production prestation TT350</title></head>
											<body>
												Bonjour,
												<br><br>
												Attention, une référence de procédé spécial non identifiée au tableu de compétences vient d'être saisie dans le suivi production de la prestation TT350<br>
												PS : ".$Qualif."
												<br>
												Bonne journée.<br>
												L'Extranet AAA.
...
<table width="100%" cellpadding="0" cellspacing="0" align="center">
	...
	<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
		<tr>
			<td colspan="8" style="font-size:15px;font-weight:bold;color:#4d4a82;" align="center">INFORMATIONS DOSSIER
		...
		<tr>
			...
			<td width='15%'>
				<select id="client" name="client">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_client WHERE Id_Prestation IN (16,316) AND Supprime=false ORDER BY Libelle";
			...
			<td width='15%' valign='top'>
				<input id='caec' name='caec' value='' size='8'>
			</td>
		</tr>
		<tr><td height='4'></td></tr>
		<?php
		...
		<tr>
			...
			<td width='25%' colspan='3'>
				<table width='100%' cellpadding='0' cellspacing='0' align='left'>
					<tr>
						<td><input type="checkbox" id="Ajustage" name="Ajustage" value="Ajustage" <?php echo $disabled;?> >Ajustage &nbsp;&nbsp;</td>
						<td><input type="checkbox" id="Composite" name="Composite" value="Composite" <?php echo $disabled;?> >Composite &nbsp;&nbsp;</td>
						<td><input type="checkbox" id="Cosmetique" name="Cosmetique" value="Cosmétique" <?php echo $disabled;?>>Cosmétique &nbsp;&nbsp;</td>
						<td><input type="checkbox" id="Hydraulique" name="Hydraulique" value="Hydraulique" <?php echo $disabled;?>>Hydraulique &nbsp;&nbsp;</td>
						<td><input type="checkbox" id="Meca" name="Meca" value="Méca" <?php echo $disabled;?> >Méca &nbsp;&nbsp;</td>
						<td><input type="checkbox" id="Autre" name="Autre" value="Autre" <?php echo $disabled;?>>(PR, hydrof., Nett., peinture, etc….) &nbsp;&nbsp;</td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<?php
			...
			<td width='15%'>
				<select id="localisation" name="localisation">
					<option name='0' value='0'></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_localisation WHERE Id_Prestation IN (16,316) AND Supprime=false ORDER BY Libelle";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
		...
		<tr>
			...
			<td width="15%">
				<select id="statutPrepa" name="statutPrepa" >
					...
					<?php
					$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation IN (16,316) AND TypeStatut='R' ORDER BY Id;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
			...
			<td width='15%'>
				<select id="poste" name="poste">
					<option name='0' value='0'></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_poste WHERE Id_Prestation IN (16,316) AND Supprime=false ORDER BY Libelle";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
							while($rowL=mysqli_fetch_array($result)){
			...
			<td width='13%' class='Libelle'>&nbsp; Priorité : <?php echo $etoile; ?></td>
			<td width='15%' valign='top'>
		...
		<tr>
			...
			<td width="15%">
				<input type="date" style="text-align:center;" id="datePrevisionnelleIntervention" name="datePrevisionnelleIntervention" size="10" value="">
			</td>
		</tr>
					...
					<table width="100%" id="tableProd" cellpadding="0" cellspacing="0" align="center">
								...
								<table cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										...
										<div id='Div_Compagnon' style='height:200px;width:200px;overflow:auto;'>
											...
											$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation IN (16) AND SUBSTR(sp_acces.Droit,3,1)='1' ORDER BY Nom, Prenom;";
											$result=mysqli_query($bdd,$req);
											$nbResulta=mysqli_num_rows($result);
											if ($nbResulta>0){
												$i=0;
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledCE;?>>
									<option name="" value=""></option>
									<?php
									$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation IN (16,316) AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										while($rowStatut=mysqli_fetch_array($result)){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation IN (16,316) ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
						...
						<tr>
							...
							<td bgcolor="#c7e048" width="20%" colspan="3">
								<select name="IQ">
									...
									$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation IN (16,316) AND SUBSTR(sp_acces.Droit,5,1)=1 ORDER BY NomPrenom;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
							...
							<td width="20%" bgcolor="#c7e048">
								<div id="statutsQualite">
									<select id="statutQualite" name="statutQualite" onchange="Recharge_StatutQualite();" <?php echo $disabledIQ;?>>
										<option name="" value=""></option>
										<?php
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation IN (16,316) AND TypeStatut='Q' ORDER BY Id;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="typeECMEPROD" name="typeECMEPROD" style="width:100px;" onchange="Recharge_RefECMEPROD()" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
												...
												$req="SELECT Id, Libelle FROM sp_olwtypeecme WHERE Supprime=false AND Id_Prestation IN (16,316) ";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
										...
										<td bgcolor='#e4e7f0'>
											<div id="listeECMEPROD">
												&nbsp; <select id="referencePROD" name="referencePROD" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
													...
													$req="SELECT Id, Libelle,Id_Type FROM sp_olwecme WHERE Supprime=false AND Id_Prestation IN (16,316) ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													if ($nbResulta>0){
														$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="typeECMECLIENT" name="typeECMECLIENT" onchange="Recharge_RefECMECLIENT()" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMECLIENT()">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwtypeecmeclient WHERE Supprime=false AND Id_Prestation IN (16,316) ";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
										...
										<td bgcolor='#e4e7f0'>
											<div id="listeECMECLIENT">
												&nbsp; <select id="referenceClient" name="referenceClient" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMECLIENT()">
													<?php
													echo"<option value='0'></option>";
													$req="SELECT Id, Libelle,Id_Type FROM sp_olwecmeclient WHERE Id_Prestation IN (16,316) AND Supprime=0 ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													if ($nbResulta>0){
														$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefProduit" name="RefProduit" onkeypress="if(event.keyCode == 13)AjouterProduit()" style="width:130px;">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwingredient WHERE Supprime=false AND Id_Prestation IN (16,316) ORDER BY Libelle;";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>
											...
											<select id="RefAIPI" name="RefAIPI" onkeypress="if(event.keyCode == 13)AjouterAIPIS()" style="width:130px;">
												...
												$req.="WHERE new_competences_prestation_qualification.Id_Prestation IN (16,316) ";
												...
												if ($nbResulta>0){
													...
													while($rowAIPI=mysqli_fetch_array($result)){
														...
														echo "<script>Liste_AIPI[".$i."] = new Array('".$rowAIPI['Id']."','".str_replace('"',' ',str_replace("'"," ",addslashes($rowAIPI['Libelle'])))."');</script>\n";
														$i+=1;
													}
												}
												?>
