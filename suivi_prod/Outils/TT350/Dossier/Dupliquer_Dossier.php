...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){	
		...
		if(isset($_POST['Cosmetique'])){$req.="Cosmetique=1, ";}else{$req.="Cosmetique=0, ";}
		if(isset($_POST['Hydraulique'])){$req.="Hydraulique=1, ";}else{$req.="Hydraulique=0, ";}
		if(isset($_POST['Meca'])){$req.="Meca=1, ";}else{$req.="Meca=0, ";}
		if(isset($_POST['Autre'])){$req.="Autre=1, ";}else{$req.="Autre=0, ";}
		...
		$req="INSERT INTO sp_olwficheintervention (";
			...
			$req.=")
				VALUES (".$_POST['idDossier'].",'".$_POST['numIC']."',".$_SESSION['Id_PersonneSP'].",'".$DateJour."',";
...
if($_GET){
	...
	$IdPersonne=$_GET['Id_Personne'];
		...
		$req="SELECT sp_olwficheintervention.Id,sp_olwficheintervention.Id_Dossier,sp_olwdossier.MSN,sp_olwdossier.Id_Client,
				...
				sp_olwdossier.TypeACP,sp_olwdossier.CaecACP,sp_olwdossier.Priorite,sp_olwdossier.DateCreation,
				sp_olwdossier.DatePrevisionnelleIntervention,sp_olwdossier.Ajusteur,sp_olwdossier.Composite,sp_olwdossier.Cosmetique,
				sp_olwdossier.Hydraulique,sp_olwdossier.Meca,sp_olwdossier.Autre,sp_olwdossier.Id_ZoneDeTravail,sp_olwdossier.Id_Statut,
		...
		$req.="sp_olwficheintervention.Id_RetourPROD,sp_olwficheintervention.CommentairePROD,";
			//QUALITE
			...
			<tr>
				...
				<td width='15%'>
					<select id="client" name="client">
						...
						<?php
							$req="SELECT Id,Libelle,Supprime FROM sp_client WHERE Id_Prestation IN (16,316) ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
				...
				<td colspan='3'>
					<table width='100%' cellpadding='0' cellspacing='0' align='left'>
						<tr>
							...
							<td><input type="checkbox" id="Cosmetique" name="Cosmetique" value="Cosmétique" <?php if($row['Cosmetique']==1){ echo "checked";}?> <?php echo $disabled;?>>Cosmétique &nbsp;&nbsp;</td>
							...
							<td><input type="checkbox" id="Autre" name="Autre" value="Autre" <?php if($row['Autre']==1){ echo "checked";}?> <?php echo $disabled;?>>(PR, hydrof., Nett., peinture, etc….) &nbsp;&nbsp;</td>
						</tr>
					</table>
				</td>
			</tr>
			...
			<tr>
				...
				<td width='15%'>
					<select id="localisation" name="localisation">
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_localisation WHERE Id_Prestation IN (16,316) ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
			...
			<tr>
				...
				<td width='15%'>
					<select id="poste" name="poste">
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_poste WHERE Id_Prestation IN (16,316) ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
								while($rowL=mysqli_fetch_array($result)){
