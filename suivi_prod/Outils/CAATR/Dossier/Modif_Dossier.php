	...
	if(isset($_POST['btnEnregistrer2'])){	
		...
		if(substr($_SESSION['DroitSP'],0,1)=='1'){
			...
			if($_POST['idFILiee']==0 || $_POST['idStatutProdFILiee']=="RETOUR PREPA"){	
				...
				foreach($listePREPA as $type){
					...
					$req.="(".$_POST['idFI'].",79,'".$type."','".$DateDebut."','".$HeureDebut."','".$DateFin."','".$HeureFin."','".addslashes($_POST['commentaire'.$type])."')";
		...
		if(substr($_SESSION['DroitSP'],1,1)=='1'){
			...
			foreach($tabCompagnon as $valeur){
				 if($valeur<>""){
					$tabCompagnon = explode("C_",$valeur);
					$tabTP = explode("TP_",$tabCompagnon[1]);
					$req="INSERT INTO sp_olwfi_travaileffectue (Id_FI,Id_Personne,TempsPasse,TempsPerdu) VALUES (".$_POST['idFI'].",".$tabCompagnon[0].",".$tabTP[0].",".$tabTP[1].")";
					$resultAjour=mysqli_query($bdd,$req);
				 }
			}
		}
					...
					if($nbECME==0){
						...
						$ReqResponsablePostePrestation="
							...
							WHERE
								...
								AND new_competences_personne_poste_prestation.Id_Prestation=79
...
if($_GET){
	...
	if($_GET['Mode']=="M"){
		...
		$req.="sp_olwficheintervention.Id_RetourQUALITE,sp_olwficheintervention.TempsQUALITE,sp_olwficheintervention.CommentaireQUALITE ";
	...
	<tr><td>
		<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
			...
			<tr>
				...
				<td width='15%'>
					<select id="programme" name="programme">
						...
						<option value="ATR" <?php if($row['Programme']=="ATR"){echo "selected";}?>>ATR</option>
					</select>
				</td>
			</tr>
			<tr><td height="4"></td></tr>
			<tr>
				...
				<td width="13%" class="Libelle">&nbsp; N° SOG/GAMME/PARA : </td>
				<td width="13%">
				...
				<td width='13%'>
					<select id="typeDossier" name="typeDossier">
						...
						<option value="Point OSW" <?php if($row['Type']=="Point OSW"){ echo "selected";}?>>Point OSW</option>
					</select>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td width='15%'>
					<select id="client" name="client">
						...
						<?php
							$req="SELECT Id,Libelle,Supprime FROM sp_client WHERE Id_Prestation=79 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
				...
				<td width='15%'>
					<select id="imputation" name="imputation">
						<option value=""></option>
						<option value="AIRBUS ATLANTIC" <?php if($row['Imputation']=="AIRBUS ATLANTIC"){ echo "selected";}?>>AIRBUS ATLANTIC</option>
						<option value="ATR" <?php if($row['Imputation']=="ATR"){ echo "selected";}?>>ATR</option>
					</select>
				</td>
			</tr>
			...
			<tr><td height="4"></td></tr>
			<tr>
				<td width="13%" class="Libelle" height="20">&nbsp; Créateur : </td>
			...
			<tr>
				...
				<td width='15%'>
					<select id="section" name="section">
						<option value=""></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_olwsection WHERE Id_Prestation=79 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
			...
			<tr>
				<td width='13%' class='Libelle'>&nbsp; Zone de travail : <?php echo $etoile; ?></td><td width='15%'>
					<select id='zone' name='zone' <?php echo $disabled; ?>>
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_olwzonedetravail  WHERE Id_Prestation=79 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
	...
	<tr><td>
		<table width="100%" cellpadding="3" cellspacing="0" align="center" class="GeneralInfo">
			...
			<tr>
				...
				<td class="EnTeteTableauCompetences" width="6%" style="text-align:center;">Date PREPA</td>
				...
				if ($nbH>0){
					...
					while($rowH=mysqli_fetch_array($resultH)){
						...
						?>
							...
							<td width="6%" style="text-align:left;<?php echo $bordure;?>"><?php echo AfficheDateFr($rowH['DatePrepa']);?></td>
							<td width="6%" style="text-align:left;<?php echo $bordure;?>"><?php echo $rowH['StatutPrepa'];?></td>
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle, Supprime FROM sp_poste WHERE Id_Prestation=79 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
								...
								<table cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										...
										<div id='Div_Compagnon' style='height:200px;width:200px;overflow:auto;'>
											...
											$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=79 AND SUBSTR(sp_acces.Droit,3,1)='1' ORDER BY Nom, Prenom;";
											$result=mysqli_query($bdd,$req);
											$nbResulta=mysqli_num_rows($result);
											if ($nbResulta>0){
												$i=0;
									...
									<tr>
										<td width="40%" bgcolor='#e4e7f0'>&nbsp;Temps perdu : </td>
									</tr>
									<tr>
										<td width="60%" bgcolor='#e4e7f0'>&nbsp; 
											<input onKeyUp="nombre(this)" style="text-align:center;" onKeypress="if(event.keyCode == 13)AjouterTE()" id="tempsPerdu" name="tempsPerdu" size="5" type="text" value="">
										</td>
									</tr>
									<tr>
							...
							<td bgcolor="#dbeef9" width='20%' valign='top'>
								<table id="tab_TravailEffectue" width='100%' cellpadding='0' cellspacing='0'>
									<tr>
										<td class="Libelle" width="80%">Personne</td>
										<td class="Libelle" width="10%">tps passé</td>
										<td class="Libelle" width="10%">tps perdu</td>
									</tr>
									<?php
											$listeCompagnon="";
											$TempsPasseTotalFI=0;
											$TempsPerduTotalFI=0;
											$req="SELECT Id, Id_Personne, TempsPasse,TempsPerdu,";
											...
											if ($nbResulta>0){
												while($rowCompagnon=mysqli_fetch_array($result)){
													$btn="";
													if(substr($_SESSION['DroitSP'],1,1)=='1'){
														$btn="<a style=\"text-decoration:none;\" href=\"javascript:SupprimerTE('".$rowCompagnon['Id_Personne']."C_".$rowCompagnon['TempsPasse']."TP_".$rowCompagnon['TempsPerdu']."')\">&nbsp;<img src=\"../../../Images/Suppression2.gif\" border=\"0\" alt=\"Suppr\" title=\"Suppr\">&nbsp;&nbsp;</a>";
													}
													echo "<tr id='".$rowCompagnon['Id_Personne']."C_".$rowCompagnon['TempsPasse']."TP_".$rowCompagnon['TempsPerdu']."'>
																<td>".$rowCompagnon['NomPrenom']."</td>
																<td>".$rowCompagnon['TempsPasse']."</td>
																<td>".$rowCompagnon['TempsPerdu']."</td>
																<td>".$btn."</td>
														</tr>";
													$listeCompagnon.=$rowCompagnon['Id_Personne']."C_".$rowCompagnon['TempsPasse']."TP_".$rowCompagnon['TempsPerdu'].";";
													$TempsPasseTotalFI+=$rowCompagnon['TempsPasse'];
													$TempsPerduTotalFI+=$rowCompagnon['TempsPerdu'];
												}
											}
										?>
									</tr>
								...
								<table id="tab_TravailEffectue" width='100%' cellpadding='0' cellspacing='0'>
									<tr>
										<td class="Libelle" width="80%">TOTAL</td>
										...
										<td class="Libelle" width="10%">
											<input type="text" size="4" style="background-color:#dbeef9;border:none" name="tpsPerduFI" id="tpsPerduFI" value="<?php echo $TempsPerduTotalFI;?>" readonly="readonly"/>
										</td>
									</tr>
								</table>
							</td>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledNew;?>>
									...
									if($nbTERA>0 && $disabledNew=="disabled='disabled'"){
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=79 AND TypeStatut='P' ORDER BY Id;";
									}
									elseif($nbTERA>0 && $disabledNew<>"disabled='disabled'"){
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=79 AND TypeStatut='P' AND Id<>'TERA' ORDER BY Id;";
									}
									else{
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=79 AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";
									}
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledNew;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=79 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
						...
						<tr>
							...
							<td bgcolor="#c7e048" width="20%" colspan="3">
								<select name="IQ">
									...
									$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=79 AND SUBSTR(sp_acces.Droit,5,1)=1 ORDER BY NomPrenom;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
							...
							<td width="20%" bgcolor="#c7e048">
								<div id="statutsQualite">
									<select id="statutQualite" name="statutQualite" onchange="Recharge_StatutQualite();" <?php echo $disabledIQNew;?>>
										<option name="" value=""></option>
										<?php
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=79 AND TypeStatut='Q' ORDER BY Id;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="typeECMEPROD" name="typeECMEPROD" style="width:100px;" onchange="Recharge_RefECMEPROD()" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
												...
												$req="SELECT Id, Libelle FROM sp_olwtypeecme WHERE Supprime=false AND Id_Prestation=79 ORDER BY Libelle ";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
										...
										<td bgcolor='#e4e7f0'>
											<div id="listeECMEPROD">
												&nbsp; <select id="referencePROD" name="referencePROD" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
													...
													$req="SELECT Id, Libelle,Id_Type FROM sp_olwecme WHERE Supprime=false AND Id_Prestation=79 ORDER BY Libelle ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													if ($nbResulta>0){
														$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefProduit" name="RefProduit" onkeypress="if(event.keyCode == 13)AjouterProduit()" style="width:130px;">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwingredient WHERE Supprime=false AND Id_Prestation=79 ORDER BY Libelle;";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>
											...
											<select id="RefAIPI" name="RefAIPI" onkeypress="if(event.keyCode == 13)AjouterAIPIS()" style="width:130px;">
												...
												$req.="WHERE new_competences_prestation_qualification.Id_Prestation IN (79,950,79) ";
