...
function AjouterTE(){
	...
	for(var k=0, l=Elements_Compagnons.length; k<l; k++)
	{
		...
		{
			if(Elements_Compagnons[k].value!="0" && (document.getElementById('tempsPasse').value!="" || document.getElementById('tempsPerdu').value!="")){
				if(document.getElementById('travailEffectue').value.indexOf(Elements_Compagnons[k].value+"C_")==-1){
					tempsPerdu=0;
					if(document.getElementById('tempsPerdu').value!=""){
						tempsPerdu=parseFloat(document.getElementById('tempsPerdu').value);
					}
					
					tempsPasse=0;
					if(document.getElementById('tempsPasse').value!=""){
						tempsPasse=parseFloat(document.getElementById('tempsPasse').value);
					}
					
					document.getElementById('travailEffectue').value = document.getElementById('travailEffectue').value+Elements_Compagnons[k].value+"C_"
																	+tempsPasse+"TP_"
																	+tempsPerdu+";";
					var table = document.getElementById("tab_TravailEffectue");
					var row = table.insertRow();
					row.id = Elements_Compagnons[k].value+"C_"
							+tempsPasse+"TP_"
							+tempsPerdu;
					btn="<a style=\"text-decoration:none;\" href=\"javascript:SupprimerTE('"+Elements_Compagnons[k].value+"C_"+
																				tempsPasse+"TP_"
																				+tempsPerdu+"')\">"
						+"&nbsp;<img src=\"../../../Images/Suppression2.gif\" border=\"0\" alt=\"Suppr\" title=\"Suppr\">&nbsp;&nbsp;</a>";
					var Personne = "";
					...
					cell.innerHTML = tempsPasse;
					var cell = row.insertCell(2);
					cell.innerHTML = tempsPerdu;
					var cell = row.insertCell(3);
					cell.innerHTML = btn;
					
					
					//Ajout du temps dans le champs tps FI
					document.getElementById('tpsFI').value=parseFloat(document.getElementById('tpsFI').value)+tempsPasse;
					document.getElementById('tpsPerduFI').value=parseFloat(document.getElementById('tpsPerduFI').value)+tempsPerdu;
					document.getElementById('tpsDossier').value=parseFloat(document.getElementById('tpsDossier').value)+tempsPasse;
				}
			}
		}
	}
...
function SupprimerTE(compagnon){
	...
	tempsFI=compagnon.substr(compagnon.indexOf("_")+1);
	tempsFI=tempsFI.substr(0,tempsFI.indexOf("TP_"));

	tempsPerdu=compagnon.substr(compagnon.indexOf("_")+1);
	tempsPerdu=tempsPerdu.substr(tempsPerdu.indexOf("_")+1);
	
	document.getElementById('travailEffectue').value = document.getElementById('travailEffectue').value.replace(compagnon+";","");
	document.getElementById('tpsFI').value=parseFloat(document.getElementById('tpsFI').value)-parseFloat(tempsFI);
	document.getElementById('tpsPerduFI').value=parseFloat(document.getElementById('tpsPerduFI').value)-parseFloat(tempsPerdu);
	document.getElementById('tpsDossier').value=parseFloat(document.getElementById('tpsDossier').value)-parseFloat(tempsFI);
}
