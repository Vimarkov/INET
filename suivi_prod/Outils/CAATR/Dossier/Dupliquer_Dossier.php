...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		$req.="Id_Dossier,Id_FILiee,Id_Createur,DateCreation,PosteAvionACP,DeroNecessaire,TravailRealise,Commentaire,OutillageSpecifique,StatutPrepa,DatePrepa,PasDePS,PasDeECMEPROD,PasDeECMEQUALITE,PasDeIngredient,ValidationPSCE,ValidationPSIQ";
		...
		if(substr($_SESSION['DroitSP'],4,1)=='1'){
			$req.=",DateInterventionQ,VacationQ,TempsQUALITE,Id_StatutQUALITE,DateCreationQUALITE,Id_RetourQUALITE,Id_QUALITE,CommentaireQUALITE,DateTERC";
		}
		$req.=") VALUES (";
		...
		$req.=$_POST['idDossier'].",".$_POST['idFI'].",".$_SESSION['Id_PersonneSP'].",'".$DateJour."','".addslashes($_POST['poste'])."',".$_POST['DeroNecessaire'].",
			'".addslashes($_POST['travailRealise'])."','".addslashes($_POST['commentaire'])."','".$outillage."' ";
			...
			if(substr($_SESSION['DroitSP'],1,1)=='1'){
				...
				foreach($tabCompagnon as $valeur){
					 if($valeur<>""){
						...
						$req="INSERT INTO sp_olwfi_travaileffectue (Id_FI,Id_Personne,TempsPasse,TempsPerdu) VALUES (".$IdFICree.",".$tabCompagnon[0].",".$tabTP[0].",".$tabTP[1].")";
						$resultAjour=mysqli_query($bdd,$req);
					 }
				}
			}
				...
				if ($nbH>0){
					...
					while($rowH=mysqli_fetch_array($resultH)){
						...
						?>
							...
							<td width="6%" style="text-align:center;<?php echo $bordure;?>"><?php echo $rowH['StatutPrepa'];?></td>
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle FROM sp_poste WHERE Id_Prestation=79 AND Supprime=false ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
							...
							<td bgcolor="#dbeef9" width='20%' valign='top'>
								...
								<table id="tab_TravailEffectue" width='100%' cellpadding='0' cellspacing='0'>
									<tr>
										...
										<td class="Libelle" width="10%">
											<input type="text" size="4" style="background-color:#dbeef9;border:none" name="tpsPerduFI" id="tpsPerduFI" value="0" readonly="readonly"/>
										</td>
									</tr>
								</table>
							</td>
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledCE;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=79 AND Supprime=0 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
