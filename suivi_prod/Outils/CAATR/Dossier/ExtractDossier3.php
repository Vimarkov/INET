...
$req2="SELECT DISTINCT sp_olwficheintervention.Id_Dossier,sp_olwdossier.MSN,sp_olwdossier.Reference,Programme,ReferenceAM,TravailRealise, 
	(SELECT sp_client.Libelle FROM sp_client WHERE sp_client.Id=sp_olwdossier.Id_Client) AS Client 
	";
...
$sheet->setCellValue('C1',utf8_encode("Programme"));
$sheet->setCellValue('D1',utf8_encode("Date du TERC"));
$sheet->setCellValue('E1',utf8_encode("Compagnons"));
$sheet->setCellValue('F1',utf8_encode("Temps objectif"));
$sheet->setCellValue('G1',utf8_encode("Temps passé"));
$sheet->setCellValue('H1',utf8_encode("Productivité"));
$sheet->setCellValue('I1',utf8_encode("Temps perdu"));
$sheet->setCellValue('J1',utf8_encode("N° AM"));
$sheet->setCellValue('K1',utf8_encode("Sujet travail à réaliser"));
$sheet->setCellValue('L1',utf8_encode("Dernier statut"));
$sheet->setCellValue('M1',utf8_encode("% avancement"));
...
$sheet->getStyle('A1:M1')->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'f2f2f2'))));
$sheet->getStyle('A1:M1')->getFont()->setBold(true);
$sheet->getStyle('A1:M1')->getFont()->getColor()->setRGB('1f49a6');
...
if($nbResulta2>0){
	...
	while($row2=mysqli_fetch_array($result2)){
		...
		$calcul="";
		$SommeTempsObjectif=0;
		$reqFI="SELECT Id,TempsObjectif
			FROM sp_olwficheintervention 
			WHERE Id_Dossier=".$row2['Id_Dossier']." ";
		$resultFI=mysqli_query($bdd,$reqFI);
		$nbFI=mysqli_num_rows($resultFI);
		if ($nbFI>0){
			while($rowFI=mysqli_fetch_array($resultFI)){
				$reqSum="SELECT Id_Personne FROM sp_olwfi_travaileffectue LEFT JOIN sp_olwficheintervention ON sp_olwfi_travaileffectue.Id_FI=sp_olwficheintervention.Id WHERE TempsPasse>0 AND sp_olwficheintervention.Id=".$rowFI['Id'];
				$resultSum=mysqli_query($bdd,$reqSum);
				$nbCompagnon=mysqli_num_rows($resultSum);
				$SommeTempsObjectif+=$rowFI['TempsObjectif']*$nbCompagnon;
			}
		}
		
		$nbTempsDossier=0;
		$nbTempsPerduDossier=0;
		$reqSum="SELECT SUM(sp_olwfi_travaileffectue.TempsPasse) AS Tps,
		SUM(sp_olwfi_travaileffectue.TempsPerdu) AS TpsPerdu
		...
		if ($nbSum>0){
			...
			if($rowSum['TpsPerdu']<>""){$nbTempsPerduDossier=$rowSum['TpsPerdu'];}
		}
		
		$compagnons="";
		$reqSum="SELECT DISTINCT (SELECT CONCAT(Nom,' ',Prenom) FROM new_rh_etatcivil WHERE Id=Id_Personne) AS Personne FROM sp_olwfi_travaileffectue LEFT JOIN sp_olwficheintervention ON sp_olwfi_travaileffectue.Id_FI=sp_olwficheintervention.Id WHERE sp_olwficheintervention.Id_Dossier=".$row2['Id_Dossier'];
		$resultSum=mysqli_query($bdd,$reqSum);
		$nbSum=mysqli_num_rows($resultSum);
		if ($nbSum>0){
			while($rowFI=mysqli_fetch_array($resultSum)){
				$compagnons.=$rowFI['Personne']."\n";
			}
		}
		
		$DateTERC=date('0001-01-01');
		$reqSum="SELECT DateTERC FROM sp_olwficheintervention WHERE DateTERC>'0001-01-01' AND sp_olwficheintervention.Id_Dossier=".$row2['Id_Dossier']." ORDER BY DateTERC DESC";
		$resultSum=mysqli_query($bdd,$reqSum);
		$nbSum=mysqli_num_rows($resultSum);
		if ($nbSum>0){
			$rowSum=mysqli_fetch_array($resultSum);
			$DateTERC=$rowSum['DateTERC'];
		}
		
		
		$Statut="";
		$Avancement="";
		$reqSum="SELECT Id_StatutPROD,Id_StatutQUALITE, Avancement FROM sp_olwficheintervention WHERE sp_olwficheintervention.Id_Dossier=".$row2['Id_Dossier']." ORDER BY DateIntervention DESC, Id DESC";
		$resultSum=mysqli_query($bdd,$reqSum);
		$nbSum=mysqli_num_rows($resultSum);
		if ($nbSum>0){
			$rowSum=mysqli_fetch_array($resultSum);
			if($rowSum['Id_StatutQUALITE']<>""){
				$Statut=$rowSum['Id_StatutQUALITE'];
				$Avancement=$rowSum['Avancement'];
			}
			elseif($rowSum['Id_StatutPROD']<>""){
				$Statut=$rowSum['Id_StatutPROD'];
				$Avancement=$rowSum['Avancement'];
			}
		...
		$sheet->setCellValue('C'.$ligne,utf8_encode($row2['Programme']));
		
		if(AfficheDateJJ_MM_AAAA($DateTERC)<>""){
			$date = explode("/",AfficheDateJJ_MM_AAAA($DateTERC));
		...
		$sheet->setCellValue('E'.$ligne,utf8_encode($compagnons));
		$sheet->getStyle('E'.$ligne)->getAlignment()->setWrapText(true);
		$productivite=0;
		if($SommeTempsObjectif>0){
			$productivite=round($nbTempsDossier/$SommeTempsObjectif,2);
		}

		$sheet->setCellValue('F'.$ligne,utf8_encode($SommeTempsObjectif));
		$sheet->setCellValue('G'.$ligne,utf8_encode($nbTempsDossier));
		$sheet->setCellValue('H'.$ligne,utf8_encode($productivite));
		$sheet->setCellValue('I'.$ligne,utf8_encode($nbTempsPerduDossier));
		$sheet->setCellValue('J'.$ligne,utf8_encode($row2['ReferenceAM']));
		$sheet->setCellValue('K'.$ligne,utf8_encode($row2['TravailRealise']));
		$sheet->setCellValue('L'.$ligne,utf8_encode($Statut));
		$sheet->setCellValue('M'.$ligne,utf8_encode($Avancement));
