...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		$req.="SectionACP,CaecACP,Priorite,Titre,DateCreation,TAI_RestantACP,Id_LigneContrat,";
		$req.="Systeme,Structure,Metal,Mastic,Peinture,Id_ZoneDeTravail,Origine,NumOrigine,Id_Statut,Avancement,Id_Retour,CommentaireZICIA,NumSN,Programme)";
		$req.=" VALUES (688,".$_SESSION['Id_PersonneSP'].",".$_POST['msn'].",".$_POST['client'].",'".$_POST['typeDossier']."','".addslashes($_POST['numDossier'])."','".addslashes($_POST['numNC'])."',".$_POST['controleEquipement'].",";
		$req.="'".$_POST['section']."','".$_POST['caec']."',".$_POST['priorite'].",'".addslashes($_POST['titre'])."',";
		$req.="'".$DateJour."',".$_POST['tai'].",".$_POST['ligneContrat'].",";
		...
		$req.="'".addslashes($_POST['commentaireZI'])."','".addslashes($_POST['numSN'])."','".addslashes($_POST['programme'])."')";
		...
		if($IdCree<>0){
			...
			if(substr($_SESSION['DroitSP'],0,1)=='1' || substr($_SESSION['DroitSP'],1,1)=='1'){
				$req.=",DateIntervention,Id_SiteIntervention,Vacation,TempsObjectif";
			}
			...
			if(substr($_SESSION['DroitSP'],0,1)=='1' || substr($_SESSION['DroitSP'],1,1)=='1'){
				...
				$req.= "'".TrsfDate_($_POST['dateIntervention'])."',".$_POST['siteIntervention'].",'".$_POST['vacation']."',".$temps."";
			}
						...
						if($nbECME==0){
							...
							$ReqResponsablePostePrestation="
								...
								WHERE
									...
									AND new_competences_personne_poste_prestation.Id_Prestation=688
	...
	<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
		...
		<tr>
			...
			<td width='15%'>
				<select id="client" name="client" onchange="AfficherSN()">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_client WHERE Id_Prestation=688 AND Supprime=false ORDER BY Libelle";
		...
		<tr>
			...
			echo"<select id='zone' name='zone' ".$disabled.">";
				...
				$req="SELECT Id,Libelle FROM sp_olwzonedetravail  WHERE Id_Prestation=688 AND Supprime=false ORDER BY Libelle;";
				$result=mysqli_query($bdd,$req);
				$nbResulta=mysqli_num_rows($result);
				if ($nbResulta>0){
			...
			<td style="display:none;" width='15%'>
				<select id="section" name="section">
					<option value=""></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_olwsection WHERE Id_Prestation=688 AND Supprime=false ORDER BY Libelle;";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
		...
		<?php
		echo "<td width='13%'class='Libelle'>&nbsp; Contrôle équipement : </td><td width='20%'>";
		?>
		...
		<tr>
			...
			<td width='13%' class='Libelle'>&nbsp; Ligne contrat : <?php echo $etoile; ?></td><td width='15%'>
					<select id='ligneContrat' name='ligneContrat' style="width:150px;" <?php echo $disabled; ?>>
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle FROM sp_lignecontrat  WHERE Id_Prestation=688 AND Supprime=0 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
								while($rowLC=mysqli_fetch_array($result)){
									$selected="";
									echo "<option name='".$rowLC['Id']."' value='".$rowLC['Id']."' ".$selected.">".$rowLC['Libelle']."</option>";
								}
							}
						?>
					</select>
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle FROM sp_poste WHERE Id_Prestation=688 AND Supprime=false ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr style="display:none;">
							...
							<td width="20%">
								<select id="pole" name="pole" <?php echo $disabled;?>>
									<option name="0" value="0"></option>
									<?php
										$req="SELECT Id, Libelle FROM sp_pole WHERE Id_Prestation=688 ORDER BY Libelle;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="siteIntervention" name="siteIntervention" <?php echo $disabledSTCE;?>>
									<option name="0" value="0"></option>
								<?php
									$req="SELECT Id,Libelle FROM sp_olwsiteintervention WHERE Id_Prestation=688 AND Supprime=false ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
								...
								<table cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										...
										<select id="compagnon" id="compagnon" name="compagnon" style="width:130px;" onkeypress="if(event.keyCode == 13)AjouterTE()">
											...
											$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=688 AND SUBSTR(sp_acces.Droit,3,1)='1' ORDER BY Nom, Prenom;";
											$result=mysqli_query($bdd,$req);
											$nbResulta=mysqli_num_rows($result);
											if ($nbResulta>0){
												$i=0;
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledCE;?>>
									<option name="" value=""></option>
									<?php
									$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=688 AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledCE;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=688 AND Supprime=0 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
						...
						<tr>
							...
							<td bgcolor="#c7e048" width="20%" colspan="3">
								<select name="IQ">
									...
									$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=688 AND SUBSTR(sp_acces.Droit,5,1)=1 ORDER BY NomPrenom;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
							...
							<td width="20%" bgcolor="#c7e048">
								<div id="statutsQualite">
									<select id="statutQualite" name="statutQualite" onchange="Recharge_StatutQualite();" <?php echo $disabledIQ;?>>
										<option name="" value=""></option>
										<?php
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=688 AND TypeStatut='Q' ORDER BY Id;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="typeECMEPROD" name="typeECMEPROD" style="width:100px;" onchange="Recharge_RefECMEPROD()" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwtypeecme WHERE Supprime=false AND Id_Prestation=688 ";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
										...
										<td bgcolor='#e4e7f0'>
											<div id="listeECMEPROD">
												&nbsp; <select id="referencePROD" name="referencePROD" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
													...
													$req="SELECT Id, Libelle,Id_Type FROM sp_olwecme WHERE Supprime=false AND Id_Prestation=688 ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													if ($nbResulta>0){
														$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefProduit" name="RefProduit" onkeypress="if(event.keyCode == 13)AjouterProduit()" style="width:130px;">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwingredient WHERE Supprime=false AND Id_Prestation=688 ORDER BY Libelle;";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>
											...
											<select id="RefAIPI" name="RefAIPI" onkeypress="if(event.keyCode == 13)AjouterAIPIS()" style="width:130px;">
												...
												$req.="WHERE new_competences_prestation_qualification.Id_Prestation IN (688,609,421,423,477,316,243,676,605,1023,935) ";
