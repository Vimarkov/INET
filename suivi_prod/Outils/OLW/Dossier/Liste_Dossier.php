	...
	<tr>
		<?php
			if(isset($_GET['Tri'])){
				...
				if($_GET['Tri']=="Client"){
					$_SESSION['TriGeneral']= str_replace("Client ASC,","",$_SESSION['TriGeneral']);
					$_SESSION['TriGeneral']= str_replace("Client DESC,","",$_SESSION['TriGeneral']);
					$_SESSION['TriGeneral']= str_replace("Client ASC","",$_SESSION['TriGeneral']);
					$_SESSION['TriGeneral']= str_replace("Client DESC","",$_SESSION['TriGeneral']);
					if($_SESSION['TriClient']==""){$_SESSION['TriClient']="ASC";$_SESSION['TriGeneral'].= "Client ".$_SESSION['TriClient'].",";}
					elseif($_SESSION['TriClient']=="ASC"){$_SESSION['TriClient']="DESC";$_SESSION['TriGeneral'].= "Client ".$_SESSION['TriClient'].",";}
					else{$_SESSION['TriClient']="";}
				}
				if($_GET['Tri']=="Zone"){
			...
			if($_SESSION['ModeFiltre']=="oui"){
				...
				$req2.="(SELECT sp_client.Libelle FROM sp_client WHERE sp_client.Id=sp_olwdossier.Id_Client) AS Client, ";
				$req2.="sp_olwficheintervention.PosteAvionACP,sp_olwficheintervention.TravailRealise,sp_olwficheintervention.Id_StatutPROD,sp_olwficheintervention.Id_StatutQUALITE,sp_olwficheintervention.Id_RetourQUALITE ";
				$req="FROM sp_olwficheintervention LEFT JOIN sp_olwdossier ON sp_olwficheintervention.Id_Dossier=sp_olwdossier.Id ";
				$req.="WHERE sp_olwdossier.Id_Prestation=688 AND ";
				if($_POST){
					if($_POST['msn']<>""){
					...
					if($_POST['vacation']<>""){
						$req.="sp_olwficheintervention.Vacation='".$_POST['vacation']."' AND ";
					}
				}
				if($_SESSION['MSN2']<>""){
		...
		<td>
			<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
				<tr bgcolor="#00325F">
					...
					<td class="EnTeteTableauCompetences" width="6%" style="text-align:center;"><a style="text-decoration:none;color:#ffffff;font-weight:bold;" id="tri" href="Liste_Dossier.php?Tri=OF">N° NC/AM<?php if($_SESSION['TriNC']=="DESC"){echo "&uarr;";} elseif($_SESSION['TriNC']=="ASC"){echo "&darr;";}?></a></td>
					<td class="EnTeteTableauCompetences" width="4%" style="text-align:center;"><a style="text-decoration:none;color:#ffffff;font-weight:bold;" id="tri" href="Liste_Dossier.php?Tri=Client">Client<?php if($_SESSION['TriClient']=="DESC"){echo "&uarr;";} elseif($_SESSION['TriClient']=="ASC"){echo "&darr;";}?></a></td>
					<td class="EnTeteTableauCompetences" width="10%" style="text-align:center;"><a style="text-decoration:none;color:#ffffff;font-weight:bold;" id="tri" href="Liste_Dossier.php?Tri=Titre">Titre<?php if($_SESSION['TriTitre']=="DESC"){echo "&uarr;";} elseif($_SESSION['TriTitre']=="ASC"){echo "&darr;";}?></a></td>
					...
					<td class="EnTeteTableauCompetences" width="16%" style="text-align:center;"><a style="text-decoration:none;color:#ffffff;font-weight:bold;" id="tri" href="Liste_Dossier.php?Tri=TravailRealise">Travail à réaliser<?php if($_SESSION['TriTravailRealise']=="DESC"){echo "&uarr;";} elseif($_SESSION['TriTravailRealise']=="ASC"){echo "&darr;";}?></a></td>
								...
								while($row=mysqli_fetch_array($result)){		
									...
									?>
										<tr bgcolor="<?php echo $couleur;?>">
											...
											<td style="text-align:center;"><?php echo $row['ReferenceNC'];?></td>
											<td style="text-align:center;"><?php echo $row['Client'];?></td>
											<td style="text-align:center;"><?php echo str_replace("\\","",$row['Titre']);?></td>
											<td style="text-align:center;"><?php echo AfficheDateFR($row['DateIntervention']);?></td>
											<td style="text-align:center;"><?php echo $vacation;?></td>
											<td style="text-align:center;"><?php echo $row['NumFI'];?></td>
											<td style="text-align:center;"><?php echo $row['TravailRealise'];?></td>
											<td style="text-align:center;"><?php echo $row['PosteAvionACP'];?></td>
											<td <?php echo $HoverPROD;?> style="text-align:center;"><?php echo $row['Id_StatutPROD'].$infoBullePROD;?></td>
											<td <?php echo $HoverQUALITE;?> style="text-align:center;"><?php echo $row['Id_StatutQUALITE'].$infoBulleQUALITE;?></td>
											...
											<td width="2%" align="center">
												<?php
													...
													if($nbResultaFI>0){
														$rowFI=mysqli_fetch_array($resultFI);
														if($rowFI['Id']==$row['Id']){
															if(($row['Id_StatutPROD']<>"" && $row['Id_StatutPROD']<>"TERA") || ($row['Id_StatutPROD']=="TERA" && $row['Id_StatutQUALITE']<>"" && $row['Id_StatutQUALITE']<>"TERC") || ($row['Id_StatutQUALITE']=="TERC" && $row['Id_RetourQUALITE']<>"0")){
												?>
