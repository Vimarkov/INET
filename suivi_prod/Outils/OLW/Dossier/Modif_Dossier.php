...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){	
		...
		$req.="Id_LigneContrat=".$_POST['ligneContrat'].", ";
		...
		if(substr($_SESSION['DroitSP'],0,1)=='1' || substr($_SESSION['DroitSP'],1,1)=='1'){
			...
			$req.= "Id_SiteIntervention='".$_POST['siteIntervention']."',";
...
if($_GET){
	...
	if($_GET['Mode']=="M"){
		...
		$req.="sp_olwdossier.Id_Client,sp_olwdossier.Mastic,sp_olwdossier.Peinture,sp_olwdossier.NumSN,sp_olwdossier.TypeACP,sp_olwdossier.Id_LigneContrat, ";
		...
		$req.="sp_olwdossier.DateCreation,sp_olwdossier.TAI_RestantACP,sp_olwdossier.Origine,sp_olwdossier.NumOrigine,sp_olwdossier.Programme,";
		...
		$req.="sp_olwficheintervention.Id_FILiee,sp_olwficheintervention.Id_SiteIntervention,";
			...
			<tr>
				...
				<td width='15%'>
					<select id="client" name="client" onchange="AfficherSN()">
						...
						<?php
							$req="SELECT Id,Libelle,Supprime FROM sp_client WHERE Id_Prestation=688 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
			...
			<tr>
				<td width='13%' class='Libelle'>&nbsp; Zone de travail : <?php echo $etoile; ?></td><td width='15%'>
					<select id='zone' name='zone' <?php echo $disabled; ?>>
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_olwzonedetravail  WHERE Id_Prestation=688 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
				...
				<td style="display:none;" width='15%'>
					<select id="section" name="section">
						<option value=""></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_olwsection WHERE Id_Prestation=688 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
			...
			<tr>
				...
				<td width='13%' class='Libelle'>&nbsp; Ligne contrat : <?php echo $etoile; ?></td><td width='15%'>
					<select id='ligneContrat' name='ligneContrat' style="width:150px;" <?php echo $disabled; ?>>
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_lignecontrat  WHERE Id_Prestation=688 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
								while($rowLC=mysqli_fetch_array($result)){
									$selected="";
									if($rowLC['Id']==$row['Id_LigneContrat']){$selected="selected";}
									if($rowLC['Id']==$row['Id_LigneContrat'] ||$rowLC['Supprime']==false){
			...
			<tr>
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle, Supprime FROM sp_poste WHERE Id_Prestation=688 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
				...
				<td colspan="2" width="33%" valign="top">
					<table width="100%" id="tableProd" cellpadding="0" cellspacing="0" align="center">
						...
						<tr>
							<td width="13%" class="Libelle" bgcolor="#dbeef9">&nbsp; Poste avion : </td>
								...
								<?php
									...
									if ($nbResulta>0){
										while($rowSI=mysqli_fetch_array($result)){
											$selected="";
											if($rowSI['Id']==$row['Id_SiteIntervention']){$selected="selected";}
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledNew;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=688 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
