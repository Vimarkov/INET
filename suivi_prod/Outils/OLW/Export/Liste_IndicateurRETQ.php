...
<head>
	...
	<script language="javascript">
		function OuvreFenetreAjout_Critere(){
			var w=window.open("Ajout_CritereIndicateurQ.php?Type=A","PageCritere","status=no,menubar=no,scrollbars=yes,width=600,height=440");
			w.focus();
		}
		function Suppr_Critere(critere,valeur){
			var w=window.open("Ajout_CritereIndicateurQ.php?Type=S&critere="+critere+"&valeur="+valeur,"PageCritere","status=no,menubar=no,scrollbars=yes,width=600,height=400");
			w.focus();
		}
	</script>
...
<form id="formulaire" class="test" method="POST" action="Liste_IndicateurRETQ.php">
	<tr>
		<td>
			<table width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td width="4"></td>
					<td class="TitrePage">Indicateurs RETQ</td>
				</tr>
			</table>
		</td>
	</tr>
	...
	<tr><td>
		<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
			...
			<?php
				...
				if($_SESSION['Indicateur_Pole']<>""){
					echo "<tr>";
					echo "<td>Pôle : ".$_SESSION['Indicateur_Pole']."</td>";
					echo "</tr>";
				}
			?>
			...
			<?php
				if($_POST){
					if(isset($_POST['BtnRechercher'])){
						...
						//**************** LISTE DES POINTS ***********************//
						$req="SELECT Id_Pole,Id_StatutQUALITE, Id_RetourQUALITE,sp_dossier.PNE, ";
						$req.="(SELECT EstRetour FROM sp_retour WHERE sp_retour.Id=sp_ficheintervention.Id_RetourQUALITE) AS EstRetour ";
						$req.="FROM sp_ficheintervention LEFT JOIN sp_dossier ON sp_ficheintervention.Id_Dossier=sp_dossier.Id ";
						$req.="WHERE sp_ficheintervention.DateInterventionQ>'0001-01-01' AND sp_ficheintervention.VacationQ<>'' AND (Id_StatutQUALITE='CERT' OR Id_StatutQUALITE='TVS') ";
						$req.=$reqCriteres;
						...
						//**************** LISTE DES RETQ ***********************//
						$req="SELECT DISTINCT Id_RetourQUALITE, (SELECT Libelle FROM sp_retour WHERE sp_retour.Id=sp_ficheintervention.Id_RetourQUALITE) AS RetourQUALITE, ";
						$req.="(SELECT EstRetour FROM sp_retour WHERE sp_retour.Id=sp_ficheintervention.Id_RetourQUALITE) AS EstRetour ";
						$req.="FROM sp_ficheintervention LEFT JOIN sp_dossier ON sp_ficheintervention.Id_Dossier=sp_dossier.Id ";
						$req.="WHERE Id_RetourQUALITE<>0 AND sp_ficheintervention.DateInterventionQ>'0001-01-01' AND sp_ficheintervention.VacationQ<>'' AND (Id_StatutQUALITE='CERT' OR Id_StatutQUALITE='TVS') ";
						$req.=$reqCriteres;
						$req.=" ORDER BY RetourQUALITE ";
						$resultRetourQUALITE=mysqli_query($bdd,$req);
						$nbRETP=mysqli_num_rows($resultRetourQUALITE);
						//***********************************************************//
						...
						//STATUTS
						$ListeStatut= array('CERT hors PNE','CERT PNE','TVS');
										...
										if ($nbResulta>0){
											...
											while($row=mysqli_fetch_array($result)){
												...
												elseif($value=="CERT PNE"){
													...
													if($row['Id_StatutQUALITE']==$value && $row['Id_Pole']==$rowPole['Id_Pole']){$nb++;}
												}
											}
										}
								...
								if ($nbResulta>0){
									...
									while($row=mysqli_fetch_array($result)){
										...
										elseif($value=="CERT PNE"){
											...
											if($row['Id_StatutQUALITE']==$value){$nb++;}
										}
									}
								}
								echo "<td align='center'  style='border:1px #000000 solid;'>".$nb."</td>";
						...
						//RETQ
						if ($nbRETP>0){
							mysqli_data_seek($resultRetourQUALITE,0);
							while($rowRETP=mysqli_fetch_array($resultRetourQUALITE)){
									$bgcolor="";
									if($rowRETP['EstRetour']==1){
										$bgcolor="bgcolor='#e1fff5'";
									}
									echo "<tr><td style='border:1px #000000 solid;' ".$bgcolor.">".$rowRETP['RetourQUALITE']."</td>";
									if ($nbPole>0){
										mysqli_data_seek($resultPole,0);
										while($rowPole=mysqli_fetch_array($resultPole)){
											$nb=0;
											if ($nbResulta>0){
												mysqli_data_seek($result,0);
												while($row=mysqli_fetch_array($result)){
													if($row['Id_RetourQUALITE']==$rowRETP['Id_RetourQUALITE'] && $row['Id_Pole']==$rowPole['Id_Pole']){$nb++;}
												}
											}
											echo "<td align='center'  style='border:1px #000000 solid;'>".$nb."</td>";
										}
									...
									if ($nbResulta>0){
										mysqli_data_seek($result,0);
										while($row=mysqli_fetch_array($result)){
											if($row['Id_RetourQUALITE']==$rowRETP['Id_RetourQUALITE']){$nb++;}
										}
									}
									echo "<td align='center'  style='border:1px #000000 solid;'>".$nb."</td>";
									echo "</tr>";
						...
						if ($nbPole>0){
							...
							while($rowPole=mysqli_fetch_array($resultPole)){
									...
									while($rowRETP=mysqli_fetch_array($resultRetourQUALITE)){
										if($rowRETP['EstRetour']==1){
											if ($nbResulta>0){
												mysqli_data_seek($result,0);
												while($row=mysqli_fetch_array($result)){
													if($row['Id_Pole']==$rowPole['Id_Pole'] && $row['Id_RetourQUALITE']==$rowRETP['Id_RetourQUALITE']){$Total++;}
												}
											}
										}
									}
									...
									while($rowRETP=mysqli_fetch_array($resultRetourQUALITE)){
										if($rowRETP['EstRetour']==1){
											...
											if ($nbResulta>0){
												mysqli_data_seek($result,0);
												while($row=mysqli_fetch_array($result)){
													if($row['Id_Pole']==$rowPole['Id_Pole'] && $row['Id_RetourQUALITE']==$rowRETP['Id_RetourQUALITE']){$nb++;}
												}
											}
											if($nb>0){
												$valeur="0%";
												if($Total>0){$valeur=round(($nb/$Total)*100,0)."% ";}
												$arrayNom[]=$valeur.$rowRETP['RetourQUALITE'];
												$arraynbRetour[]=$nb;
											}
										}
									}
								...
								$myPicture->drawGradientArea(0,0,799,25,DIRECTION_VERTICAL,array("StartR"=>0,"StartG"=>0,"StartB"=>0,"EndR"=>50,"EndG"=>50,"EndB"=>50,"Alpha"=>100));
								...
								$myPicture->setFontProperties(array("FontName"=>"../../../pChart/fonts/verdana.ttf","FontSize"=>6,"R"=>0,"G"=>0,"B"=>0));

								/* Enable shadow computing */ 
								$myPicture->setShadow(TRUE,array("X"=>2,"Y"=>2,"R"=>0,"G"=>0,"B"=>0,"Alpha"=>50));
								$myPicture->setGraphArea(26,40,800,400);
								
								/* Create the pPie object */ 
								$PieChart = new pPie($myPicture,$MyData);

								/* Draw an AA pie chart */ 
								$PieChart->draw3DPie(400,200,array("Radius"=>170,"WriteValues"=>FALSE,"DrawLabels"=>TRUE,"LabelStacked"=>TRUE,"Border"=>TRUE));
							...
							while($rowRETP=mysqli_fetch_array($resultRetourQUALITE)){
								if($rowRETP['EstRetour']==1){
									if ($nbResulta>0){
										mysqli_data_seek($result,0);
										while($row=mysqli_fetch_array($result)){
											if($row['Id_RetourQUALITE']==$rowRETP['Id_RetourQUALITE']){$Total++;}
										}
									}
								}
							}
