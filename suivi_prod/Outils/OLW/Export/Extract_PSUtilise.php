...
$sheet->setCellValue('H1',utf8_encode("Client"));
...
$req="SELECT 
	...
	(SELECT (SELECT Libelle FROM sp_client WHERE Id=Id_Client) FROM sp_olwdossier WHERE sp_olwdossier.Id=sp_olwficheintervention.Id_Dossier) AS Client,
	...
	WHERE (SELECT Id_Prestation FROM sp_olwdossier WHERE sp_olwdossier.Id=sp_olwficheintervention.Id_Dossier)=688
...
if ($nbResulta>0){	
	while($row=mysqli_fetch_array($result)){
		...
		$sheet->setCellValue('H'.$ligne,utf8_encode($row['Client']));
