			...
			<tr>
				...
				<td width='15%'>
					<select id="client" name="client">
						...
						<?php
							$req="SELECT Id,Libelle,Supprime FROM sp_client WHERE Id_Prestation = -16 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
			...
			<tr>
				...
				<td width='15%'>
					<select id="localisation" name="localisation">
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_localisation WHERE Id_Prestation = -16 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
			...
			<tr>
				...
				<td width='15%'>
					<select id="poste" name="poste">
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_poste WHERE Id_Prestation = -16 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
								while($rowL=mysqli_fetch_array($result)){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledNew;?>>
									...
									if($nbTERA==0){$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation = -16 AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";}
									else{$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation = -16 AND TypeStatut='P' AND Id<>'TERA' ORDER BY Id;";}
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										while($rowStatut=mysqli_fetch_array($result)){
