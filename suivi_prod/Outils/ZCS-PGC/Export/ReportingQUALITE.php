...
elseif($vacation=="N" && $NumJour=="1"){
	$req.="WHERE sp_ficheintervention.DateInterventionQ='".$laVeille."' AND sp_ficheintervention.VacationQ='VSD Nuit' ";
}
else{
	$req.="WHERE sp_ficheintervention.DateInterventionQ='".$leJour."' AND sp_ficheintervention.VacationQ='".$vacation."' ";
}
...
$req.="FROM sp_ficheintervention LEFT JOIN sp_dossier ON sp_ficheintervention.Id_Dossier=sp_dossier.Id ";
$req.="WHERE sp_ficheintervention.DateCreation>='2016-05-23' AND sp_ficheintervention.DateIntervention<='".$leJour."' AND (sp_ficheintervention.Id_StatutPROD='QARJ' OR sp_ficheintervention.Id_StatutPROD='REWORK') AND ( sp_ficheintervention.Id_StatutQUALITE='' OR ( ";
if($laVacation=="VSD"){
...
elseif($vacation=="N" && $NumJour=="1"){
	$req.=" sp_ficheintervention.DateInterventionQ='".$laVeille."'  AND sp_ficheintervention.Id_StatutQUALITE<>'' AND sp_ficheintervention.VacationQ='VSD Nuit' )) ";
}
else{
	$req.=" sp_ficheintervention.DateInterventionQ='".$leJour."' AND sp_ficheintervention.Id_StatutQUALITE<>'' AND sp_ficheintervention.VacationQ='".$vacation."')) ";
}
...
if($laVacation=="VSD"){
	$object = "Reporting QUALITE - du ".$leVendredi." | ".$leSamedi." | ".$leDimanche." Vacation ".$laVacation;
}
else{
	$object = "Reporting QUALITE - du ".$dateReporting." Vacation ".$laVacation;
}
