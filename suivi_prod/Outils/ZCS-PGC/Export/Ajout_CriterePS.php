			...
			<td width=70%>
				<select name="compagnon">
					...
					$req="SELECT DISTINCT sp_olwfi_travaileffectue.Id_Personne AS Id, 
						...
						WHERE (SELECT Id_Prestation FROM sp_olwdossier WHERE Id=sp_olwficheintervention.Id_Dossier)=1539
						ORDER BY Personne;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
			...
			<td width=70%>
				<select name="qualiticien">
					...
					$req="SELECT DISTINCT sp_olwficheintervention.Id_QUALITE AS Id, 
						...
						WHERE sp_olwdossier.Id_Prestation=1539
						ORDER BY Personne;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
