...
function AjouterTE(){
	...
	for(var k=0, l=Elements_Compagnons.length; k<l; k++)
	{
		...
		{
			if(Elements_Compagnons[k].value!="0"){
				if(document.getElementById('travailEffectue').value.indexOf(Elements_Compagnons[k].value+';')==-1){
					document.getElementById('travailEffectue').value = document.getElementById('travailEffectue').value+Elements_Compagnons[k].value+';';
...
function VerifChamps(){
	...
	if(formulaire.codeUsine.value==''){alert('Vous n\'avez pas renseigné le code usine.');return false;}
	if(formulaire.tai.value==''){alert('Vous n\'avez pas renseigné le TAI.');return false;}
