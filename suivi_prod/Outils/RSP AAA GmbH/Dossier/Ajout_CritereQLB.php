		...
		<tr>
			...
			<td >
				<select name="type">
					...
					<?php
					$req="SELECT DISTINCT sp_atram.Id_Type, sp_atrtype.Libelle FROM sp_atram LEFT JOIN sp_atrtype ON sp_atram.Id_Type = sp_atrtype.Id WHERE sp_atram.Id_Prestation=1047 ORDER BY Libelle;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
		...
		<tr>
			...
			<td >
				<select name="localisation">
					<option name="" value=""></option>
					<?php
					$req="SELECT DISTINCT sp_atrcqlb.Id_Localisation, sp_atrlocalisation.Libelle FROM sp_atrcqlb LEFT JOIN sp_atrlocalisation ON sp_atrcqlb.Id_Localisation = sp_atrlocalisation.Id WHERE sp_atrcqlb.Id_Prestation=1047 ORDER BY Libelle;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
						while($row=mysqli_fetch_array($result)){
