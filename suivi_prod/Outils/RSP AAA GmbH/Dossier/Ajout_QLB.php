...
if($_POST){
	if($_POST['Mode']=="A"){
		...
		$requete.="VALUES ('".$DateJour."',".$_SESSION['Id_PersonneSP'].",1047,".$_POST['msn'].",'".$_POST['numCQLB']."','".$_POST['numCV']."',";
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="A" || $_GET['Mode']=="M"){
		$req="SELECT OrdreMontage, Designation FROM sp_atrot WHERE Id_Prestation=1047";
						...
						<tr>
							...
							<td>
								<select name='type' id='type'>
									<option value='0'></option>
									<?php
										$req="SELECT Id, Libelle, Supprime FROM sp_atrtype WHERE Id_Prestation=1047 ORDER BY Libelle ";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											while($rowType=mysqli_fetch_array($result)){
