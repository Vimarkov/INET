...
$req2.="(SELECT sp_olwzonedetravail.Libelle FROM sp_olwzonedetravail WHERE sp_olwzonedetravail.Id=sp_olwdossier.Id_ZoneDeTravail) AS Zone,TempsObjectif, ";
...
$req.="WHERE sp_olwdossier.Id_Prestation=418 AND ";
...
$sheet->setCellValue('R1',utf8_encode("Calcul Temps objectif de l'intervention"));
$sheet->setCellValue('S1',utf8_encode("Temps objectif de l'intervention"));
$sheet->setCellValue('T1',utf8_encode("Temps passé sur l'intervention"));
$sheet->setCellValue('U1',utf8_encode("Calcul Temps objectif du dossier"));
$sheet->setCellValue('V1',utf8_encode("Temps objectif du dossier"));
$sheet->setCellValue('W1',utf8_encode("Temps passé sur le dossier"));

$sheet->getStyle('A1:W1')->getBorders()->applyFromArray(array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN ,'color' => array('rgb' => '#000000'))));
$sheet->getStyle('A1:W1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
$sheet->getStyle('A1:W1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$sheet->getStyle('A1:W1')->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'f2f2f2'))));
$sheet->getStyle('A1:W1')->getFont()->setBold(true);
$sheet->getStyle('A1:W1')->getFont()->getColor()->setRGB('1f49a6');
	...
	while($row2=mysqli_fetch_array($result2)){
		...
		if ($nbFI>0){
			while($rowFI=mysqli_fetch_array($resultFI)){
				...
				if($calcul<>""){$calcul.="+ ";}
				$calcul.="(".$rowFI['TempsObjectif']." x ".$nbCompagnon.")\n";
				$SommeTempsObjectif+=$rowFI['TempsObjectif']*$nbCompagnon;
			}
		}
		
		$nbTempsDossier=0;
		$reqSum="SELECT SUM(sp_olwfi_travaileffectue.TempsPasse) AS Tps FROM sp_olwfi_travaileffectue LEFT JOIN sp_olwficheintervention ON sp_olwfi_travaileffectue.Id_FI=sp_olwficheintervention.Id WHERE sp_olwficheintervention.Id_Dossier=".$row2['Id_Dossier'];
		$resultSum=mysqli_query($bdd,$reqSum);
		...
		$calculInter="";
		$TempsObjectifInter=$row2['TempsObjectif'];
		$nbTempsDossierInter=0;
		
		$reqSum="SELECT Id_Personne FROM sp_olwfi_travaileffectue LEFT JOIN sp_olwficheintervention ON sp_olwfi_travaileffectue.Id_FI=sp_olwficheintervention.Id WHERE TempsPasse>0 AND sp_olwficheintervention.Id=".$row2['Id'];
		$resultSum=mysqli_query($bdd,$reqSum);
		$nbCompagnonInter=mysqli_num_rows($resultSum);
		if($calculInter<>""){$calculInter.="+ ";}
		$calculInter.="(".$TempsObjectifInter." x ".$nbCompagnonInter.")\n";
		
		$reqSum="SELECT SUM(sp_olwfi_travaileffectue.TempsPasse) AS Tps FROM sp_olwfi_travaileffectue LEFT JOIN sp_olwficheintervention ON sp_olwfi_travaileffectue.Id_FI=sp_olwficheintervention.Id WHERE sp_olwficheintervention.Id=".$row2['Id'];
		$resultSum=mysqli_query($bdd,$reqSum);
		$nbSum=mysqli_num_rows($resultSum);
		if ($nbSum>0){
			$rowSum=mysqli_fetch_array($resultSum);
			if($rowSum['Tps']<>""){$nbTempsDossierInter=$rowSum['Tps'];}
		}
		...
		$sheet->setCellValue('R'.$ligne,utf8_encode($calculInter));
		$sheet->getStyle('R'.$ligne)->getAlignment()->setWrapText(true);
		$sheet->setCellValue('S'.$ligne,utf8_encode($TempsObjectifInter));
		$sheet->setCellValue('T'.$ligne,utf8_encode($nbTempsDossierInter));
		
		$sheet->setCellValue('U'.$ligne,utf8_encode($calcul));
		$sheet->getStyle('U'.$ligne)->getAlignment()->setWrapText(true);
		$sheet->setCellValue('V'.$ligne,utf8_encode($SommeTempsObjectif));
		$sheet->setCellValue('W'.$ligne,utf8_encode($nbTempsDossier));
		$sheet->getStyle('A'.$ligne.':W'.$ligne)->getBorders()->applyFromArray(array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN ,'color' => array('rgb' => '#000000'))));
		$ligne++;
	}
}
