...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		$req.=" VALUES (576,".$_SESSION['Id_PersonneSP'].",".$_POST['msn'].",".$_POST['client'].",'".$_POST['typeDossier']."','".addslashes($_POST['numDossier'])."','".addslashes($_POST['numNC'])."',".$_POST['controleEquipement'].",";
...
<table width="100%" cellpadding="0" cellspacing="0" align="center">
	...
	<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
		...
		<tr>
			...
			<td width='15%'>
				<select id="client" name="client" onchange="AfficherSN()">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_client WHERE Id_Prestation=576 AND Supprime=false ORDER BY Libelle";
		...
		<?php
			...
			$req="SELECT Id,Reference FROM sp_olwdossier WHERE Id_Prestation=576 ";
		...
		<tr>
			...
			echo"<select id='zone' name='zone' ".$disabled.">";
				...
				$req="SELECT Id,Libelle FROM sp_olwzonedetravail  WHERE Id_Prestation=576 AND Supprime=false ORDER BY Libelle;";
				$result=mysqli_query($bdd,$req);
				$nbResulta=mysqli_num_rows($result);
				if ($nbResulta>0){
			...
			<td style="display:none;" width='15%'>
				<select id="section" name="section">
					<option value=""></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_olwsection WHERE Id_Prestation=576 AND Supprime=false ORDER BY Libelle;";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledCE;?>>
									<option name="" value=""></option>
									<?php
									$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=576 AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
