...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		$req.=" VALUES (1792,".$_SESSION['Id_PersonneSP'].",".$_POST['msn'].",".$tai.",".$_POST['client'].",'".addslashes($_POST['numDossier'])."',
		...
		<tr>
			...
			<td width='15%'>
				<select id="client" name="client">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_client WHERE Id_Prestation = 1792 AND Supprime=false ORDER BY Libelle";
		...
		<?php
			...
			<td width='15%'>
				<select id="localisation" name="localisation">
					<option name='0' value='0'></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_localisation WHERE Id_Prestation = 1792 AND Supprime=false ORDER BY Libelle";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
		...
		<tr>
			...
			<td width='15%'>
				<select id="poste" name="poste">
					<option name='0' value='0'></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_poste WHERE Id_Prestation = 1792 AND Supprime=false ORDER BY Libelle";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
							while($rowL=mysqli_fetch_array($result)){
					...
					<table width="100%" id="tableProd" cellpadding="0" cellspacing="0" align="center">
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledCE;?>>
									<option name="" value=""></option>
									<?php
									$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation = 1792 AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										while($rowStatut=mysqli_fetch_array($result)){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation = 1792 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
