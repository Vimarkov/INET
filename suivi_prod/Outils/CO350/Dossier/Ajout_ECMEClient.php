...
if($_POST){
	if($_POST['Mode']=="A"){
		$droit="";
		$requete="INSERT INTO sp_olwecmeclient (Libelle,Id_Type,Id_Prestation) VALUES ('".addslashes($_POST['libelle'])."',".$_POST['type'].",1792) ";
		$result=mysqli_query($bdd,$requete);
		echo "<script>FermerEtRecharger();</script>";
	}
	elseif($_POST['Mode']=="M"){
		$requete="UPDATE sp_olwecmeclient SET ";
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="A" || $_GET['Mode']=="M"){
		if($_GET['Id']!='0')
		{
			$result=mysqli_query($bdd,"SELECT Id, Libelle,Id_Type FROM sp_olwecmeclient WHERE Id=".$_GET['Id']);
			$Ligne=mysqli_fetch_array($result);
		}
		
		//LIste des ECME existants
		$req="SELECT Id,Libelle FROM sp_olwecmeclient WHERE Id_Prestation=1792 AND Supprime=0 ";
...
?>
		...
		<form id="formulaire" method="POST" action="Ajout_ECMEClient.php" onSubmit="return VerifChamps();">
			...
			<tr>
				...
				<td>
					<select name='type' id='type'>
						...
						<?php
							$req="SELECT Id, Libelle, Supprime FROM sp_olwtypeecmeclient WHERE Id_Prestation=1792 ORDER BY Libelle ";
