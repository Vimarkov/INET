		...
		<tr>
			...
			<td >
				<select name="Client">
					<option value=""></option>
					<?php
					$req="SELECT DISTINCT sp_olwdossier.Id_Client,(SELECT Libelle FROM sp_client WHERE sp_client.Id=sp_olwdossier.Id_Client) AS Client FROM sp_olwdossier WHERE Id_Prestation=1792 ORDER BY Client;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
		...
		<tr>
			...
			<td>
				<select name="Section">
					<option value=""></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_olwsection WHERE Id_Prestation=1792 AND Supprime=false ORDER BY Libelle;";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
		...
		<tr>
			...
			<td>
				<select name="StatutDossier">
					<option name="" value=""></option>
					<?php
					$req="SELECT DISTINCT Id FROM sp_olwstatut WHERE Id_Prestation=1792 AND TypeStatut IN ('P','Q') ORDER BY Id;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
						while($rowStatut=mysqli_fetch_array($result)){
		...
		<tr>
			...
			<td>
				<select name="StatutPROD">
					...
					<?php
					$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=1792 AND TypeStatut IN ('P') ORDER BY Id;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
						while($rowStatut=mysqli_fetch_array($result)){
		...
		<tr>
			...
			<td >
				<select name="IQ">
					...
					$req.="FROM sp_olwficheintervention WHERE (SELECT sp_olwdossier.Id_Prestation FROM sp_olwdossier WHERE sp_olwdossier.Id=sp_olwficheintervention.Id_Dossier)=1792 ORDER BY NomPrenom;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
						while($row=mysqli_fetch_array($result)){
		...
		<tr>
			...
			<td>
				<select name="StatutQUALITE">
					...
					<option value="">(vide)</option>
					<?php
					$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=1792 AND TypeStatut IN ('Q') ORDER BY Id;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
						while($rowStatut=mysqli_fetch_array($result)){
