	...
	if(isset($_POST['btnEnregistrer2'])){
		...
		if(substr($_SESSION['DroitSP'],0,1)=='1' || substr($_SESSION['DroitSP'],1,1)=='1'){
			$req.= ",DateIntervention='".TrsfDate_($_POST['dateIntervention'])."',
				TravailRealise='".addslashes($_POST['travailRealise'])."',";
...
if($_GET){
	...
	if($_GET['Mode']=="M"){
		...
		$req.="
			TravailRealise,
			(SELECT CONCAT(Nom,' ',Prenom) FROM new_rh_etatcivil WHERE new_rh_etatcivil.Id=sp_olwficheintervention.Id_Createur) AS CreateurFI,
			sp_olwficheintervention.DateCreation AS DateCreationFI,
			sp_olwficheintervention.DateIntervention,sp_olwficheintervention.Vacation,sp_olwficheintervention.Id_StatutPROD,sp_olwficheintervention.DateTERA,";
			...
			<tr>
				...
				<td width='15%'>
					<select id="client" name="client">
						...
						<?php
							$req="SELECT Id,Libelle,Supprime FROM sp_client WHERE Id_Prestation = 1598 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
			...
			<tr>
				...
				<td width='15%'>
					<select id="localisation" name="localisation">
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_localisation WHERE Id_Prestation = 1598 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
			...
			<tr>
				...
				<td width='15%'>
					<select id="poste" name="poste">
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_poste WHERE Id_Prestation = 1598 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
								while($rowL=mysqli_fetch_array($result)){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledNew;?>>
									...
									if($nbTERA==0){$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation = 1598 AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";}
									else{$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation = 1598 AND TypeStatut='P' AND Id<>'TERA' ORDER BY Id;";}
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										while($rowStatut=mysqli_fetch_array($result)){
