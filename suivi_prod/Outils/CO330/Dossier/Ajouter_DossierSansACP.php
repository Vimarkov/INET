...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		//Ajout du dossier
		$req="INSERT INTO sp_olwdossier (Id_Prestation,Id_Personne,MSN,TAI_RestantACP,Id_Client,Reference,DateDossier,HeureDossier,
				TypeACP,CaecACP,CodeUsine,Priorite,Titre,DateCreation,DatePrevisionnelleIntervention,
				Ajusteur,Elec,Meca,Id_ZoneDeTravail,Id_Statut,Id_Retour,Programme,Id_StatutPREPA,Id_Poste)";
		
		$req.=" VALUES (1598,".$_SESSION['Id_PersonneSP'].",".$_POST['msn'].",".$tai.",".$_POST['client'].",'".addslashes($_POST['numDossier'])."',
			'".$DateJour."','".$HeureJour."','".addslashes($_POST['typeDossier'])."','".$_POST['caec']."','".$_POST['codeUsine']."',
		...
		if($IdCree<>0){
			...
			$req.=")
				VALUES (".$IdCree.",'".$_POST['numIC']."','".addslashes($_POST['travailRealise'])."',".$_SESSION['Id_PersonneSP'].",'".$DateJour."',";
			...
			$statutProd='';
			$retourProd=0;
			if(isset($_POST['statutProd'])){$statutProd=$_POST['statutProd'];}
			if(isset($_POST['retourProd'])){$retourProd=$_POST['retourProd'];}
			$req.= "'".TrsfDate_($_POST['dateIntervention'])."','".$_POST['vacation']."',".$tempsProd.",'".$statutProd."','".$DateJour."',".$retourProd.",
						...
						if($nbECME==0){
							...
							$ReqResponsablePostePrestation="
								...
								WHERE
									...
									AND new_competences_personne_poste_prestation.Id_Prestation = 1598
...
<table width="100%" cellpadding="0" cellspacing="0" align="center">
	...
	<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
		...
		<tr>
			...
			<td width='15%'>
				<select id="client" name="client">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_client WHERE Id_Prestation = 1598 AND Supprime=false ORDER BY Libelle";
		...
		<?php
			...
			<td width='15%'>
				<select id="localisation" name="localisation">
					<option name='0' value='0'></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_localisation WHERE Id_Prestation = 1598 AND Supprime=false ORDER BY Libelle";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
		...
		<tr>
			...
			<td width="15%">
				<select id="statutPrepa" name="statutPrepa" >
					...
					<?php
					$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation = 1598 AND TypeStatut='R' ORDER BY Id;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
			...
			<td width='15%'>
				<select id="poste" name="poste">
					<option name='0' value='0'></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_poste WHERE Id_Prestation = 1598 AND Supprime=false ORDER BY Libelle";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
							while($rowL=mysqli_fetch_array($result)){
		...
		<tr>
			...
			<td width='15%'><input onKeyUp='nombre(this)' id='tai' name='tai' value='' size='5'></td>
					...
					<table width="100%" cellpadding="0" cellspacing="0" align="center">
						...
						<tr>
							<td colspan="2" width="20%">
								<textarea id="travailRealise" name="travailRealise" rows="5" cols="40" style="resize:none;" <?php echo $readSTCE;?>></textarea>
							</td>
						</tr>
						<tr><td height="4" colspan="2"></td></tr>
					</table>
					<table width="100%" id="tableProd" cellpadding="0" cellspacing="0" align="center">
								...
								<table cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										...
										<div id='Div_Compagnon' style='height:200px;width:200px;overflow:auto;'>
											...
											$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation IN (1598) AND SUBSTR(sp_acces.Droit,3,1)='1' ORDER BY Nom, Prenom;";
											$result=mysqli_query($bdd,$req);
											$nbResulta=mysqli_num_rows($result);
											if ($nbResulta>0){
												$i=0;
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledCE;?>>
									<option name="" value=""></option>
									<?php
									$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation = 1598 AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										while($rowStatut=mysqli_fetch_array($result)){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation = 1598 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
						...
						<tr>
							...
							<td bgcolor="#c7e048" width="20%" colspan="3">
								<select name="IQ">
									...
									$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation = 1598 AND SUBSTR(sp_acces.Droit,5,1)=1 ORDER BY NomPrenom;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
							...
							<td width="20%" bgcolor="#c7e048">
								<div id="statutsQualite">
									<select id="statutQualite" name="statutQualite" onchange="Recharge_StatutQualite();" <?php echo $disabledIQ;?>>
										<option name="" value=""></option>
										<?php
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation = 1598 AND TypeStatut='Q' ORDER BY Id;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="typeECMEPROD" name="typeECMEPROD" style="width:100px;" onchange="Recharge_RefECMEPROD()" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
												...
												$req="SELECT Id, Libelle FROM sp_olwtypeecme WHERE Supprime=false AND Id_Prestation = 1598 ";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
										...
										<td bgcolor='#e4e7f0'>
											<div id="listeECMEPROD">
												&nbsp; <select id="referencePROD" name="referencePROD" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
													...
													$req="SELECT Id, Libelle,Id_Type FROM sp_olwecme WHERE Supprime=false AND Id_Prestation = 1598 ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													if ($nbResulta>0){
														$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="typeECMECLIENT" name="typeECMECLIENT" onchange="Recharge_RefECMECLIENT()" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMECLIENT()">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwtypeecmeclient WHERE Supprime=false AND Id_Prestation = 1598 ";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
										...
										<td bgcolor='#e4e7f0'>
											<div id="listeECMECLIENT">
												&nbsp; <select id="referenceClient" name="referenceClient" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMECLIENT()">
													<?php
													echo"<option value='0'></option>";
													$req="SELECT Id, Libelle,Id_Type FROM sp_olwecmeclient WHERE Id_Prestation = 1598 AND Supprime=0 ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													if ($nbResulta>0){
														$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefProduit" name="RefProduit" onkeypress="if(event.keyCode == 13)AjouterProduit()" style="width:130px;">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwingredient WHERE Supprime=false AND Id_Prestation = 1598 ORDER BY Libelle;";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>
											...
											<select id="RefAIPI" name="RefAIPI" onkeypress="if(event.keyCode == 13)AjouterAIPIS()" style="width:130px;">
												...
												$req.="WHERE new_competences_prestation_qualification.Id_Prestation = 1598 ";
