...
if($_POST){
	if($_POST['Mode']=="A"){
		...
		$req="DELETE FROM sp_acces WHERE Id_Prestation=1598 AND Id_Personne=".$_POST['personne'];
		$result=mysqli_query($bdd,$req);
		
		$req="INSERT INTO sp_acces (Id_Personne,Id_Prestation,Droit) VALUES (".$_POST['personne'].",1598,'".$droit."')";
		$result=mysqli_query($bdd,$req);
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="A" || $_GET['Mode']=="M"){
		if($_GET['Id']!='0'){
			...
			$result=mysqli_query($bdd,"SELECT Id, Droit FROM sp_acces WHERE Id_Prestation=1598 AND Id_Personne=".$_GET['Id']);
			...
			<tr class="TitreColsUsers">
				...
				<td>
					...
					if($_GET['Mode']=="A"){
						...
						$req="SELECT new_rh_etatcivil.Id, Nom, Prenom FROM new_rh_etatcivil WHERE NOT EXISTS (SELECT Id FROM sp_acces WHERE sp_acces.Id_Prestation=1598 AND sp_acces.Id_Personne=new_rh_etatcivil.Id) ORDER BY Nom, Prenom;";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
...
<?php
	...
	//Mode suppression
	{
		...
		*/
		...
		$req="DELETE FROM sp_acces WHERE Id_Prestation=1598 AND Id_Personne=".$_GET['Id'];
		$result=mysqli_query($bdd,$req);
