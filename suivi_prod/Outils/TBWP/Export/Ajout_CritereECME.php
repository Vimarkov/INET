		...
		elseif($_GET['critere']=="type"){
			...
			foreach($tab as $Id){
				if($Id<>""){
					...
					$req="SELECT Libelle FROM sp_typeecme WHERE Id=".$Id;
		...
		<tr>
			...
			<td width=80%>
				<select name="type">
					...
					<?php
					$req="SELECT DISTINCT sp_fi_ecme.Id_TypeECME AS Id, (SELECT Libelle FROM sp_typeecme ";
					$req.="WHERE sp_typeecme.Id=sp_fi_ecme.Id_TypeECME) AS Libelle 
					FROM sp_fi_ecme 
