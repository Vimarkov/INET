		...
		<tr>
			...
			<td width=70%>
				<select name="compagnon">
					...
					<?php
					$req="SELECT DISTINCT sp_fi_travaileffectue.Id_Personne AS Id, 
						(SELECT CONCAT(Nom,' ',Prenom) FROM new_rh_etatcivil 
						WHERE new_rh_etatcivil.Id=sp_fi_travaileffectue.Id_Personne) AS Personne 
						FROM sp_fi_travaileffectue 
						LEFT JOIN sp_ficheintervention
						ON sp_fi_travaileffectue.Id_FI=sp_ficheintervention.Id
						ORDER BY Personne;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
			...
			<td width=70%>
				<select name="qualiticien">
					...
					$req="SELECT DISTINCT sp_ficheintervention.Id_QUALITE AS Id, 
						(SELECT CONCAT(Nom,' ',Prenom) FROM new_rh_etatcivil 
						WHERE new_rh_etatcivil.Id=sp_ficheintervention.Id_QUALITE) AS Personne 
						FROM sp_ficheintervention 
						LEFT JOIN sp_dossier
						ON sp_ficheintervention.Id_Dossier=sp_dossier.Id
						ORDER BY Personne;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
