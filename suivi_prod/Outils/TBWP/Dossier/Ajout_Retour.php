...
if($_POST){
	if($_POST['Mode']=="A"){
		$droit="";
		$requete="INSERT INTO sp_retour (Libelle,Id_Statut,EstRetour) VALUES ('".addslashes($_POST['libelle'])."','".addslashes($_POST['statut'])."',".$_POST['estRetour'].")";
		$result=mysqli_query($bdd,$requete);
		echo "<script>FermerEtRecharger();</script>";
	}
	elseif($_POST['Mode']=="M"){
		$requete="UPDATE sp_retour SET ";
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="A" || $_GET['Mode']=="M"){
		if($_GET['Id']!='0')
		{
			$result=mysqli_query($bdd,"SELECT Id, Libelle,Id_Statut,EstRetour FROM sp_retour WHERE Id=".$_GET['Id']);
...
<?php
	...
	//Mode suppression
	{
		$requete="UPDATE sp_retour SET Supprime=true WHERE Id=".$_GET['Id'];
		$result=mysqli_query($bdd,$requete);
		echo "<script>FermerEtRecharger();</script>";
