...
$req="SELECT Id, MSN,TypeACP AS Type,SectionACP AS MCA,Priorite,Reference,NumOrigine,Titre,DateCreation,TAI_RestantACP AS TAI_Restant,Elec,Systeme,Structure,Oxygene,";
...
$req.="(SELECT sp_zonedetravail.Id_CritereZone FROM sp_zonedetravail WHERE sp_zonedetravail.Id=sp_dossier.Id_ZoneDeTravail) AS CritereZone, ";
...
$req.="(SELECT new_rh_etatcivil.TelephoneProFixe FROM new_rh_etatcivil WHERE new_rh_etatcivil.Id=sp_dossier.Id_Personne) AS Tel ";
$req.="FROM sp_dossier ";
$req.="WHERE Id=".$Id."";
...
$sheet->setCellValue('V33',utf8_encode($row['Nom']." ".$row['Prenom']));
$sheet->setCellValue('Z33',$row['Tel']);

//Liste des ATA
$req="SELECT ATA,SousATA FROM sp_dossier_ata WHERE Id_Dossier=".$row['Id'];
$result=mysqli_query($bdd,$req);
...
if($Id_FI>0){$req="SELECT Id,PieceAuPoste, PowerOffPartielCIA,PosteAvionACP,TravailRealise,DeposeRepose FROM sp_ficheintervention WHERE Id=".$Id_FI." ORDER BY Id DESC";}
else{$req="SELECT Id,PieceAuPoste, PowerOffPartielCIA,PosteAvionACP,TravailRealise,DeposeRepose FROM sp_ficheintervention WHERE Id_Dossier=".$Id." ORDER BY Id DESC";}
$result=mysqli_query($bdd,$req);
$nbResulta=mysqli_num_rows($result);
if ($nbResulta>0){
	...
	if($rowFI['PowerOffPartielCIA']==1){
		$sheet->setCellValue('U17','OUI');
		$sheet->getStyle('U17')->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'ff0000'))));
	}
	else{
		$sheet->setCellValue('U17','NON');
	}
	$req2="SELECT Id FROM sp_fi_mesuresecurite WHERE MesureSecurite='Alimentation électrique' AND Type='OP' AND Id_FI=".$Id_FI." ";
	...
	if($nbResulta2>0){
		$sheet->setCellValue('U17','OUI');
		$sheet->getStyle('U17')->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'ff0000'))));
		$alimElec="Remplir le cahier zéro électrisation\n";
		if($rowFI['DeposeRepose']==0){
			if($row['Origine']=="DA"){
				$sheet->getStyle('O22')->applyFromArray(array('font' => array('color' => array('rgb' => 'ff0000'))));
			}
			else{
				$sheet->getStyle('O21')->applyFromArray(array('font' => array('color' => array('rgb' => 'ff0000'))));
			}
		}
		else{
			$sheet->getStyle('U22')->applyFromArray(array('font' => array('color' => array('rgb' => 'ff0000'))));
		}
	}
	
	if($rowFI['DeposeRepose']==0){
		if($row['Origine']=="DA"){$sheet->setCellValue('O22',utf8_encode($alimElec.$rowFI['TravailRealise']));$sheet->getStyle('O22')->getAlignment()->setWrapText(true);}
		else{$sheet->setCellValue('O21',utf8_encode($alimElec.$rowFI['TravailRealise']));$sheet->getStyle('O21')->getAlignment()->setWrapText(true);}
	}
	else{
		$sheet->setCellValue('U22',utf8_encode($alimElec.$rowFI['TravailRealise']));$sheet->getStyle('U22')->getAlignment()->setWrapText(true);
	}
...
//Liste des FI
$req="SELECT Id,NumFI,DateIntervention,Vacation,Id_StatutPROD,Id_StatutQUALITE,Commentaire FROM sp_ficheintervention WHERE Id_Dossier=".$Id." ORDER BY Id DESC";
$result=mysqli_query($bdd,$req);
$nbResulta=mysqli_num_rows($result);
$TempsNecessaire=0;
if ($nbResulta>0){
	...
	while($rowFI=mysqli_fetch_array($result)){
		...
		$req.="(SELECT CONCAT(Nom,' ',Prenom) FROM new_rh_etatcivil WHERE new_rh_etatcivil.Id=sp_fi_travaileffectue.Id_Personne) AS Compagnon ";
		$req.="FROM sp_fi_travaileffectue ";
