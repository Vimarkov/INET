...
if($_GET){
	...
	if($_GET['Mode']=="M"){
		...
		$req.="sp_olwdossier.Id_Client,sp_olwdossier.Mastic,sp_olwdossier.Peinture, ";
		$req.="sp_olwdossier.Priorite,sp_olwdossier.CaecACP AS Caec,sp_olwdossier.Metal,sp_olwdossier.Structure,sp_olwdossier.Systeme,";
		...
		//INFORMATION INTERVENTION
			//PREPA
		$req.="sp_olwficheintervention.PosteAvionACP,sp_olwficheintervention.Id_Pole,sp_olwficheintervention.TravailRealise,";
	...
	<tr><td>
		<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
			...
			<?php
				...
				$req="SELECT Id,Reference FROM sp_olwdossier WHERE Id_Prestation=379 AND Id<>".$row['Id_Dossier']." ";
			...
			<tr>
				...
				<td width='15%'>
					<select id="client" name="client">
						...
						<?php
							$req="SELECT Id,Libelle,Supprime FROM sp_client WHERE Id_Prestation=379 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
			...
			<tr>
				...
				<td width='15%'>
					<select id="section" name="section">
						<option value=""></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_olwsection WHERE Id_Prestation=379 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
			...
			<tr>
				<td width='13%' class='Libelle'>&nbsp; Zone de travail : <?php echo $etoile; ?></td><td width='15%'>
					<select id='zone' name='zone' <?php echo $disabled; ?>>
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_olwzonedetravail  WHERE Id_Prestation=379 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle, Supprime FROM sp_poste WHERE Id_Prestation=379 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledNew;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=379 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
