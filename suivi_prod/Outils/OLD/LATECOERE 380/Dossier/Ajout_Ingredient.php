...
if($_POST){
	if($_POST['Mode']=="A"){
		$droit="";
		$requete="INSERT INTO sp_olwingredient (Libelle,Id_Prestation) VALUES ('".addslashes($_POST['libelle'])."',379) ";
		$result=mysqli_query($bdd,$requete);
		echo "<script>FermerEtRecharger();</script>";
	}
	elseif($_POST['Mode']=="M"){
		$requete="UPDATE sp_olwingredient SET ";
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="A" || $_GET['Mode']=="M"){
		if($_GET['Id']!='0')
		{
			$result=mysqli_query($bdd,"SELECT Id, Libelle FROM sp_olwingredient WHERE Id=".$_GET['Id']);
			$Ligne=mysqli_fetch_array($result);
		}
?>

		<form id="formulaire" method="POST" action="Ajout_Ingredient.php" onSubmit="return VerifChamps();">
		...
		<table width="95%" height="95%" align="center" class="TableCompetences">
			<tr class="TitreColsUsers">
				<td>Libellé </td>
				<td>
					<input type="texte" name="libelle" id="libelle" size="60" value="<?php if($_GET['Mode']=="M"){echo stripslashes($Ligne['Libelle']);}?>">
				</td>
