...
<form class="test" method="POST" action="Liste_MSN.php">
	...
	<tr><td>
		<table cellpadding="0" cellspacing="0" align="left" class="GeneralInfo" style="width:100%;">
			<tr>
				<td class="EnTeteTableauCompetences" width="10%" >&nbsp;MSN</td>
				<td class="EnTeteTableauCompetences" width="10%" >&nbsp;Date moulage</td>
				<td class="EnTeteTableauCompetences" width="10%" >&nbsp;Date démoulage</td>
				<td class="EnTeteTableauCompetences" width="5%" >&nbsp;Cycle théorique</td>
				<td class="EnTeteTableauCompetences" width="5%" >&nbsp;Surveillance ATR</td>
				<td class="EnTeteTableauCompetences" width="30%" >&nbsp;Visites clients</td>
				<td class="EnTeteTableauCompetences" width="2%"></td>
				<td class="EnTeteTableauCompetences" width="2%"></td>
			</tr>
			<?php
				$req="SELECT Id,MSN,DateMoulage,HeureMoulage,DateDemoulage,HeureDemoulage,CycleTheorique,SurveillanceATR ";
				$req.="FROM sp_atrmsn WHERE Id_Prestation=262 ORDER BY MSN;";
				$result=mysqli_query($bdd,$req);
				$nbResulta=mysqli_num_rows($result);
				if ($nbResulta>0){
					$couleur="#ffffff";
					while($row=mysqli_fetch_array($result)){
						$req="SELECT sp_atrvisite.Libelle,sp_atrmsn_customer.DateVisite FROM sp_atrmsn_customer LEFT JOIN sp_atrvisite ON sp_atrmsn_customer.Id_Visite = sp_atrvisite.Id WHERE Id_MSN=".$row['Id'];
						$resultVisite=mysqli_query($bdd,$req);
						$nbVisite=mysqli_num_rows($resultVisite);
						$Visites="";
						if ($nbVisite>0){
							while($rowVisite=mysqli_fetch_array($resultVisite)){
								$Visites.=$rowVisite['Libelle']." (".AfficheDateFR($rowVisite['DateVisite']).")<br>";
							}
						}
						...
						?>
							<tr bgcolor="<?php echo $couleur;?>">
								...
								<td>&nbsp;<?php echo AfficheDateFR($row['DateMoulage'])." ".$row['HeureMoulage'];?></td>
								<td>&nbsp;<?php echo AfficheDateFR($row['DateDemoulage'])." ".$HeureDemoulage;?></td>
								<td>&nbsp;<?php echo $row['CycleTheorique'];?></td>
								<td>&nbsp;<?php if($row['SurveillanceATR']=="0"){echo "Non";}else{echo "Oui";}?></td>
