...
if($_POST){
	if($_POST['Mode']=="A"){
		$requete="INSERT INTO sp_atrcqlb (DateCreation,Id_Createur,Id_Prestation,MSN,NumCQLB,NumCV,Id_Localisation,ImputationAAA,OMAssocie,AMAssociee,Id_Type,Recurrence) ";
		$requete.="VALUES ('".$DateJour."',".$_SESSION['Id_PersonneSP'].",262,".$_POST['msn'].",'".$_POST['numCQLB']."','".$_POST['numCV']."',";
		$requete.="".$_POST['localisation'].",".$_POST['imputationAAA'].",'".$_POST['omAssocie']."','".$_POST['amAssociee']."',";
		$requete.="".$_POST['type'].",".$_POST['recurrence'].") ";
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="A" || $_GET['Mode']=="M"){
		$req="SELECT OrdreMontage, Designation FROM sp_atrot WHERE Id_Prestation=262";
		...
		if($_GET['Id']!='0'){
			$result=mysqli_query($bdd,"SELECT Id,MSN,NumCQLB,NumCV,Id_Localisation,ImputationAAA,AMAssociee,OMAssocie,Id_Type,Recurrence FROM sp_atrcqlb WHERE Id=".$_GET['Id']);
			$Ligne=mysqli_fetch_array($result);
		}
?>
		<form id="formulaire" method="POST" action="Ajout_CQLB.php" onSubmit="return VerifChamps();">
		<table width="100%">
			<tr>
				<td>
						...
						<tr>
							...
							<td>
								<select name='localisation' id='localisation'>
									...
									<?php
										$req="SELECT Id, Libelle, Supprime FROM sp_atrlocalisation WHERE Id_Prestation=262 ORDER BY Libelle ";
						...
						<tr>
							...
							<td>
								<select name='type' id='type'>
									<option value='0'></option>
									<?php
										$req="SELECT Id, Libelle, Supprime FROM sp_atrtype WHERE Id_Prestation=262 ORDER BY Libelle ";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											while($rowType=mysqli_fetch_array($result)){
