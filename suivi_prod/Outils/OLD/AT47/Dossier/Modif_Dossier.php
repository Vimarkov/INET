...
<head>
	...
	<script>
		...
		function Affiche_ECMEQUALITE()
		{
			var SourceImage = document.getElementById('Image_PlusMoinsECME').src;
			var result = SourceImage.substring(SourceImage.length -6, SourceImage.length);
			
			if(result == "us.gif"){
				document.getElementById('Image_PlusMoinsECME').src="../../../Images/Moins.gif";
				document.getElementById('id_ECMEQUALITE').style.display = '';
			}
			else{
				document.getElementById('Image_PlusMoinsECME').src="../../../Images/Plus.gif";
				document.getElementById('id_ECMEQUALITE').style.display = 'none';
			}
		}		
	</script>
	<!-- HTML5 Shim -->
...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){	
		...
		if(isset($_POST['PasDeAM'])){$req.="PasDeAM=1 ";}
		else{$req.="PasDeAM=0 ";}
					...
					if($nbECME==0){
						...
						$ReqResponsablePostePrestation="
							...
							WHERE
								...
								AND new_competences_personne_poste_prestation.Id_Prestation=262
						...
						$Objet="Nouvel ECME à mettre à jour, suivi production prestation AT47";
						$MessageMail="	<html>
										<head><title>Nouvel ECME à mettre à jour, suivi production prestation AT47</title></head>
										<body>
											Bonjour,
											<br><br>
											Attention, une référence d'ECME non identifiée dans la liste prédéfinie de votre outil de suivi production vient d'être saisie pour la prestation AT47<br>
		...
		foreach($tabPL as $valeur){
			 if($valeur<>""){
				...
				$req="INSERT INTO sp_atrot_ecmeclient (Id_OT,NumClient,DateFinEtalonnage,Id_Type) 
				VALUES (".$_POST['id'].",'".$tabECME[0]."','".TrsfDate_($tabECME[1])."',".$tabECME[2].")";
				$resultAjour=mysqli_query($bdd,$req);
			 }
		}
		
		if(substr($_SESSION['DroitSP'],1,1)=='1' || substr($_SESSION['DroitSP'],4,1)=='1'){
			//Suppression des AM / NC majeures
			$req="DELETE FROM sp_atram WHERE MSN=".$_POST['MSN']." AND OMAssocie='".$_POST['OrdreMontage']."' ";
			$resultDelete=mysqli_query($bdd,$req);
			
			//Ajout des AM / NC majeures
			$tabPL = explode("AMNC;",$_POST['AMNC']);
			foreach($tabPL as $valeur){
				 if($valeur<>""){
					$tabAMNC = explode("_",$valeur);

					$req="INSERT INTO sp_atram (ImputationAAA,NcMajeure,OMAssocie,Id_Type,Recurrence,Id_Prestation,DateCreation,Id_Createur,MSN,NumAMNC,Commentaire) 
					VALUES (".$tabAMNC[1].",".$tabAMNC[2].",'".$_POST['OrdreMontage']."',".$tabAMNC[3].",".$tabAMNC[4].",262,'".TrsfDate_(date('Y-m-d'))."',".$_SESSION['Id_PersonneSP'].",".$_POST['MSN'].",'".$tabAMNC[0]."','".addslashes($tabAMNC[5])."')";
					$resultAjour=mysqli_query($bdd,$req);
				 }
			}
		}
			 ...
			 if($valeur<>""){
				...
				if($InfosProduit<>""){
					...
					if($nbProduit==0){
						...
						$Objet="Nouvel ingrédient à mettre à jour, suivi production prestation AT47";
						$MessageMail="	<html>
										<head><title>Nouvel ingrédient à mettre à jour, suivi production prestation AT47</title></head>
										<body>
											Bonjour,
											<br><br>
											Attention, un ingrédient non identifié dans la liste prédéfinie de votre outil de suivi production vient d'être saisi pour la prestation AT47<br>
			 ...
			 if($valeur<>""){
				...
				if($Qualif<>""){
					...
					if($nbQualif==0){
						...
						$Objet="Nouveau PS identifié, suivi production prestation AT47";
						$MessageMail="	<html>
										<head><title>Nouveau PS identifié, suivi production prestation AT47</title></head>
										<body>
											Bonjour,
											<br><br>
											Attention, une référence de procédé spécial non identifiée au tableu de compétences vient d'être saisie dans le suivi production de la prestation AT47<br>
											PS : ".$Qualif."
											<br>
											Bonne journée.<br>
											L'Extranet AAA.
...
if($_GET){
	$Id=$_GET['Id'];
	if($_GET['Mode']=="M"){
		$req="SELECT Id,MSN,OrdreMontage,Designation,Id_StatutPROD,Id_StatutQUALITE,Id_CauseRetardPROD,Id_CauseRetardQUALITE,Commentaire,PasDeAM,";
		$req.="(SELECT sp_atrarticle.Ligne FROM sp_atrarticle WHERE sp_atrarticle.Article=sp_atrot.Article LIMIT 1) AS Ligne,PasDeIngredient, ";
		$req.="(SELECT sp_atrarticle.Poste45 FROM sp_atrarticle WHERE sp_atrarticle.Article=sp_atrot.Article LIMIT 1) AS Poste45,PasDePS,ValidationPSCE,ValidationPSIQ,PasDeECMEPROD,PasDeECMEQUALITE ";
		$req.="FROM sp_atrot WHERE Id=".$Id;
		$result=mysqli_query($bdd,$req);
		$row=mysqli_fetch_array($result);
	}
	elseif($_GET['Mode']=="S"){
		$req="UPDATE sp_atrot SET Supprime=1 WHERE Id=".$Id;
...
<table width="100%" cellpadding="0" cellspacing="0" align="center">
	...
	<tr><td>
		<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
			...
			<tr style="display:none;"><td><input id="OrdreMontage" name="OrdreMontage" value="<?php echo $row['OrdreMontage']; ?>"  readonly="readonly"></td></tr>
			<tr style="display:none;"><td><input id="MSN" name="MSN" value="<?php echo $row['MSN']; ?>"  readonly="readonly"></td></tr>
			<tr>
			...
			<tr><td height="4"></td></tr>
			<tr>
				<td width="13%" class="Libelle">&nbsp; Ligne : </td>
				<td width="20%">
					<?php if($row['Ligne']==null){echo "?";}else{echo $row['Ligne'];}?>
				</td>
				<td width="13%" class="Libelle">&nbsp; Poste 45 : </td>
				<td width="20%">
					<?php if($row['Poste45']==null){echo "?";}else{if($row['Poste45']==1){echo "Oui";}else{echo "Non";}}?>
				</td>
			</tr>
			<tr><td height="4"></td></tr>
			<tr>
	...
	<tr><td>
		<table width="100%" cellpadding="3" cellspacing="0" align="center" class="GeneralInfo">
			<tr>
				...
				<td colspan="2" width="33%" valign="top">
					<table width="100%" id="tableProd" cellpadding="0" cellspacing="0" align="center">
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="statutProd" name="statutProd" <?php echo $disabledCE;?>>
									...
									<option value="Pas de responsabilité AAA" <?php if($row['Id_StatutPROD']=="Pas de responsabilité AAA"){echo "selected";} ?>>Pas de responsabilité AAA</option>
						...
						<tr>
							<td width="13%" class="Libelle" bgcolor="#dbeef9">&nbsp; Information statut production : </td>
							<td width="20%" bgcolor="#dbeef9">
								<select id="causeRetardProd" name="causeRetardProd" <?php echo $disabledCE;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Supprime FROM sp_atrcauseretard WHERE Id_Prestation=262 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
						...
						<tr>
							<td bgcolor="#dbeef9" width='13%' class="Libelle" valign="top">
								<table cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
										<select id="compagnon" name="compagnon" style="width:130px;" onkeypress="if(event.keyCode == 13)AjouterTE()">
											<?php
											echo"<option name='0' value='0'></option>";
											$req="SELECT DISTINCT new_rh_etatcivil.Id, new_rh_etatcivil.Nom, new_rh_etatcivil.Prenom ";
											$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=262 AND SUBSTR(sp_acces.Droit,3,1)='1' ORDER BY Nom, Prenom;";
											$result=mysqli_query($bdd,$req);
											$nbResulta=mysqli_num_rows($result);
											if ($nbResulta>0){
												$i=0;
					...
					$modifiable="";
					if(substr($_SESSION['DroitSP'],1,1)=='0' || substr($_SESSION['DroitSP'],4,1)=='0'){$modifiable="onclick='return false'";}
				?>
					<table width="100%" cellpadding="0" cellspacing="0" align="center">
						<tr><td height="4" colspan="2" bgcolor="#dbeef9"></td></tr>
						<tr>
							<td bgcolor="#dbeef9" width='35%' class="Libelle" valign="center">
								<table width="100%" cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										...
										<tr>
											<td bgcolor='#e4e7f0'>&nbsp; 
												<select id="typeECMECLIENT" name="typeECMECLIENT" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMECLIENT()">
													<?php
													echo"<option name='0' value='0'></option>";
													$req="SELECT Id, Libelle FROM sp_atrtypeecme WHERE Id_Prestation=262 AND Supprime=false ORDER BY Libelle ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													if ($nbResulta>0){
														$i=0;
							...
							<td bgcolor="#dbeef9" width='65%' valign='top'>
								<table id="tab_ECMECLIENT" width='100%' cellpadding='0' cellspacing='0'>
									...
									<?php
										$listeECMECLIENT="";
										$req="SELECT NumClient,DateFinEtalonnage,Id_Type,
										...
										if ($nbResulta>0){
											while($rowECME=mysqli_fetch_array($result)){
												$btn="";
												if(substr($_SESSION['DroitSP'],1,1)=='1' || substr($_SESSION['DroitSP'],4,1)=='1'){
													$btn="<a style=\"text-decoration:none;\" href=\"javascript:SupprimerECMECLIENT('".$rowECME['NumClient']."_".AfficheDateFR($rowECME['DateFinEtalonnage'])."_".$rowECME['Id_Type']."')\">&nbsp;<img src=\"../../../Images/Suppression2.gif\" border=\"0\" alt=\"Suppr\" title=\"Suppr\">&nbsp;&nbsp;</a>";
												}
												echo "<tr id='ECMECLIENT".$rowECME['NumClient']."_".AfficheDateFR($rowECME['DateFinEtalonnage'])."_".$rowECME['Id_Type']."'>";
												$listeECMECLIENT.=$rowECME['NumClient']."_".AfficheDateFR($rowECME['DateFinEtalonnage'])."_".$rowECME['Id_Type'].";";
											
												echo "<td>".$rowECME['Type']."</td><td>".$rowECME['NumClient']."</td><td>".AfficheDateFR($rowECME['DateFinEtalonnage'])."</td><td>".$btn."</td></tr>";
											}
										}
									?>
								</table>
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="typeECMEPROD" name="typeECMEPROD" style="width:100px;" onchange="Recharge_RefECMEPROD()" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_atrtypeecme WHERE Supprime=false AND Id_Prestation=262 ";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
										...
										<td bgcolor='#e4e7f0'>
											<div id="listeECMEPROD">
												&nbsp; <select id="referencePROD" name="referencePROD" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
													...
													$req="SELECT Id, Libelle,Id_Type FROM sp_atrecme WHERE Supprime=false AND Id_Prestation=262 ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													if ($nbResulta>0){
														$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefProduit" name="RefProduit" onkeypress="if(event.keyCode == 13)AjouterProduit()" style="width:130px;">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_atrtypeproduit WHERE Supprime=false AND Id_Prestation=262 ORDER BY Libelle;";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>
											...
											<select id="RefAIPI" name="RefAIPI" onkeypress="if(event.keyCode == 13)AjouterAIPIS()" style="width:130px;">
												...
												$req.="WHERE new_competences_prestation_qualification.Id_Prestation=262 ";
				...
				<td colspan="2" width="33%" valign="top">
					<table width="100%" cellpadding="0" cellspacing="0" align="center">
							...
							<td width="20%" bgcolor="#c7e048">
								<div id="statutsQualite">
									<select id="statutQualite" name="statutQualite" <?php echo $disabledIQ;?>>
										...
										<option value="TVS AM" <?php if($row['Id_StatutQUALITE']=="TVS AM"){echo "selected";} ?>>TVS AM</option>
										<option value="TERC" <?php if($row['Id_StatutQUALITE']=="TERC"){echo "selected";} ?>>TERC</option>
										<option value="YMCA" <?php if($row['Id_StatutQUALITE']=="YMCA"){echo "selected";} ?>>YMCA</option>
									</select>
								</div>
							</td>
						</tr>
						<tr><td height="4" colspan="2" bgcolor="#c7e048"></td></tr>
						<tr>
							<td width="10%" class="Libelle" bgcolor="#c7e048">&nbsp; Information statut qualité : </td>
							<td width="20%" bgcolor="#c7e048">
								...
								<table cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										...
										<select id="controleur" name="controleur" style="width:130px;" onkeypress="if(event.keyCode == 13)AjouterControleur()">
											...
											echo"<option name='0' value='0'></option>";
											$req="SELECT DISTINCT new_rh_etatcivil.Id, new_rh_etatcivil.Nom, new_rh_etatcivil.Prenom ";
											$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=262 AND SUBSTR(sp_acces.Droit,5,1)='1' ORDER BY Nom, Prenom;";
											$result=mysqli_query($bdd,$req);
											$nbResulta=mysqli_num_rows($result);
											if ($nbResulta>0){
												$i=0;
					...
					<table width="100%" cellpadding="0" cellspacing="0" align="center">
						...
						<tr><td height="4" colspan="2" bgcolor="#c7e048"></td></tr>
						<tr>
							<td bgcolor="#c7e048" width='35%' class="Libelle" valign="center">
								<table width="100%" cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									<tr>
										<td bgcolor="#e4e7f0" colspan='2'><i>&nbsp; Ajouter les AM/NC majeures :</i></td>
									</tr>
									<tr><td bgcolor="#e4e7f0" height="2" colspan="2"></td></tr>
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp;N° AM/NC : </td>
									</tr>
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<input style="text-align:left;" id="numAMNC" onkeypress="if(event.keyCode == 13)AjouterAMNC()" name="numAMNC" size="20" type="text" value="">
										</td>
									</tr>
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp;Imputation AAA : </td>
									</tr>
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select name='imputationAAA' id='imputationAAA' onkeypress="if(event.keyCode == 13)AjouterAMNC()">
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp;NC majeure : </td>
									</tr>
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select name='ncMajeure' id='ncMajeure' onkeypress="if(event.keyCode == 13)AjouterAMNC()">
													<option value='1'>Oui</option>
													<option value='0' selected>Non</option>
											</select>
										</td>
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select name='typeAM' id='typeAM' onkeypress="if(event.keyCode == 13)AjouterAMNC()">
												<option value='0' ></option>
												<?php
													$req="SELECT Id, Libelle FROM sp_atrtype WHERE Supprime=0 AND Id_Prestation=262 ORDER BY Libelle ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													$i=0;
													if ($nbResulta>0){
														while($rowType=mysqli_fetch_array($result)){
															echo "<option value='".$rowType['Id']."'>".$rowType['Libelle']."</option>";
															echo "<script>Liste_TypeAM[".$i."] = new Array('".$rowType['Id']."','".str_replace("'"," ",str_replace('"',' ',stripslashes($rowType['Libelle'])))."');</script>\n";
															$i+=1;
														}
													}
												?>
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp;Récurrence : </td>
									</tr>
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select name='recurrenceAM' id='recurrenceAM' onkeypress="if(event.keyCode == 13)AjouterAMNC()">
													<option value='1'>Oui</option>
													<option value='0' selected>Non</option>
											</select>
										</td>
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp;Commentaire : </td>
									</tr>
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<textarea id="commentaireAM" name="commentaireAM" rows="2" cols="30" style="resize:none;"></textarea>
										</td>
									</tr>
									<tr>
										<td bgcolor='#e4e7f0' align='center' colspan='2' style='height:25px;' valign='center'>
											<?php
												if(substr($_SESSION['DroitSP'],1,1)=='1' || substr($_SESSION['DroitSP'],4,1)=='1'){
											?>
											<a style='text-decoration:none;' class='Bouton' href='javascript:AjouterAMNC()'>&nbsp;Ajouter&nbsp;</a>
											<?php
												}
											?>
										</td>
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<input style="text-align:left;" id="PasDeAM" <?php echo $modifiable; ?> name="PasDeAM" type="checkbox" value="PasDeAM" <?php if($row['PasDeAM']==1){echo "checked";} ?>> Pas d'AM/NC
										</td>
									</tr>
								</table>
							</td>
							<td bgcolor="#c7e048" width='65%' valign='top'>
								<table id="tab_AM" width='100%' cellpadding='0' cellspacing='0'>
									<tr><td class="Libelle">N° AM/NC</td><td class="Libelle">Imputation AAA</td><td class="Libelle">NC Majeure</td><td class="Libelle">Type</td><td class="Libelle">Récurrence</td><td class="Libelle">Commentaire</td></tr>
									<?php
										$listeAM="";
										$req="SELECT IF(ImputationAAA=0,'Non','Oui') AS LibelleImputationAAA,ImputationAAA,
										IF(NcMajeure=0,'Non','Oui') AS LibelleNcMajeure,NcMajeure,
										IF(Recurrence=0,'Non','Oui') LibelleRecurrence,Recurrence,
										NumAMNC,Id_Type,Commentaire,
										(SELECT Libelle FROM sp_atrtype WHERE sp_atrtype.Id=sp_atram.Id_Type) AS Type
										FROM sp_atram
										WHERE Id_Prestation=262
										AND MSN=".$row['MSN']."
										AND OMAssocie=".$row['OrdreMontage']." ORDER BY NumAMNC;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											while($rowAM=mysqli_fetch_array($result)){
												$btn="";
												if(substr($_SESSION['DroitSP'],1,1)=='1' || substr($_SESSION['DroitSP'],4,1)=='1'){
													$btn="<a style=\"text-decoration:none;\" href=\"javascript:SupprimerAM('".$rowAM['NumAMNC']."_".$rowAM['ImputationAAA']."_".$rowAM['NcMajeure']."_".$rowAM['Id_Type']."_".$rowAM['Recurrence']."_".str_replace('"',' ',stripslashes($rowAM['Commentaire']))."')\">&nbsp;<img src=\"../../../Images/Suppression2.gif\" border=\"0\" alt=\"Suppr\" title=\"Suppr\">&nbsp;&nbsp;</a>";
												}
												echo "<tr id='".$rowAM['NumAMNC']."_".$rowAM['ImputationAAA']."_".$rowAM['NcMajeure']."_".$rowAM['Id_Type']."_".$rowAM['Recurrence']."_".str_replace('"',' ',stripslashes($rowAM['Commentaire']))."AMNC'>";
												$listeAM.=$rowAM['NumAMNC']."_".$rowAM['ImputationAAA']."_".$rowAM['NcMajeure']."_".$rowAM['Id_Type']."_".$rowAM['Recurrence']."_".str_replace('"',' ',stripslashes($rowAM['Commentaire']))."AMNC;";

												echo "<td>".$rowAM['NumAMNC']."</td><td>".$rowAM['LibelleImputationAAA']."</td><td>".$rowAM['LibelleNcMajeure']."</td><td>".$rowAM['Type']."</td><td>".$rowAM['LibelleRecurrence']."</td><td>".$rowAM['Commentaire']."</td><td>".$btn."</td></tr>";
						...
						<tr><td height="8" colspan="2" bgcolor="#c7e048"></td></tr>
					</table>
					<table width="100%" cellpadding="0" cellspacing="0" align="center">
						<tr><td height="10" colspan="2"></td></tr>
						<tr>
							<td height="4" bgcolor="#c7e048" style="font-size:13px;font-weight:bold;"><i>&nbsp; ECME QUALITE</i></td>
							<td height="4" bgcolor="#c7e048" style="font-size:13px;font-weight:bold;" align="right" valign="center">
								<img id="Image_PlusMoinsECME" width="13px" src="../../../Images/Plus.gif" style="cursor:pointer;" onclick="javascript:Affiche_ECMEQUALITE();">&nbsp;&nbsp;
							</td>
						</tr>
						<tr><td height="4" colspan="2" bgcolor="#c7e048"></td></tr>
						<tr id="id_ECMEQUALITE" style="display:none;">
							<td bgcolor="#c7e048" width='35%' class="Libelle" valign="center">
								<table width="100%" cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									<tr>
					...
					<table width="100%" height="100px" cellpadding="0" valign="center" cellspacing="0" align="center">
						<tr>
							<?php
							...
							<td colspan="6" align="center">
								...
								<input class="Bouton" type="button" style="font-size:16px;cursor:pointer;" id="btnEnregistrer" name="btnEnregistrer" value="Enregistrer" onClick="AfficherSubmit()">
							</td>
							<?php
							}
							?>
			...
			<tr style="display:none;"><td><input id="AMNC" name="AMNC" value="<?php echo $listeAM;?>" readonly="readonly"></td></tr>
		</table>
	</td></tr>
	<tr><td height="4"></td></tr>
</table>
