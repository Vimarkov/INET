...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){	
		...
		if(isset($_POST['Ajusteur'])){$req.="Ajusteur=1, ";}else{$req.="Ajusteur=0, ";}
		if(isset($_POST['Etancheur'])){$req.="Etancheur=1, ";}else{$req.="Etancheur=0, ";}
		if(isset($_POST['Hydraulique'])){$req.="Hydraulique=1, ";}else{$req.="Hydraulique=0, ";}
		if(isset($_POST['CommandeVol'])){$req.="CommandeVol=1 ";}else{$req.="CommandeVol=0 ";}
		$req.="WHERE Id=".$_POST['idDossier'];
		$resultUpdate=mysqli_query($bdd,$req);
		
		//Suppression des anciens ATA
		$req="DELETE FROM sp_olwdossier_pointlie WHERE Id_Dossier=".$_POST['idDossier'];
		$resultDelete=mysqli_query($bdd,$req);

		//Ajout points liés
		$tabPL = explode(";",$_POST['pointsLies']);
		foreach($tabPL as $valeur){
			 if($valeur<>""){
				$tab2 = explode("_",$valeur);
				$req="INSERT INTO sp_olwdossier_pointlie (Id_Dossier,Type,Num) VALUES (".$_POST['idDossier'].",'".$tab2[0]."','".$tab2[1]."')";
		...
		$req.="Id_Dossier,Id_Createur,DateCreation,PosteAvionACP,Id_Pole,TravailGN,NumFI,TravailRealise,Commentaire";
		...
		$req.=$_POST['idDossier'].",".$_SESSION['Id_PersonneSP'].",'".$DateJour."','".addslashes($_POST['poste'])."',".$_POST['pole'].",";
		$req.="".$_POST['travailEnGN'].",'".addslashes($_POST['numFI'])."','".addslashes($_POST['travailRealise'])."','".addslashes($_POST['commentaire'])."'";
...
if($_GET){
	...
	$req.="sp_olwdossier.Id_Client,sp_olwdossier.Ajusteur,sp_olwdossier.Etancheur,sp_olwdossier.Hydraulique, ";
	$req.="sp_olwdossier.Priorite,sp_olwdossier.CaecACP AS Caec,sp_olwdossier.CommandeVol,";
	$req.="sp_olwdossier.SectionACP AS MCA,sp_olwdossier.Titre,sp_olwdossier.DateCreation,";
	...
	<tr><td>
		<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
			...
			<?php
				...
				$req="SELECT Id,Reference FROM sp_olwdossier WHERE Id_Prestation=316 AND Id<>".$row['Id_Dossier']." ";
			...
			<tr>
				...
				<td width='15%'>
					<select id="client" name="client">
						...
						<?php
							$req="SELECT Id,Libelle,Supprime FROM sp_client WHERE Id_Prestation=316 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
			...
			<tr>
				...
				<td width='15%'>
					<select id="section" name="section">
						<option value=""></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_olwsection WHERE Id_Prestation=316 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
			...
			<tr>
				...
				<td>
					<table width='100%' cellpadding='0' cellspacing='0' align='left'>
						<tr>
							<td><input type="checkbox" id="Ajusteur" name="Ajusteur" value="Ajusteur" <?php if($row['Ajusteur']==1){ echo "checked";}?> <?php echo $disabled;?> >Ajusteur &nbsp;&nbsp;</td>
							<td><input type="checkbox" id="Etancheur" name="Etancheur" value="Etancheur" <?php if($row['Etancheur']==1){ echo "checked";}?> <?php echo $disabled;?>>Etancheur &nbsp;&nbsp;</td>
						</tr>
						<tr>
							...
							<td><input type="checkbox" id="CommandeVol" name="CommandeVol" value="Commande de vol" <?php if($row['CommandeVol']==1){ echo "checked";}?> <?php echo $disabled;?>>Commande de vol &nbsp;&nbsp;</td>
						</tr>
					</table>
				</td>
				<td width='15%' class='Libelle'>
					<table cellpadding='0' cellspacing='0' style='-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;'>
						<tr>
							<td colspan='2'>&nbsp; Ajouter les points liés : </td>
						</tr>
						<tr>
							<td bgcolor='#e4e7f0'>&nbsp; Type : </td>
							<td bgcolor='#e4e7f0'>
								<select name='typePL' id='typePL' onkeypress='if(event.keyCode == 13)Ajouter()'>
									<?php
										echo"<option name='' value=''></option>";
										echo"<option name='DA' value='DA'>DA</option>";
										echo"<option name='Déro' value='Déro'>Déro</option>";
										echo"<option name='NC' value='NC'>NC</option>";
									?>
								</select>
							</td>
						</tr>
						<tr>
							<td bgcolor='#e4e7f0'>&nbsp; N° associé : </td>
							<td bgcolor='#e4e7f0'>
								&nbsp;<input style='text-align:center;' id='numAssocie' onkeypress='if(event.keyCode == 13)Ajouter()' name='numLot' size='8' type='text' value=''>
							</td>
						</tr>
						<tr>
				...
				<td width='20%' valign='top'>
					<table id='tab_PL' width='100%' cellpadding='0' cellspacing='0'>
						<tr><td class='Libelle'>Type</td><td class='Libelle'>N° associé</td><td></td></tr>
						<?php
							$listePL="";
							$req="SELECT Type, Num FROM sp_olwdossier_pointlie WHERE Id_Dossier=".$row['Id_Dossier']." ORDER BY Type, Num;";
							$result=mysqli_query($bdd,$req);
							$nbResultaPL=mysqli_num_rows($result);
							if ($nbResultaPL>0){
								while($rowPL=mysqli_fetch_array($result)){
									$btn="";
									if(substr($_SESSION['DroitSP'],0,1)=='1' || substr($_SESSION['DroitSP'],1,1)=='1' || substr($_SESSION['DroitSP'],4,1)=='1'){
										$btn="<a style=\"text-decoration:none;\" href=\"javascript:Supprimer('".$rowPL['Type']."_".$rowPL['Num'].";')\">&nbsp;<img src=\"../../../Images/Suppression2.gif\" border=\"0\" alt=\"Suppr\" title=\"Suppr\">&nbsp;&nbsp;</a>";
									}
									echo "<tr id='".$rowPL['Type']."_".$rowPL['Num'].";'>";
									echo "<td>".$rowPL['Type']."</td>";
									echo "<td>".$rowPL['Num']."</td>";
									echo "<td>".$btn."</td></tr>";
									$listePL.=$rowPL['Type']."_".$rowPL['Num'].";";
								}
							}
						?>
					</table>
			...
			<?php
			echo "<tr style='display:none;'><td><input id='pointsLies' name='pointsLies' value='".$listePL."'  readonly='readonly'></td></tr>";
			?>
					...
					<table width="100%" cellpadding="0" cellspacing="0" align="center">
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle FROM sp_poste WHERE Id_Prestation=316 AND Supprime=false ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%">
								<select id="pole" name="pole" <?php echo $disabled;?>>
									<option name="0" value="0"></option>
									<?php
										$req="SELECT Id, Libelle FROM sp_pole WHERE Id_Prestation=316 ORDER BY Libelle;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
						...
						<tr>
							<td width="13%" class="Libelle">
								&nbsp; Travail en GN <?php echo $etoile;?>
							<td width="20%" class="Libelle">
								Oui <input type="radio" id='travailEnGN' name='travailEnGN' value="1">
								&nbsp; Non<input type="radio" id='travailEnGN' name='travailEnGN' value="0" checked>
							</td>
						</tr>
						<tr><td height="4" colspan="2"></td></tr>
						<tr>
								...
								<table cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										...
										<select id="compagnon" id="compagnon" name="compagnon" style="width:130px;" onkeypress="if(event.keyCode == 13)AjouterTE()">
											...
											$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=316 AND SUBSTR(sp_acces.Droit,3,1)='1' ORDER BY Nom, Prenom;";
											$result=mysqli_query($bdd,$req);
											$nbResulta=mysqli_num_rows($result);
											if ($nbResulta>0){
												$i=0;
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledCE;?>>
									<option name="" value=""></option>
									<?php
									$req="SELECT Id FROM sp_olwstatut WHERE TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledCE;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=316 AND Supprime=0 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
						...
						<tr>
							...
							<td bgcolor="#c7e048" width="20%" colspan="3">
								<select name="IQ">
									...
									$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=316 AND SUBSTR(sp_acces.Droit,5,1)=1 ORDER BY NomPrenom;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
							...
							<td width="20%" bgcolor="#c7e048">
								<div id="statutsQualite">
									<select id="statutQualite" name="statutQualite" onchange="Recharge_StatutQualite();" <?php echo $disabledIQ;?>>
										<option name="" value=""></option>
										<?php
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=316 AND TypeStatut='Q' ORDER BY Id;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefIngredient" name="RefIngredient" onkeypress="if(event.keyCode == 13)AjouterIngredient()" style="width:130px;">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwingredient WHERE Id_Prestation=316 AND Supprime=false ORDER BY Libelle;";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefAIPI" name="RefAIPI" onkeypress="if(event.keyCode == 13)AjouterAIPIS()" style="width:130px;">
												...
												$req.="WHERE new_competences_prestation_qualification.Id_Prestation=316 ";
