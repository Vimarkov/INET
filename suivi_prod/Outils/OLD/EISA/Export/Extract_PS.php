...
$req="SELECT DISTINCT sp_atrot_aipi.Qualification,sp_atrot.MSN,sp_atrot.OrdreMontage
	FROM sp_atrot_aipi 
	LEFT JOIN sp_atrot 
	ON sp_atrot_aipi.Id_OT=sp_atrot.Id 
	WHERE sp_atrot.Id_Prestation=463 
	AND sp_atrot.Supprime=0
	AND sp_atrot_aipi.Qualification<>''";
...
if ($nbResulta>0){	
	while($row=mysqli_fetch_array($result)){
		...
		$sheet->setCellValue('B'.$ligne,utf8_encode($row['OrdreMontage']));
