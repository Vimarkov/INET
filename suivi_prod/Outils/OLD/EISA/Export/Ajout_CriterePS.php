		...
		<tr>
			...
			<td width=70%>
				<select name="compagnon">
					...
					<?php
					$req="SELECT DISTINCT sp_atrot_compagnon.Id_Personne AS Id, 
						(SELECT CONCAT(Nom,' ',Prenom) FROM new_rh_etatcivil 
						WHERE new_rh_etatcivil.Id=sp_atrot_compagnon.Id_Personne) AS Personne 
						FROM sp_atrot_compagnon 
						LEFT JOIN sp_atrot
						ON sp_atrot_compagnon.Id_OT=sp_atrot.Id
						WHERE sp_atrot.Supprime=0
						AND sp_atrot.Id_Prestation=463
						ORDER BY Personne;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
		...
		<tr>
			...
			<td width=70%>
				<select name="qualiticien">
					<option name="" value=""></option>
					<?php
					$req="SELECT DISTINCT sp_atrot_controleur.Id_Personne AS Id, 
						(SELECT CONCAT(Nom,' ',Prenom) FROM new_rh_etatcivil 
						WHERE new_rh_etatcivil.Id=sp_atrot_controleur.Id_Personne) AS Personne 
						FROM sp_atrot_controleur 
						LEFT JOIN sp_atrot
						ON sp_atrot_controleur.Id_OT=sp_atrot.Id
						WHERE sp_atrot.Supprime=0
						AND sp_atrot.Id_Prestation=463
						ORDER BY Personne;";
					$result=mysqli_query($bdd,$req);
