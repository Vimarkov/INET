		...
		elseif($_GET['critere']=="type"){
			...
			foreach($tab as $Id){
				if($Id<>""){
					...
					$req="SELECT Libelle FROM sp_atrtypeecme WHERE Id=".$Id;
		...
		<tr>
			...
			<td width=80%>
				<select name="type">
					...
					<?php
					$req="SELECT DISTINCT sp_atrot_ecme.Id_TypeECME AS Id, (SELECT Libelle FROM sp_atrtypeecme ";
					$req.="WHERE sp_atrtypeecme.Id=sp_atrot_ecme.Id_TypeECME) AS Libelle 
						FROM sp_atrot_ecme 
						LEFT JOIN sp_atrot 
						ON sp_atrot_ecme.Id_OT=sp_atrot.Id 
						WHERE sp_atrot.Id_Prestation=463 ORDER BY Libelle;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
