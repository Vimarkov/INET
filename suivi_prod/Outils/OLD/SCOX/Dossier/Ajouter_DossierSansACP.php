...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		$req.="Fuel,Elec,Hydraulique,Metal,Meca,Structure,Systeme,Oxygene,Id_ZoneDeTravail,Origine,NumOrigine,Id_Statut,Avancement,Id_Retour,CommentaireZICIA)";
		$req.=" VALUES (842,".$_SESSION['Id_PersonneSP'].",".$_POST['msn'].",".$_POST['client'].",'".addslashes($_POST['numDossier'])."',";
...
<table width="100%" cellpadding="0" cellspacing="0" align="center">
	...
	<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
		...
		<tr>
			...
			<td width='15%'>
				<select id="client" name="client">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_client WHERE Id_Prestation=842 AND Supprime=false ORDER BY Libelle";
		...
		<?php
			...
			$req="SELECT Id,Reference FROM sp_olwdossier WHERE Id_Prestation=842 ";
		...
		<tr>
			...
			<td width='15%'>
				<select id="section" name="section">
					<option value=""></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_olwsection WHERE Id_Prestation=842 AND Supprime=false ORDER BY Libelle;";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
		...
		<tr>
			...
			<td width='15%'>
				<select id="urgence" name="urgence">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_olwurgence WHERE Id_Prestation=842 AND Supprime=false ORDER BY Libelle;";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
							while($rowSection=mysqli_fetch_array($result)){
		...
		echo "<tr>";
		echo "<td width='13%' class='Libelle' valign='top'>&nbsp; Priorité : </td>";
		...
		echo "<tr>";
			...
			echo"<select id='zone' name='zone' ".$disabled.">";
				...
				$req="SELECT Id,Libelle FROM sp_olwzonedetravail  WHERE Id_Prestation=842 AND Supprime=false ORDER BY Libelle;";
				$result=mysqli_query($bdd,$req);
				$nbResulta=mysqli_num_rows($result);
				if ($nbResulta>0){
		...
		?>
			...
			<tr>
				...
				<td><input type="checkbox" id="Meca" name="Meca" value="Meca" <?php echo $disabled;?>>Méca &nbsp;&nbsp;</td>
			</tr>
			<tr>
		...
		echo "<td width='13%' class='Libelle'>";
				...
				echo "<tr>";
				echo "<td colspan='2'>&nbsp; Liste des ATA/Sous-ATA : &nbsp;";
				echo "</td>";
				echo "</tr>";
				echo "<tr>";
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle FROM sp_poste WHERE Id_Prestation=842 AND Supprime=false ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledCE;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=842 AND Supprime=0 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
