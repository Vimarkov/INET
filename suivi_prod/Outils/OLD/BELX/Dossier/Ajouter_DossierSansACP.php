...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		$req.="TypeACP,SectionACP,CaecACP,Priorite,Titre,DateCreation,TAI_RestantACP,Id_Urgence,PNE,";
		$req.="Fuel,Elec,Hydraulique,Metal,Structure,Systeme,Oxygene,Id_ZoneDeTravail,Origine,NumOrigine,Id_Statut,Avancement,Id_Retour,CommentaireZICIA)";
		$req.=" VALUES (815,".$_SESSION['Id_PersonneSP'].",".$_POST['msn'].",".$_POST['client'].",'".addslashes($_POST['numDossier'])."',";
		$req.="'".$_POST['type']."','".$_POST['section']."','".$_POST['caec']."',".$_POST['priorite'].",'".addslashes($_POST['titre'])."',";
		$req.="'".$DateJour."',".$_POST['tai'].",".$_POST['urgence'].",".$_POST['pne'].",";
...
<table width="100%" cellpadding="0" cellspacing="0" align="center">
	...
	<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
		...
		<tr>
			...
			<td width='15%'>
				<select id="client" name="client">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_client WHERE Id_Prestation=815 AND Supprime=false ORDER BY Libelle";
		...
		<?php
			...
			$req="SELECT Id,Reference FROM sp_olwdossier WHERE Id_Prestation=815 ";
		...
		<tr>
			...
			<td width='15%'>
				<select id="section" name="section">
					<option value=""></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_olwsection WHERE Id_Prestation=815 AND Supprime=false ORDER BY Libelle;";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
		...
		<tr>
			...
			<td width='15%'>
				<select id="urgence" name="urgence">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_olwurgence WHERE Id_Prestation=815 AND Supprime=false ORDER BY Libelle;";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
							while($rowSection=mysqli_fetch_array($result)){
		...
		echo "<tr><td height='4'></td></tr>";
		echo "<tr>";
		echo "<td width='13%' class='Libelle' valign='top'>&nbsp; Type : ".$etoile."</td>";
		echo "<td width='15%' valign='top'>";
		?>
			<select id="type" name="type" <?php echo $disabled;?>>
				<option value=""></option>
				<option value="AM">AM</option>
				<option value="E QLB">E QLB</option>
		...
		echo "<tr>";
			...
			echo"<select id='zone' name='zone' ".$disabled.">";
				...
				$req="SELECT Id,Libelle FROM sp_olwzonedetravail  WHERE Id_Prestation=815 AND Supprime=false ORDER BY Libelle;";
				$result=mysqli_query($bdd,$req);
				$nbResulta=mysqli_num_rows($result);
				if ($nbResulta>0){
		...
		?>
			...
			<tr>
				...
				<td><input type="checkbox" id="Metal" name="Metal" value="Metal" <?php echo $disabled;?>>Métal &nbsp;&nbsp;</td>
			</tr>
			<tr>
				<td><input type="checkbox" id="Structure" name="Structure" value="Structure" <?php echo $disabled;?>>Structure &nbsp;&nbsp;</td>
				<td><input type="checkbox" id="Systeme" name="Systeme" value="Systeme" <?php echo $disabled;?>>Système &nbsp;&nbsp;</td>
			</tr>
			<tr>
				<td><input type="checkbox" id="Oxygene" name="Oxygene" value="Oxygene" <?php echo $disabled;?>>Oxygène &nbsp;&nbsp;</td>
			</tr>
		<?php
		echo "</table>";
		echo "</td>";
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle FROM sp_poste WHERE Id_Prestation=815 AND Supprime=false ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%">
								<select id="pole" name="pole" <?php echo $disabled;?>>
									<option name="0" value="0"></option>
									<?php
										$req="SELECT Id, Libelle FROM sp_pole WHERE Id_Prestation=815 ORDER BY Libelle;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
								...
								<table cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										...
										<select id="compagnon" id="compagnon" name="compagnon" style="width:130px;" onkeypress="if(event.keyCode == 13)AjouterTE()">
											...
											$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=815 AND SUBSTR(sp_acces.Droit,3,1)='1' ORDER BY Nom, Prenom;";
											$result=mysqli_query($bdd,$req);
											$nbResulta=mysqli_num_rows($result);
											if ($nbResulta>0){
												$i=0;
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledCE;?>>
									<option name="" value=""></option>
									<?php
									$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=815 AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledCE;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=815 AND Supprime=0 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
						...
						<tr>
							...
							<td bgcolor="#c7e048" width="20%" colspan="3">
								<select name="IQ">
									...
									$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=815 AND SUBSTR(sp_acces.Droit,5,1)=1 ORDER BY NomPrenom;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
							...
							<td width="20%" bgcolor="#c7e048">
								<div id="statutsQualite">
									<select id="statutQualite" name="statutQualite" onchange="Recharge_StatutQualite();" <?php echo $disabledIQ;?>>
										<option name="" value=""></option>
										<?php
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=815 AND TypeStatut='Q' ORDER BY Id;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefIngredient" name="RefIngredient" onkeypress="if(event.keyCode == 13)AjouterIngredient()" style="width:130px;">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwingredient WHERE Id_Prestation=815 AND Supprime=false ORDER BY Libelle;";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefAIPI" name="RefAIPI" onkeypress="if(event.keyCode == 13)AjouterAIPIS()" style="width:130px;">
												...
												$req.="WHERE new_competences_prestation_qualification.Id_Prestation=815 ";
