...
$req.="WHERE sp_olwdossier.Id_Prestation=815 AND ";
if($_SESSION['MSN2']<>""){
...
if($nbResulta>0){
	while($row=mysqli_fetch_array($result)){
		...
		$sheet->setCellValue('C1',utf8_encode("Zone"));
		$sheet->setCellValue('D1',utf8_encode("Priorité"));
		$sheet->setCellValue('E1',utf8_encode("TAI restant"));
		$sheet->setCellValue('F1',utf8_encode("Titre"));
		$sheet->setCellValue('G1',utf8_encode("Date d'intervention"));
		$sheet->setCellValue('H1',utf8_encode("Vacation"));
		$sheet->setCellValue('I1',utf8_encode("N° FI"));
		...
		$sheet->setCellValue('P1',utf8_encode("Création IC"));
		$sheet->setCellValue('Q1',utf8_encode("Section"));
		...
		while($row2=mysqli_fetch_array($result2)){
			if($row2['MSN']==$row['MSN']){
				...
				$sheet->setCellValue('C'.$ligne,utf8_encode($row2['Zone']));
				$sheet->setCellValue('D'.$ligne,utf8_encode($Priorite));
				$sheet->setCellValue('E'.$ligne,utf8_encode($row2['TAI_RestantACP']));
				$sheet->setCellValue('F'.$ligne,utf8_encode(stripslashes($row2['Titre'])));
				$sheet->setCellValue('G'.$ligne,utf8_encode(AfficheDateFR($row2['DateIntervention'])));
				$sheet->setCellValue('H'.$ligne,utf8_encode($Vacation));
				$sheet->setCellValue('I'.$ligne,utf8_encode($row2['NumFI']));
				$sheet->setCellValue('J'.$ligne,utf8_encode(stripslashes($row2['TravailRealise'])));
				$sheet->setCellValue('K'.$ligne,utf8_encode($row2['PosteAvionACP']));
				$sheet->setCellValue('L'.$ligne,utf8_encode($row2['Id_StatutPROD']));
				$sheet->setCellValue('M'.$ligne,utf8_encode($row2['RetourPROD']));
				$sheet->setCellValue('N'.$ligne,utf8_encode($row2['Id_StatutQUALITE']));
				$sheet->setCellValue('O'.$ligne,utf8_encode($row2['RetourQUALITE']));
				$sheet->setCellValue('P'.$ligne,utf8_encode($row2['CreateurIC']));
				$sheet->setCellValue('Q'.$ligne,utf8_encode($row2['SectionACP']));
