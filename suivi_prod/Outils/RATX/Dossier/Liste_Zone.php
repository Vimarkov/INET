...
<head>
	...
	<script language="javascript">
		function OuvreFenetreAjout(){
			var w=window.open("Ajout_Zone.php?Mode=A&Id=0","PageUtilisateur","status=no,menubar=no,scrollbars=yes,width=600,height=120");
			w.focus();
			}
		function OuvreFenetreModif(Id){
			var w=window.open("Ajout_Zone.php?Mode=M&Id="+Id,"PageUtilisateur","status=no,menubar=no,scrollbars=yes,width=600,height=120");
			w.focus();
			}
		function OuvreFenetreSuppr(Id){
			if(window.confirm('Etes-vous sûr de vouloir supprimer ?')){
			var w=window.open("Ajout_Zone.php?Mode=S&Id="+Id,"PageUtilisateur","status=no,menubar=no,scrollbars=yes,width=60,height=40");
			w.focus();
			}
		}
	</script>
...
<form class="test" method="POST" action="Liste_Zone.php">
	<tr>
		<td>
			<table width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td width="4"></td>
					<td class="TitrePage">Liste des zones</td>
				</tr>
			</table>
		</td>
	</tr>
	...
	<tr>
		<td align="left">
			<table align="left" style="width:30%;">
				<tr>
				<td align="center">
				<a style='text-decoration:none;' class='Bouton' href='javascript:OuvreFenetreAjout()'>&nbsp;Ajouter une zone&nbsp;</a>
				</td>
				</tr>
			</table>
		</td>
	...
	<tr><td>
		<table cellpadding="0" cellspacing="0" align="left" class="GeneralInfo" style="width:30%;">
			<tr>
				<td class="EnTeteTableauCompetences" width="10%" >&nbsp;Libellé</td>
				<td class="EnTeteTableauCompetences" width="16%" >Description</td>
				<td class="EnTeteTableauCompetences" width="16%" >Critère zone</td>
				<td class="EnTeteTableauCompetences" width="2%"></td>
				<td class="EnTeteTableauCompetences" width="2%"></td>
			</tr>
			<?php
				$req="SELECT Id,Libelle,Intitule,(SELECT sp_criterezone.Libelle FROM sp_criterezone WHERE sp_criterezone.Id= sp_olwzonedetravail.Id_CritereZone) AS Critere FROM sp_olwzonedetravail WHERE Id_Prestation=834 AND Supprime=false ORDER BY Libelle;";
					...
					while($row=mysqli_fetch_array($result)){
						?>
							<tr bgcolor="<?php echo $couleur;?>">
								<td width="10%">&nbsp;<?php echo stripslashes($row['Libelle']);?></td>
								<td width="16%"><?php echo stripslashes($row['Intitule']);?></td>
								<td width="16%"><?php echo $row['Critere'];?></td>
