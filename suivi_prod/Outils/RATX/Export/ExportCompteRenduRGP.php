...
$req.="WHERE sp_ficheintervention.Id_Pole=".$pole." AND Id_RetourPROD<>0 AND sp_ficheintervention.Vacation<>'' AND Id_StatutPROD<>'' AND ";
...
$req.="FROM sp_ficheintervention LEFT JOIN sp_dossier ON sp_ficheintervention.Id_Dossier=sp_dossier.Id ";
$req.="WHERE sp_ficheintervention.Id_Pole=".$pole." AND Id_StatutPROD<>'' AND ";
if($NumJour>=2 && $NumJour<=4){
	$req.="((sp_ficheintervention.DateIntervention='".$leJour."' AND sp_ficheintervention.Vacation='N') OR ";
	$req.="(sp_ficheintervention.DateIntervention='".$laVeille."' AND (sp_ficheintervention.Vacation='J' OR sp_ficheintervention.Vacation='S'))) ";
}
...
$req.="(SELECT EstRetour FROM sp_retour WHERE sp_retour.Id=sp_ficheintervention.Id_RetourPROD) AS EstRetour ";
$req.="FROM sp_ficheintervention LEFT JOIN sp_dossier ON sp_ficheintervention.Id_Dossier=sp_dossier.Id ";
$req.="WHERE sp_ficheintervention.Id_Pole=".$pole." AND Id_StatutPROD='TFS' AND ";
if($NumJour>=2 && $NumJour<=4){
	$req.="((sp_ficheintervention.DateIntervention='".$leJour."' AND sp_ficheintervention.Vacation='N') OR ";
	$req.="(sp_ficheintervention.DateIntervention='".$laVeille."' AND (sp_ficheintervention.Vacation='J' OR sp_ficheintervention.Vacation='S'))) ";
}
...
$req.="FROM sp_ficheintervention LEFT JOIN sp_dossier ON sp_ficheintervention.Id_Dossier=sp_dossier.Id ";
$req.="WHERE sp_ficheintervention.Id_Pole=".$pole." AND sp_ficheintervention.Vacation<>'' AND ";
if($NumJour>=2 && $NumJour<=4){
	$req.="((sp_ficheintervention.DateIntervention='".$leJour."' AND sp_ficheintervention.Vacation='N') OR ";
	$req.="(sp_ficheintervention.DateIntervention='".$laVeille."' AND (sp_ficheintervention.Vacation='J' OR sp_ficheintervention.Vacation='S'))) ";
}
...
$sheet->getStyle('A'.$ligneJour.':M'.$ligneJour)->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'f2f2f2'))));
$sheet->getStyle('A'.$ligneJour.':M'.$ligneJour)->getBorders()->applyFromArray(array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN ,'color' => array('rgb' => '#000000'))));
$sheet->getStyle('A'.$ligneJour.':M'.$ligneJour)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
$sheet->getStyle('A'.$ligneJour.':M'.$ligneJour)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$ligneJour++;
	...
	while($row=mysqli_fetch_array($result)){
		...
		if($vacation=="Jour"){
			...
			if($row['Id_StatutQUALITE']=="CERT"){
				$sheet->getStyle('A'.$ligneJour.':M'.$ligneJour)->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'00b050'))));
			}
			elseif($row['Id_StatutQUALITE']=="TVS" || $row['Id_StatutPROD']=="TFS"){
				$sheet->getStyle('A'.$ligneJour.':M'.$ligneJour)->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'f6b132'))));
			}
			elseif($row['Id_StatutPROD']=="QARJ"){
				$sheet->getStyle('A'.$ligneJour.':M'.$ligneJour)->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'f9fb3f'))));
			}
			elseif($row['Id_StatutPROD']=="REWORK"){
				$sheet->getStyle('A'.$ligneJour.':M'.$ligneJour)->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'41f9e9'))));
			}
