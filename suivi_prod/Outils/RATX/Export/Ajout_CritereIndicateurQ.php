...
<head>
	...
	<script>
		function FermerEtRecharger(){
			opener.location="Liste_IndicateurRETQ.php";
		}
		...
		elseif($_GET['critere']=="pole"){
			...
			foreach($tab as $Id){
				if($Id<>""){
					...
					$req="SELECT IF(Id=5,'STRUCTURE',Libelle) AS Libelle FROM new_competences_pole WHERE Id=".substr($Id,1);
...
<form class="test" method="POST" action="Ajout_CritereIndicateurQ.php">
	<tr>
		<td>
		...
		<tr>
			...
			<td width=80%>
				<select name="pole">
					<option name="" value=""></option>
					<?php
					$req="SELECT DISTINCT Id, IF(Id=5,'STRUCTURE',Libelle) AS Libelle FROM new_competences_pole WHERE Id IN (1,2,3,5,6) AND Id_Prestation=255 ORDER BY Libelle;";
					$result=mysqli_query($bdd,$req);
					$nbResulta=mysqli_num_rows($result);
					if ($nbResulta>0){
