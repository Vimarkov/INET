...
$req="SELECT IF(sp_olwfi_ecme.ProdQualite=0,'Production','Qualité') AS Metier,
	sp_olwfi_ecme.ProdQualite,
	IF(Id_ECME>0,(SELECT Libelle FROM sp_olwtypeecme WHERE sp_olwtypeecme.Id=(SELECT sp_olwecme.Id_Type FROM sp_olwecme WHERE sp_olwecme.Id=sp_olwfi_ecme.Id_ECME)),
	(SELECT Libelle FROM sp_olwtypeecme WHERE sp_olwtypeecme.Id=sp_olwfi_ecme.Id_TypeECME)) AS TypeECME,
	IF(Id_ECME>0,(SELECT sp_olwecme.Id_Type FROM sp_olwecme WHERE sp_olwecme.Id=sp_olwfi_ecme.Id_ECME),
	sp_olwfi_ecme.Id_TypeECME) AS Id_TypeECME,
	IF(sp_olwfi_ecme.Id_ECME>0,(SELECT Libelle FROM sp_olwecme WHERE sp_olwecme.Id=sp_olwfi_ecme.Id_ECME),sp_olwfi_ecme.ECME) AS Reference,
	...
	(SELECT sp_olwdossier.Reference FROM sp_olwdossier WHERE sp_olwdossier.Id=sp_olwficheintervention.Id_Dossier) AS ReferenceDossier,
	sp_olwficheintervention.DateTERA,sp_olwficheintervention.DateTERC,sp_olwfi_ecme.Id_FI
	FROM sp_olwfi_ecme
	LEFT JOIN sp_olwficheintervention 
	ON sp_olwfi_ecme.Id_FI=sp_olwficheintervention.Id 
	WHERE (SELECT Id_Prestation FROM sp_olwdossier WHERE sp_olwdossier.Id=sp_olwficheintervention.Id_Dossier)=1242 AND ";
	...
	if($_SESSION['EXTRACT_ECMEMetier2']<>""){
		...
		foreach($tab as $valeur){
			 if($valeur<>""){
				$req.="sp_olwfi_ecme.ProdQualite=".$valeur." OR ";
			 }
		}
		$req=substr($req,0,-3);
		$req.=") AND ";
