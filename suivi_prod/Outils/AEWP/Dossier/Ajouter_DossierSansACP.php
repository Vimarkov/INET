...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		$req.=" VALUES (237,".$_SESSION['Id_PersonneSP'].",".$_POST['msn'].",'".$dateRenvoiNC."','".$_POST['imputation']."',".$_POST['client'].",";
		...
		if(substr($_SESSION['DroitSP'],0,1)=='1'){
			...
			foreach($listePREPA as $type){
				...
				$req.="(".$IdFICree.",237,'".$type."','".$DateDebut."','".$HeureDebut."','".$DateFin."','".$HeureFin."','".addslashes($_POST['commentaire'.$type])."')";
	...
	<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
		...
		<tr>
			...
			<td width='15%'>
				<select id="client" name="client">
					...
					<?php
						$req="SELECT Id,Libelle FROM sp_client WHERE Id_Prestation=237 AND Supprime=false ORDER BY Libelle";
		...
		?>
			...
			<td width='15%'>
				<select id="section" name="section">
					<option value=""></option>
					<?php
						$req="SELECT Id,Libelle FROM sp_olwsection WHERE Id_Prestation=237 AND Supprime=false ORDER BY Libelle;";
						$result=mysqli_query($bdd,$req);
						$nbResulta=mysqli_num_rows($result);
						if ($nbResulta>0){
		...
		echo "<tr>";
			...
			echo"<select id='zone' name='zone' ".$disabled.">";
				...
				$req="SELECT Id,Libelle FROM sp_olwzonedetravail  WHERE Id_Prestation=237 AND Supprime=false ORDER BY Libelle;";
				$result=mysqli_query($bdd,$req);
				$nbResulta=mysqli_num_rows($result);
				if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle FROM sp_poste WHERE Id_Prestation=237 AND Supprime=false ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledCE;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=237 AND Supprime=0 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
