		...
		if(substr($_SESSION['DroitSP'],0,1)=='1'){
			...
			if($_POST['idFILiee']==0 || $_POST['idStatutProdFILiee']=="RETOUR PREPA"){	
				...
				foreach($listePREPA as $type){
					...
					$req.="(".$_POST['idFI'].",237,'".$type."','".$DateDebut."','".$HeureDebut."','".$DateFin."','".$HeureFin."','".addslashes($_POST['commentaire'.$type])."')";
					...
					if($nbECME==0){
						...
						$ReqResponsablePostePrestation="
							...
							WHERE
								...
								AND new_competences_personne_poste_prestation.Id_Prestation=237
...
if($_GET){
	...
	if($_GET['Mode']=="M"){
		...
		$req.="sp_olwficheintervention.Id_RetourPROD,sp_olwficheintervention.CommentairePROD,sp_olwficheintervention.Id_SiteIntervention,
			sp_olwficheintervention.TypeAppro,sp_olwficheintervention.AMLiee,sp_olwficheintervention.OTLie,";
			//QUALITE
			...
			<tr>
				...
				<td width='15%'>
					<select id="client" name="client">
						...
						<?php
							$req="SELECT Id,Libelle,Supprime FROM sp_client WHERE Id_Prestation=237 ORDER BY Libelle";
							$result=mysqli_query($bdd,$req);
			...
			<tr>
				...
				<td width='15%'>
					<select id="section" name="section">
						<option value=""></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_olwsection WHERE Id_Prestation=237 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
			...
			<tr>
				<td width='13%' class='Libelle'>&nbsp; Zone de travail : <?php echo $etoile; ?></td><td width='15%'>
					<select id='zone' name='zone' <?php echo $disabled; ?>>
						<option name='0' value='0'></option>
						<?php
							$req="SELECT Id,Libelle, Supprime FROM sp_olwzonedetravail  WHERE Id_Prestation=237 ORDER BY Libelle;";
							$result=mysqli_query($bdd,$req);
							$nbResulta=mysqli_num_rows($result);
							if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%">
								...
								<?php
									$IdPole=0;
									$poste="";
									$req="SELECT Id,Libelle, Supprime FROM sp_poste WHERE Id_Prestation=237 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								<select id="siteIntervention" name="siteIntervention" <?php echo $disabledSTCE;?>>
									<option name="0" value="0"></option>
								<?php
									$req="SELECT Id,Libelle FROM sp_olwsiteintervention WHERE Id_Prestation=237 AND Supprime=false ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
								...
								<table cellpadding='0' cellspacing='0' style="-moz-box-shadow: 4px 4px 10px #888;-webkit-box-shadow: 4px 4px 10px #888;box-shadow:4px 4px 6px #888;">
									...
									<tr>
										...
										<div id='Div_Compagnon' style='height:200px;width:200px;overflow:auto;'>
											...
											$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=237 AND SUBSTR(sp_acces.Droit,3,1)='1' ORDER BY Nom, Prenom;";
											$result=mysqli_query($bdd,$req);
											$nbResulta=mysqli_num_rows($result);
											if ($nbResulta>0){
												$i=0;
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="statutProd" name="statutProd" onchange="Recharge_StatutProd();" <?php echo $disabledNew;?>>
									...
									if($nbTERA>0 && $disabledNew=="disabled='disabled'"){
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=237 AND TypeStatut='P' ORDER BY Id;";
									}
									elseif($nbTERA>0 && $disabledNew<>"disabled='disabled'"){
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=237 AND TypeStatut='P' AND Id<>'TERA' ORDER BY Id;";
									}
									else{
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=237 AND TypeStatut='P' AND Id<>'REWORK' ORDER BY Id;";
									}
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
						...
						<tr>
							...
							<td width="20%" bgcolor="#dbeef9">
								...
								<select id="retourProd" name="retourProd" <?php echo $disabledNew;?>>
									<option name="0" value="0"></option>
									<?php
									$req="SELECT Id, Libelle, Id_Statut, Supprime FROM sp_olwretour WHERE Id_Prestation=237 ORDER BY Libelle;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
										$i=0;
						...
						<tr>
							...
							<td bgcolor="#c7e048" width="20%" colspan="3">
								<select name="IQ">
									...
									$req.="FROM new_rh_etatcivil LEFT JOIN sp_acces ON new_rh_etatcivil.Id=sp_acces.Id_Personne WHERE sp_acces.Id_Prestation=237 AND SUBSTR(sp_acces.Droit,5,1)=1 ORDER BY NomPrenom;";
									$result=mysqli_query($bdd,$req);
									$nbResulta=mysqli_num_rows($result);
									if ($nbResulta>0){
							...
							<td width="20%" bgcolor="#c7e048">
								<div id="statutsQualite">
									<select id="statutQualite" name="statutQualite" onchange="Recharge_StatutQualite();" <?php echo $disabledIQNew;?>>
										<option name="" value=""></option>
										<?php
										$req="SELECT Id FROM sp_olwstatut WHERE Id_Prestation=237 AND TypeStatut='Q' ORDER BY Id;";
										$result=mysqli_query($bdd,$req);
										$nbResulta=mysqli_num_rows($result);
										if ($nbResulta>0){
											$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="typeECMEPROD" name="typeECMEPROD" style="width:100px;" onchange="Recharge_RefECMEPROD()" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
												...
												$req="SELECT Id, Libelle FROM sp_olwtypeecme WHERE Supprime=false AND Id_Prestation=237 ORDER BY Libelle ";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
										...
										<td bgcolor='#e4e7f0'>
											<div id="listeECMEPROD">
												&nbsp; <select id="referencePROD" name="referencePROD" style="width:100px;" onkeypress="if(event.keyCode == 13)AjouterECMEPROD()">
													...
													$req="SELECT Id, Libelle,Id_Type FROM sp_olwecme WHERE Supprime=false AND Id_Prestation=237 ORDER BY Libelle ";
													$result=mysqli_query($bdd,$req);
													$nbResulta=mysqli_num_rows($result);
													if ($nbResulta>0){
														$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>&nbsp; 
											<select id="RefProduit" name="RefProduit" onkeypress="if(event.keyCode == 13)AjouterProduit()" style="width:130px;">
												<?php
												echo"<option name='0' value='0'></option>";
												$req="SELECT Id, Libelle FROM sp_olwingredient WHERE Supprime=false AND Id_Prestation=237 ORDER BY Libelle;";
												$result=mysqli_query($bdd,$req);
												$nbResulta=mysqli_num_rows($result);
												if ($nbResulta>0){
													$i=0;
									...
									<tr>
										<td bgcolor='#e4e7f0'>
											...
											<select id="RefAIPI" name="RefAIPI" onkeypress="if(event.keyCode == 13)AjouterAIPIS()" style="width:130px;">
												...
												$req.="WHERE new_competences_prestation_qualification.Id_Prestation IN (237,950,418) ";
