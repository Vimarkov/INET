...
$req="SELECT DISTINCT sp_olwfi_ecme.Id_FI,sp_olwfi_ecme.ProdQualite,
	...
	WHERE (SELECT Id_Prestation FROM sp_olwdossier WHERE sp_olwdossier.Id=sp_olwficheintervention.Id_Dossier)=-15 
...
if ($nbResulta>0){
	...
	while($row=mysqli_fetch_array($result)){
		...
		if($row['ProdQualite']==0){
			...
			$req.="WHERE (SELECT Id_Prestation FROM sp_olwdossier WHERE sp_olwdossier.Id=sp_olwficheintervention.Id_Dossier)=-15 AND sp_olwficheintervention.Id=".$row['Id_FI'];
