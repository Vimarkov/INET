...
$excel = $workbook->load('BesoinsFormation.xlsx');
$sheet = $excel->getSheetByName('Besoins');
...
SELECT (SELECT MetierPaie FROM new_rh_etatcivil WHERE Id=Id_Personne) AS MetierPaie,
	...
	'0001-01-01' AS DateDebut,
	'0001-01-01' AS DateFin,
	...
	AND epe_personne_souhaitformation.Favorable=1
...
SELECT (SELECT MetierPaie FROM new_rh_etatcivil WHERE Id=Id_Personne) AS MetierPaie,
	...
	(SELECT Cadre FROM new_rh_etatcivil WHERE Id=Id_Personne) AS Cadre,
	DateDebut,DateFin,
	epe_personne_besoinformation.Formation,
	'' AS Priorite,
	epe_personne_besoinformation.Commentaire,
	epe_personne.DateEntretien,
	'Manager' AS Emetteur
	FROM epe_personne_besoinformation
	LEFT JOIN epe_personne
	ON epe_personne_besoinformation.Id_epepersonne=epe_personne.Id
	WHERE epe_personne.Suppr=0 AND epe_personne.Type='EPE' 
	AND epe_personne_besoinformation.Suppr=0
	AND YEAR(epe_personne.DateButoir) = ".$_SESSION['FiltreEPEIndicateurs_Annee']." 
	AND IF(ModeBrouillon=1,'Brouillon',IF(DateSalarie<='0001-01-01','Signature salarié',IF(DateEvaluateur>'0001-01-01' ,'Réalisé','Signature manager'))) IN ('Signature salarié','Signature manager','Réalisé') ";
if(DroitsFormation1Plateforme(17,array($IdPosteAssistantRH,$IdPosteResponsableRH))){
...
if($nbResulta>0){
	...
	while($row=mysqli_fetch_array($result)){
		...
		if($row['DateAncienneteCDI']>'0001-01-01' && $row['DateAncienneteCDI']<>'0001-01-01' ){
		...
		if($row['DateEntretien']>'0001-01-01' && $row['DateEntretien']<>'0001-01-01' ){
		...
		if(($row['DateDebut']>'0001-01-01' && $row['DateDebut']<>'0001-01-01' ) || ($row['DateFin']>'0001-01-01' && $row['DateFin']<>'0001-01-01' )){
			$sheet->setCellValue('L'.$ligne,utf8_encode(stripslashes(AfficheDateJJ_MM_AAAA($row['DateDebut'])." - ").AfficheDateJJ_MM_AAAA($row['DateFin'])));
		}
		...
		if($row['Emetteur']=="Salarie"){$sheet->setCellValue('O'.$ligne,utf8_encode("Souhait du salarié validé par le manager"));}
		else{$sheet->setCellValue('O'.$ligne,utf8_encode("Emis par le manager"));}
