...
$excel = $workbook->load('TemplateEPP.xlsx');
...
if($nbResulta>0){
	...
	while($row=mysqli_fetch_array($result)){
		...
		if($rowNb['EPPReprise']==1){
			$sheet->setCellValue('M'.$ligne,utf8_encode(stripslashes("X")));
		}
		if($rowNb['EPPRefuseSalarie']==1){
			$sheet->setCellValue('N'.$ligne,utf8_encode(stripslashes("X")));
		}
		if($rowNb['SouhaitEvolutionON']==1){
			$sheet->setCellValue('O'.$ligne,utf8_encode(stripslashes($rowNb['SouhaitEvolution'])));
		}
		if($rowNb['SouhaitMobiliteON']==1){
			$sheet->setCellValue('P'.$ligne,utf8_encode(stripslashes($rowNb['SouhaitMobilite'])));
		}
		if($rowNb['FormationEvolutionON']==1){
			$sheet->setCellValue('Q'.$ligne,utf8_encode(stripslashes($rowNb['FormationEvolution'])));
		}
		$sheet->setCellValue('R'.$ligne,utf8_encode(stripslashes($rowNb['ComEvaluateur'])));
		...
		$sheet->getStyle('A'.$ligne.':T'.$ligne)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		$sheet->getStyle('A'.$ligne.':T'.$ligne)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
