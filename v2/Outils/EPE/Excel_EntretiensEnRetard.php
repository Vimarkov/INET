...
if($LangueAffichage=="FR"){
	...
	$sheet->setCellValue('G1',utf8_encode("Date prévisionnelle"));
	$sheet->setCellValue('H1',utf8_encode("Responsable"));
}
else{
	...
	$sheet->setCellValue('G1',utf8_encode("Expected date"));
	$sheet->setCellValue('H1',utf8_encode("Responsible"));
}
...
if($nbenreg>0){
	...
	while($row=mysqli_fetch_array($result)){
		...
		if($trouve==1){
			...
			if($row['DatePrevisionnelle']>'0001-01-01' ){
				$date = explode("-",$row['DatePrevisionnelle']);
				$time = PHPExcel_Shared_Date::FormattedPHPToExcel($date[0], $date[1], $date[2]);
			...
			$sheet->setCellValue('H'.$ligne,utf8_encode(stripslashes($Manager)));
