...
if($LangueAffichage=="FR"){
	...
	$sheet->setCellValue('E1',utf8_encode("Cadre"));
	...
	$sheet->setCellValue('E1',utf8_encode("Organization"));
...
$req="SELECT DISTINCT new_rh_etatcivil.Id, CONCAT(new_rh_etatcivil.Nom,' ',new_rh_etatcivil.Prenom) AS Personne,MatriculeAAA,
	...
	(SELECT (SELECT Libelle FROM new_competences_plateforme WHERE Id=Id_Plateforme) FROM new_competences_personne_plateforme
					WHERE Id=Id_Personne AND Id_Plateforme NOT IN (11,14) LIMIT 1) AS Plateforme
...
if($nbenreg>0){
	...
	while($row=mysqli_fetch_array($result)){
		...
		$sheet->setCellValue('D'.$ligne,utf8_encode(stripslashes($row['Contrat'])));
		$sheet->setCellValue('E'.$ligne,utf8_encode(stripslashes($cadre)));
		$sheet->setCellValue('F'.$ligne,utf8_encode(stripslashes($row['Plateforme'])));
