			...
			<?php if($_POST){ 
				if($_POST['annee']<>""){
					...
					if($_SESSION['FiltreEPEIndicateurs_Type']<>""){
						...
						$requete.="AND TypeEntretien IN ('EPP','EPP Bilan') ";
					}
					...
					if($nbResulta>0){
						while($row=mysqli_fetch_array($result))
						{
							...
							'A faire')
							AS Etat,
							(SELECT IF(Type='EPP',EPPRefuseSalarie,EPPBilanRefuseSalarie)
										FROM epe_personne 
										WHERE Suppr=0 AND epe_personne.Type=epe_personne_datebutoir.TypeEntretien AND epe_personne.Id_Personne=new_rh_etatcivil.Id AND YEAR(epe_personne.DateButoir) = ".$_SESSION['FiltreEPE_Annee']." LIMIT 1) AS Refus,
							...
							if($_SESSION['FiltreEPEIndicateurs_Type']<>""){
								...
								$reqNb.="AND TypeEntretien IN ('EPP','EPP Bilan') ";
							}
							...
							if($leNb>0){
								while($rowNb=mysqli_fetch_array($ResultNb))
								{
									...
									if($trouve==1){
										...
										if($rowNb['Refus']==0){$Non++;}
										elseif($rowNb['Refus']==1){$Oui++;}
									}
								}
							}
					...
					$array[0]=array("Abscisse" => utf8_encode("Refusé"),"Nombre" => $Oui);
					$array[1]=array("Abscisse" => utf8_encode("Réalisé"),"Nombre" => $Non);
			?>
			<tr>
				<td height="400px;">
					<table class="TableCompetences" width="99%" height="100%" cellpadding="0" cellspacing="0">
						<tr>
							<td class="Libelle" height="5%" style="font-size:15px;"><?php if($_SESSION['Langue']=="FR"){echo "TAUX DE REFUS";}else{echo "REFUSAL RATE";} ?></td>
