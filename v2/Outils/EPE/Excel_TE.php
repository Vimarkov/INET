...
if($LangueAffichage=="FR"){
	...
	$sheet->setCellValue('G1',utf8_encode("Type d'évolution"));
	$sheet->setCellValue('H1',utf8_encode("Souhait d'évolution"));
}
else{
	$sheet->setTitle(utf8_encode("RPS"));
	...
	$sheet->setCellValue('G1',utf8_encode("Type of evolution"));
	$sheet->setCellValue('H1',utf8_encode("Wish evolution"));
...
SELECT DISTINCT Id_EPE, Id_SouhaitEvolution,
	...
	(SELECT Libelle FROM epe_typeevolution WHERE Id=Id_SouhaitEvolution) AS TypeEvolution,
...
SELECT DISTINCT Id_EPE, Id_SouhaitEvolution,
	...
	LEFT JOIN epe_personne
	ON epe_personne_souhaitevolution2.Id_EPE=epe_personne.Id
	WHERE epe_personne.Suppr=0 AND epe_personne.Type='EPP' 
	AND SouhaitEvolutionON=1
	AND PasEvolutionEPP=0
	AND YEAR(epe_personne.DateButoir) = ".$_SESSION['FiltreEPEIndicateurs_Annee']." 
...
if($nbResulta>0){
	...
	while($row=mysqli_fetch_array($result)){
		...
		$sheet->setCellValue('G'.$ligne,utf8_encode(stripslashes($row['TypeEvolution'])));
		
		$sheet->setCellValue('H'.$ligne,utf8_encode(stripslashes($row['SouhaitEvolution'])));
