...
<table width="100%" cellpadding="0" cellspacing="0" align="center">
	...
	<tr>
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#f3bad3;">
				<tr>
					<td class="TitrePage">
					<?php
		...
		<td align="center" valign="top" width="85%">
			...
			<table width="100%">
				<?php 
					...
					";
					if($Id_Plateformes<>""){
						$requete.=" AND Id_Plateforme IN (".$Id_Plateformes.") ";
					}
					...
					$requete="SELECT SUM(Nombre)-(SELECT COUNT(Id) 
									...
									AND Id_Annonce=talentboost_annonce.Id AND CandidatRetenu=1 ";
					...
					$requete="SELECT Id
							FROM talentboost_annonce
							WHERE talentboost_annonce.Suppr=0  
							AND ValidationContratDG=1  ";
							if($_SESSION['FiltreRecrutementIndicateur_DateDebut']<>""){
								$requete.="AND DateValidationDG>='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateDebut'])."' ";
							}
							if($_SESSION['FiltreRecrutementIndicateur_DateFin']<>""){
								$requete.="AND DateValidationDG<='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateFin'])."' ";
							}
							$requete.="
							AND EtatPoste>=0
							 ";
					if($Id_Plateformes<>""){
						$requete.=" AND talentboost_annonce.Id_Plateforme IN (".$Id_Plateformes.") ";
					}
					$result=mysqli_query($bdd,$requete);
					...
					$requete="SELECT Id
							...
							$requete.="
							AND EtatPoste>=0
							AND (SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id AND CandidatRetenu=1)>0
							 ";
					if($Id_Plateformes<>""){
						$requete.=" AND talentboost_annonce.Id_Plateforme IN (".$Id_Plateformes.") ";
					}
					...
					$requete="SELECT talentboost_candidature.Id_Personne
							FROM talentboost_candidature
							LEFT JOIN talentboost_annonce
							ON talentboost_annonce.Id=talentboost_candidature.Id_Annonce
							WHERE talentboost_annonce.Suppr=0  
							AND talentboost_candidature.Suppr=0
							AND ValidationContratDG=1 ";
							if($_SESSION['FiltreRecrutementIndicateur_DateDebut']<>""){
								$requete.="AND DateValidationDG>='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateDebut'])."' ";
							}
					...
					$requete="SELECT DISTINCT talentboost_candidature.Id_Personne
							FROM talentboost_candidature
							LEFT JOIN talentboost_annonce
							ON talentboost_annonce.Id=talentboost_candidature.Id_Annonce
							WHERE talentboost_annonce.Suppr=0  
						...
						<table class="GeneralInfo" width="15%">
							...
							<tr>
								...
								<td width="50%" valign="top">
									<table width="100%">
										<tr>
											<td class="Libelle" align="left" >&nbsp;<?php if($LangueAffichage=="FR"){echo "NOMBRE DE PROPOSITIONS : ";}else{echo "NUMBER OF PROPOSALS : ";}?><b style="font-size:25px;color:<?php if($nbPropositions>0){echo "#ff3b3b";}else{echo "#000000";}?>"><?php echo $nbPropositions; ?></b></td>
										</tr>
									</table>
								</td>
							</tr>
	...
	<tr>
		<td height="350px;" width="85%">
			<?php 
				...
				$requete="SELECT (SELECT Libelle FROM new_competences_plateforme WHERE Id=Id_Plateforme) AS Plateforme,
							COUNT(Id) AS Nb
							FROM talentboost_annonce
							WHERE Suppr=0  
							AND ValidationContratDG=1  
				...
				$requete.="GROUP BY Id_Plateforme ";
				$result=mysqli_query($bdd,$requete);
				$nbenreg=mysqli_num_rows($result);
				if($nbenreg>0){
					$i=0;
				...
				$requete="SELECT IF(ValidationContratDG=0,'',
				...
				$requete.="GROUP BY IF(ValidationContratDG=0,'',
						IF(ValidationContratDG=-1,'',
							IF(EtatPoste=0,'Poste ouvert',IF(EtatPoste=1,'Poste pourvu',IF(EtatPoste=2,'Poste non pourvu',IF(EtatPoste=3,'Poste pourvu partiellement',IF(EtatPoste=4,'Demande clôturée','Poste annulé')))))
						)
					) ";
				...
				$requete="SELECT Categorie,
						...
						FROM
						(SELECT IF((SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id)=0,0,
									IF((SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id)>0 AND (SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id)<3,'1-2',
										IF((SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id)>2 AND (SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id)<6,'3-5','>5'
										)
									)
								) AS Categorie,
							(SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id) AS NbCandidature
							FROM talentboost_annonce
							WHERE Suppr=0  
							AND ValidationContratDG=1 
							AND EtatPoste=1 ";
				...
				$requete="SELECT Categorie,
							COUNT(NbCandidature) AS NbCandidature
						FROM
						(SELECT IF((SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id ";
							if($_SESSION['FiltreRecrutementIndicateur_DateDebut']<>""){
								$requete.="AND talentboost_candidature.DateCreation>='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateDebut'])."' ";
							}
							if($_SESSION['FiltreRecrutementIndicateur_DateFin']<>""){
								$requete.="AND talentboost_candidature.DateCreation<='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateFin'])."' ";
							}
							$requete.=")=0,0,
									IF((SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id ";
							if($_SESSION['FiltreRecrutementIndicateur_DateDebut']<>""){
								$requete.="AND talentboost_candidature.DateCreation>='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateDebut'])."' ";
							}
							if($_SESSION['FiltreRecrutementIndicateur_DateFin']<>""){
							...
							$requete.=")>0 AND (SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id ";
							if($_SESSION['FiltreRecrutementIndicateur_DateDebut']<>""){
								$requete.="AND talentboost_candidature.DateCreation>='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateDebut'])."' ";
							}
							if($_SESSION['FiltreRecrutementIndicateur_DateFin']<>""){
							...
							$requete.=")>2 AND (SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id ";
							if($_SESSION['FiltreRecrutementIndicateur_DateDebut']<>""){
								$requete.="AND talentboost_candidature.DateCreation>='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateDebut'])."' ";
							}
							if($_SESSION['FiltreRecrutementIndicateur_DateFin']<>""){
							...
							(SELECT COUNT(Id) FROM talentboost_candidature WHERE Suppr=0 AND Id_Annonce=talentboost_annonce.Id ";
							if($_SESSION['FiltreRecrutementIndicateur_DateDebut']<>""){
								$requete.="AND talentboost_candidature.DateCreation>='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateDebut'])."' ";
							}
							if($_SESSION['FiltreRecrutementIndicateur_DateFin']<>""){
								...
								<td valign="top">
									<table class="TableCompetences" width="99%" cellpadding="0" cellspacing="0">
										...
										<?php 
											...
											) AS Ref,Lieu,talentboost_annonce.Nombre AS NombrePoste,
											(SELECT COUNT(Id) FROM talentboost_candidature WHERE Id_Annonce=talentboost_annonce.Id AND talentboost_candidature.Suppr=0 ";
							$requete2.=") AS Nombre
											FROM talentboost_annonce
											WHERE Suppr=0  
											AND ValidationContratDG=1 ";
											if($_SESSION['FiltreRecrutementIndicateur_DateDebut']<>""){
												$requete2.="AND DateValidationDG>='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateDebut'])."' ";
											}
											if($_SESSION['FiltreRecrutementIndicateur_DateFin']<>""){
												$requete2.="AND DateValidationDG<='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateFin'])."' ";
											}
					...
					<td width="50%" valign="top">
						<table class="TableCompetences" width="99%" height="100%" cellpadding="0" cellspacing="0">
							...
							$requete2.=") AS Nombre
											...
											WHERE Suppr=0  
											AND ValidationContratDG=1 
											AND ((SELECT COUNT(Id) FROM talentboost_candidature WHERE Id_Annonce=talentboost_annonce.Id AND talentboost_candidature.Suppr=0 ";
							$requete2.=")=0
											OR ((SELECT COUNT(Id) FROM talentboost_candidature WHERE Id_Annonce=talentboost_annonce.Id AND talentboost_candidature.Suppr=0 ";
							$requete2.=")/talentboost_annonce.Nombre)<0.4
											) ";
											if($_SESSION['FiltreRecrutementIndicateur_DateDebut']<>""){
												$requete2.="AND DateValidationDG>='".TrsfDate_($_SESSION['FiltreRecrutementIndicateur_DateDebut'])."' ";
											}
								...
								<td valign="top">
									<table class="TableCompetences" width="99%" cellpadding="0" cellspacing="0">
										...
										<?php 
											...
											Lieu,'-',
											Programme,'-',IF(PosteDefinitif=1,'D',IF(PosteDefinitif=2 OR PosteDefinitif=3 OR PosteDefinitif=4,'C','M')),'-',DATE_FORMAT(DateValidationDG,'%d%m%y')
											) AS Ref,Metier,Lieu,talentboost_annonce.Nombre AS NombrePoste,
											(SELECT COUNT(Id) FROM talentboost_candidature WHERE Id_Annonce=talentboost_annonce.Id AND talentboost_candidature.Suppr=0 
											AND CandidatRetenu=1) AS Nombre,
											talentboost_annonce.Nombre-(SELECT COUNT(Id) FROM talentboost_candidature WHERE Id_Annonce=talentboost_annonce.Id AND talentboost_candidature.Suppr=0 
												AND CandidatRetenu=1
											) AS Restant
											FROM talentboost_annonce
											WHERE Suppr=0  
