...
if($_POST)
{
	if($_POST['Mode']=="Ajout")
	{
		$requeteInsertUpdate="INSERT INTO talentboost_savoiretre (Libelle)";
	...
	{		
		$requeteInsertUpdate="UPDATE talentboost_savoiretre SET";
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="Ajout" || $_GET['Mode']=="Modif")
	{
		...
		{
			$Modif=True;
			$result=mysqli_query($bdd,"SELECT Id, Libelle FROM talentboost_savoiretre WHERE Id=".$_GET['Id']." AND Suppr=0");
...
<?php
	...
	//Mode suppression
	{
		$result=mysqli_query($bdd,"UPDATE talentboost_savoiretre SET Suppr=1,Id_Suppr=".$_SESSION['Id_Personne'].", DateSuppr='".date('Y-m-d')."' WHERE Id=".$_GET['Id']);
