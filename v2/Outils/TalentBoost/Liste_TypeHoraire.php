...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
	{
		...
		{
			var w= window.open("Ajout_TypeHoraire.php?Mode="+Mode+"&Id="+Id,"PageLieu","status=no,menubar=no,width=450,height=200");
			w.focus();
		}
	}
</script>
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#f5f74b;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Types horaires";}else{echo "Hourly types";}
					?>
					</td>
				</tr>
			</table>
	...
	<tr>
		...
		<td align="left">
			<table class="TableCompetences" width="50%">
				<tr>
					<td class="EnTeteTableauCompetences" width="90%"><?php if($_SESSION["Langue"]=="FR"){echo "Type";}else{echo "Type";}?></td>
			...
			<?php
				$req="SELECT Id, 
					Libelle
					FROM talentboost_typehoraire WHERE Suppr=0 
					ORDER BY Libelle ";
