...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
	{
		...
		{
			var w= window.open("Ajout_Prerequis.php?Mode="+Mode+"&Id="+Id,"PagePrerequis","status=no,menubar=no,width=450,height=200");
			w.focus();
		}
	}
</script>
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#f5f74b;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Prérequis";}else{echo "Prerequisites";}
					?>
					</td>
				</tr>
			</table>
	...
	<tr>
		...
		<td align="left">
			...
			<?php
				$req="SELECT Id, 
					Libelle
					FROM talentboost_prerequis WHERE Suppr=0 
					ORDER BY Libelle ";
