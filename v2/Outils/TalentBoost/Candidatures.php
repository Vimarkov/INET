...
if($_GET){
	...
	if($nbResulta>0){
		...
		{
			if(isset($_POST['CandidatRetenu_'.$row['Id']])){$req="UPDATE talentboost_candidature SET CandidatRetenu=1 WHERE Id=".$row['Id']." ";}
			else{$req="UPDATE talentboost_candidature SET CandidatRetenu=0 WHERE Id=".$row['Id']." ";}
			$resulM=mysqli_query($bdd,$req);
		}
	}
			
	if(isset($_POST['btnEnregistrer'])){
		$req="UPDATE talentboost_annonce SET EtatPoste=".$_POST['statutPoste']." WHERE Id=".$Id." ";
		$resulM=mysqli_query($bdd,$req);
...
$requete="SELECT Id,DateDemande,Id_Demandeur,Id_Prestation,Metier,Nombre,Lieu,RaisonRefus,Suppr,
			...
			Programme,CreationPoste,CategorieProf,
			DescriptifPoste,SavoirFaire,SavoirEtre,Prerequis,DateRecrutement,Diplome,
			DateBesoin,PosteDefinitif,Id_Prestation,Id_Domaine,Horaire,
									...
									<tr>
										...
										<td width="18%">
											&nbsp;<select name="statutPoste" id="statutPoste">
												...
												<option value="4" <?php if($row2['EtatPoste']==4){echo "selected";} ?>><?php if($_SESSION["Langue"]=="FR"){echo "Demande clôturée";}else{echo "Request closed";} ?></option>
											</select>
										</td>
										<td width="8%">
											<?php 
