...
<head>
	...
	<script type="text/javascript">
		...
		{
			window.opener.location="Liste_Statut.php";
			window.close();
		}
	</script>
</head>
...
if($_POST)
{
	if($_POST['Mode']=="Ajout")
	{
		$requeteInsertUpdate="INSERT INTO talentboost_categorieprofessionnelle (Libelle)";
	...
	{		
		$requeteInsertUpdate="UPDATE talentboost_categorieprofessionnelle SET";
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="Ajout" || $_GET['Mode']=="Modif")
	{
		...
		{
			$Modif=True;
			$result=mysqli_query($bdd,"SELECT Id, Libelle FROM talentboost_categorieprofessionnelle WHERE Id=".$_GET['Id']." AND Suppr=0");
			$row=mysqli_fetch_array($result);
		}
?>
		<form id="formulaire" enctype="multipart/form-data" method="POST" action="Ajout_Statut.php" onSubmit="return VerifChamps();">
...
<?php
	...
	//Mode suppression
	{
		$result=mysqli_query($bdd,"UPDATE talentboost_categorieprofessionnelle SET Suppr=1 WHERE Id=".$_GET['Id']);
