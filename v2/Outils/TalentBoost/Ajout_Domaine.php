...
if($_POST)
{
	if($_POST['Mode']=="Ajout")
	{
		...
		$RequeteInsertUpdate="
			INSERT INTO talentboost_domaine
			(
				Libelle
			)
			VALUES
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="Ajout" || $_GET['Mode']=="Modif")
	{
		...
		{
			$Modif=true;
			$Result=mysqli_query($bdd,"SELECT Id, Libelle FROM talentboost_domaine WHERE Id='".$_GET['Id']."';");
...
<?php
	...
	//Mode suppression
	{
		$Result=mysqli_query($bdd,"UPDATE talentboost_domaine SET Suppr=1 WHERE Id=".$_GET['Id']);
		echo "<script>FermerEtRecharger();</script>";
	}
