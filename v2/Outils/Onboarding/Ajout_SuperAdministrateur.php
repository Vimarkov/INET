...
if($_POST){
	if($_POST['Mode']=="A"){
		$req="INSERT INTO onboarding_superadministrateur (Id_Personne) VALUES (".$_POST['personne'].")";
...
?>
		...
		<form id="formulaire" method="POST" action="Ajout_SuperAdministrateur.php" onSubmit="return VerifChamps('<?php echo $_SESSION['Langue'];?>');">
			...
			<tr class="TitreColsUsers">
				...
				<td>
					...
					if($_GET['Mode']=="A"){
						...
						$req="SELECT DISTINCT new_rh_etatcivil.Id, Nom, Prenom FROM new_rh_etatcivil WHERE Id NOT IN (SELECT Id_Personne FROM onboarding_superadministrateur) ORDER BY Nom, Prenom;";
						$result=mysqli_query($bdd,$req);
...
<?php
	...
	//Mode suppression
	{
		$req="DELETE FROM onboarding_superadministrateur WHERE Id_Personne=".$_GET['Id'];
		$result=mysqli_query($bdd,$req);
