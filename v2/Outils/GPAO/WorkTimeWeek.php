	...
	<script>
		function OuvreFenetreExcel(){
			if(document.getElementById('annee').value!=""){
				var w=window.open("Extract_WorkTimeWeek.php?Annee="+document.getElementById('annee').value+"&Semaine="+document.getElementById('semaine').value,"PageProduction","status=no,menubar=no,scrollbars=yes,width=50,height=50");
				w.focus();
			}
		}
	</script>
...
<form id="formulaire" method="POST" action="WorkTimeWeek.php">
	...
	<table width="95%" align="center" class="TableCompetences">
		...
		<tr>
			...
			<td class="Libelle" width='10%'><?php if($_SESSION['Langue']=="EN"){echo "Week";}else{echo "Week";} ?> :</td>
			<td width='10%'>
				<input type="texte" onKeyUp="nombre(this)" name="semaine" id="semaine" size="8" value="<?php echo $semaine;?>">
			</td>
			<td align="center">
