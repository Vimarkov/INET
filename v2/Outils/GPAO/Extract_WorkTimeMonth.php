...
$sheet->setCellValue('C1',utf8_encode('Month'));
...
if($nbRapport>0){
	...
	while($row=mysqli_fetch_array($resultRapport)){
		...
		$sheet->setCellValue('C'.$ligne,utf8_encode($mois));
...
header('Content-Disposition: attachment;filename="Extract_WorkTimeMonth.xlsx"'); 
header('Cache-Control: max-age=0'); 

$writer = PHPExcel_IOFactory::createWriter($workbook, 'Excel2007');

$chemin = '../../tmp/Extract_WorkTimeMonth.xlsx';
$writer->save($chemin);
readfile($chemin);
?>