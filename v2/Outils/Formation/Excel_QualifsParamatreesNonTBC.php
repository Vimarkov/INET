...
if($nbPresta>0){
	while($rowPrestation=mysqli_fetch_array($resultPrestation))
	{
		...
		$reqParam="SELECT Id_Qualification,
				...
				AND Id_Formation IN (
						...
						WHERE Suppr=0 
						AND Id_Prestation=".$rowPrestation['Id_Prestation']." AND Id_Pole=".$rowPrestation['Id_Pole']." 
					) 
		...
		if($nbParam>0){
			while($rowParam=mysqli_fetch_array($resultParam))
			{
				...
				WHERE Id_Prestation=".$rowPrestation['Id_Prestation']." 
				AND Id_Qualification=".$rowParam['Id_Qualification']." ";
				...
				if($nbQualif==0){
					...
					$sheet->setCellValue('B'.$ligne,utf8_encode($rowParam['CategorieMaitre']));
					$sheet->setCellValue('C'.$ligne,utf8_encode($rowParam['Categorie']));
					$sheet->setCellValue('D'.$ligne,utf8_encode($rowParam['Qualification']));
...
if($LangueAffichage=="FR"){header('Content-Disposition: attachment;filename="QualificationsParametreesSansTBC.xlsx"');}
else{header('Content-Disposition: attachment;filename="QualificationsParametreesSansTBC.xlsx"');}
header('Cache-Control: max-age=0'); 

$writer = PHPExcel_IOFactory::createWriter($workbook, 'Excel2007');
$chemin = '../../tmp/QualificationsParametreesSansTBC.xlsx';
$writer->save($chemin);
readfile($chemin);
?>