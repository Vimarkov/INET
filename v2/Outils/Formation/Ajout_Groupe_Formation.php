...
if($_POST)
{
	if(isset($_POST['validation'])){
		$requete="";
		if($_POST['Mode']=="Ajout"){
			if(mysqli_num_rows(mysqli_query($bdd,"SELECT Id FROM form_groupe_formation WHERE Libelle='".addslashes($_POST['Libelle'])."' AND Id_Plateforme=".$_POST['Id_Plateforme']))==0)
			{
				$requete="INSERT INTO form_groupe_formation (Id_Plateforme, Libelle,Id_Personne_MAJ,Date_MAJ)";
				$requete.=" VALUES (";
		...
		elseif($_POST['Mode']=="Modif"){
			if(mysqli_num_rows(mysqli_query($bdd,"SELECT Id FROM form_groupe_formation WHERE Libelle='".addslashes($_POST['Libelle'])."' AND Id_Plateforme=".$_POST['Id_Plateforme']." AND Id!=".$_POST['Id']))==0)
			{
				$result=mysqli_query($bdd,"DELETE FROM form_groupe_formation_formation WHERE Id_Groupe_Formation=".$_POST['Id']);
				$requete="UPDATE form_groupe_formation SET";
		...
		if($requete!="")
		{
			...
			$requeteDeb="INSERT INTO form_groupe_formation_formation (Id_Groupe_Formation, Id_Formation,Recyclage,Id_Personne_MAJ,Date_MAJ) VALUES";
			$requeteFin="";
...
if($Mode=="Ajout" || $Mode=="Modif")
{
	...
	{
		$Modif=True;
		$result=mysqli_query($bdd,"SELECT Id, Id_Plateforme, Libelle FROM form_groupe_formation WHERE Id=".$IdGroupe);
		$row=mysqli_fetch_array($result);
	}
?>
	<form id="formulaire" method="POST" action="Ajout_Groupe_Formation.php" onSubmit="return VerifChamps();">
	<input type="hidden" name="Mode" value="<?php echo $Mode; ?>">
	<input type="hidden" name="Id" value="<?php echo $IdGroupe;?>">
	...
	<table style="width:100%;align:center;" class="TableCompetences">
		<tr class="TitreColsUsers">
			...
			<td width="85%">
				<textarea name="Libelle" id="Libelle" cols="60" rows="4" style='resize: none;'><?php if($_GET){if($Modif){echo stripslashes($row['Libelle']);}}else{echo $_POST['Libelle'];} ?></textarea>
			</td>
		</tr>
		<tr>
				...
				<table style="width:100%;">
					...
					<tr>
						<td width="50%" valign="top">
							<?php
								if($Modif){
									$rqFormation="SELECT form_groupe_formation_formation.Id_Formation, form_groupe_formation_formation.Recyclage ";
									$rqFormation.="FROM form_groupe_formation_formation LEFT JOIN form_groupe_formation ";
									$rqFormation.="ON form_groupe_formation_formation.Id_Groupe_Formation=form_groupe_formation.Id ";
									$rqFormation.="WHERE form_groupe_formation_formation.Id_Groupe_Formation=".$IdGroupe." ";
									$rqFormation.="AND form_groupe_formation_formation.Suppr=0 ";
									$rqFormation.="AND (form_groupe_formation.Id_Plateforme=0 OR form_groupe_formation.Id_Plateforme=".$Plateforme.") ";
							...
							<select name="Id_Formation" id="Id_Formation" multiple size="20" style="width:450px;" onDblclick="Transferer_Liste('Id_Formation','FormationSelect');">
								...
								$rqFormation.= "FROM form_groupe_formation_formation WHERE Id_Groupe_Formation=".$IdGroupe." ";
...
//Mode suppression
{
	$result=mysqli_query($bdd,"UPDATE form_groupe_formation SET Suppr=1,Id_Personne_MAJ=".$IdPersonneConnectee.", Date_MAJ='".date('Y-m-d')."' WHERE Id=".$_GET['Id']);
	echo "<script>FermerEtRecharger();</script>";
}
