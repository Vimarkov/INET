...
if($_POST)
{
	if(isset($_POST['valider'])){
		$ReqSelectFormSessionPersonneQualification="
			SELECT Id
			FROM
				form_session_personne_qualification
			WHERE
				DateHeureRepondeur<='0001-01-01'
			 AND Id=".$_POST['Id_Session_Personne_Qualification'];
		$ResultSelectFormSessionPersonneQualification=mysqli_query($bdd,$ReqSelectFormSessionPersonneQualification);
		$nbSelectFormSessionPersonneQualification=mysqli_num_rows($ResultSelectFormSessionPersonneQualification);
		
		if($nbSelectFormSessionPersonneQualification>0){
			//Enregistrement des réponses de la personne
			$ReqInsertReponseQCM="
				INSERT INTO
					form_session_personne_qualification_question_reponse
				(
					Id_Session_Personne_Qualification_Question,
					Id_QCM_Langue_Question_Reponse,
					Valeur
				)
				VALUES ";
			$Tableau_Reponses=explode("#",$_POST["ListeReponses"]);
			for($i=0;$i<sizeof($Tableau_Reponses)-1;$i++)
			{
				$Reponses=explode("|",$Tableau_Reponses[$i]);
				$ReqInsertReponseQCM.="
					(".
						$Reponses[0].",".
						$Reponses[1].",".
			...
			$Tableau_Reponses=explode("#",$_POST["ListeReponsesFille"]);
			for($i=0;$i<sizeof($Tableau_Reponses)-1;$i++)
			{
				$Reponses=explode("|",$Tableau_Reponses[$i]);
				$ReqInsertReponseQCM.="
			...
			$ReqInsertReponseQCM=substr($ReqInsertReponseQCM,0,-1);
			$ResultInsertReponseQCM=mysqli_query($bdd,$ReqInsertReponseQCM);
			
			//Validation de la présence si QCM suite à session de formation 
			$req="SELECT Id_Session_Personne
				FROM form_session_personne_qualification
				WHERE Id=".$_POST['Id_Session_Personne_Qualification'];
			$ResultSessionPersonne=mysqli_query($bdd,$req);	
			$RowSessionPersonne=mysqli_fetch_array($ResultSessionPersonne);
			If($RowSessionPersonne['Id_Session_Personne']>0)
			{
				$ReqUpdateFormSessionPersonne="
				UPDATE
					form_session_personne
				SET
					Presence=1
				WHERE
					Id=".$RowSessionPersonne['Id_Session_Personne'];
				$ResultUpdateFormSessionPersonne=mysqli_query($bdd,$ReqUpdateFormSessionPersonne);
			}
			...
			$ReqUpdateFormSessionPersonneQualification="
				...
				WHERE
					Id=".$_POST['Id_Session_Personne_Qualification'];
			$ResultUpdateFormSessionPersonneQualification=mysqli_query($bdd,$ReqUpdateFormSessionPersonneQualification);
			
			//#################################################################
			//Noter les QCM et les différentes questions
			//#################################################################
			calculResultatQCMs($_POST['Id_Session_Personne_Qualification']);
		}
		echo "<script>FermerEtRecharger('".$_POST['Id_Session_Personne_Qualification']."','".$_POST['Page']."');</script>";
	}
	elseif(isset($_POST['terminer'])){
		echo "<script>FermerEtRecharger2('".$_POST['Id_Session_Personne_Qualification']."','".$_POST['Page']."');</script>";
	}
}
else
{
	...
	elseif($RowTypePassage['TypePassageQCM']==1)
	{
		$ReqFormSessionPersonneQualification="
            SELECT
				...
				(SELECT (SELECT Id_Plateforme FROM new_competences_prestation WHERE new_competences_prestation.Id=Id_Prestation) FROM form_besoin WHERE form_besoin.Id=form_session_personne_qualification.Id_Besoin) AS Id_Plateforme
            FROM
                form_session_personne_qualification
    ...
    if($RowFormSessionPersonneQualification['DateHeureRepondeur']>'0001-01-01 00:00:00'){$QCM_Repondu=true;}
    ...
    if(($QCM_Ouvert || $QCM_Repondu || DroitsFormationPlateforme($TableauIdPostesAF_RF_FORM_PS_RQP) || (DroitsFormationPrestation($TableauIdPostesCQ) && $RowTypePassage['TypePassageQCM']>0)) && $QCM_Acces_OK)
    {
		...
		echo "
			<form id='formulaire' method='POST' action='QCM_Web_v3.php' onSubmit='return VerifChamps();'>
		...
		foreach($tabQCM AS $QCM)
		{
			...
			foreach($QCM_Q_R_RStagiaires as $Ligne_Q_R_RStagiaires)
			{
				...
				if($Ligne_Q_R_RStagiaires[0]."_".$Ligne_Q_R_RStagiaires[1] <> $numQuestionPrecedente."_".$QuestionPrecedente)
				{
				    if($couleur=="#ffffff"){$couleur="#dcdcdc";}else{$couleur="#ffffff";}
				    echo "<tr bgcolor='".$couleur."'>\n";
				    $QuestionPrecedente=$Ligne_Q_R_RStagiaires[1];
					$numQuestionPrecedente=$Ligne_Q_R_RStagiaires[0];
					$ImageQuestion="";
		...
		if($QCM_Repondu)
        {
			...
			$req="UPDATE form_session_personne_qualification SET DateHeureFermeture = NOW() WHERE Id=".$_GET['Id_Session_Personne_Qualification'];
