...
if($_POST){
	...
	$_SESSION['FiltreTabNbBesoins_Prestations']=$Id_Presta;
	$_SESSION['FiltreTabNbBesoins_Plateforme']=$_POST['Id_Plateforme'];
	$_SESSION['FiltreTabNbBesoins_Formation']=$_POST['Id_Formation'];
	$_SESSION['FiltreTabNbBesoins_Type']=$Id_Type;
	$_SESSION['FiltreTabNbBesoins_MotifNouveau']="";
	if(isset($_POST['MotifNouveau'])){
		$_SESSION['FiltreTabNbBesoins_MotifNouveau']="1";
	}
	$_SESSION['FiltreTabNbBesoins_MotifRenouvellement']="";
	if(isset($_POST['MotifRenouvellement'])){
		$_SESSION['FiltreTabNbBesoins_MotifRenouvellement']="1";
	}
	$_SESSION['FiltreTabNbBesoins_RespProjet']=$Id_RespProjet;
} 
?>
...
<tr>
					...
					<?php
						if($_POST){
							$requete="	SELECT
										...
										form_besoin.Id_Pole,
										COUNT(form_besoin.Id) AS NbBesoin
										FROM
										form_besoin,
										form_typeformation,
										form_formation,
									...
									WHERE
										...
										AND new_competences_prestation.Id_Plateforme=".$_POST['Id_Plateforme']."
										";
							if($Id_Type<>""){$requete.="AND Id_TypeFormation IN (".$Id_Type.") ";}
							
							if(isset($_POST['MotifNouveau']) && isset($_POST['MotifRenouvellement'])){
							}
							elseif(isset($_POST['MotifNouveau'])){
								$requete.="AND Motif <>'Renouvellement' ";
							}
							elseif(isset($_POST['MotifRenouvellement'])){
								$requete.="AND Motif ='Renouvellement' ";
							}
							...
							if($_POST['Id_Formation']<>"" && $_POST['Id_Formation']<>"0_0"){
							...
							$requete.=" GROUP BY Id_Prestation, Id_Pole 
									ORDER BY NbBesoin DESC ";
						...
						?>
							<tr >
								<td class="EnTeteTableauCompetences" width="80%"><?php if($LangueAffichage=="FR"){echo "Prestation";}else{echo "Site";}?></td>
								<td class="EnTeteTableauCompetences" width="16%"><?php if($LangueAffichage=="FR"){echo "Nombre de besoins";}else{echo "Number of needs";}?></td>
								<td class="EnTeteTableauCompetences" width="4%">
									&nbsp;<a style="text-decoration:none;" href="javascript:ExcelNbBesoin();">
						...
						?>
							<tr bgcolor="<?php echo $couleur; ?>">
								<td><?php echo $row['Prestation']." ".$row['Pole'] ; ?></td>
								<td colspan="2"><?php echo $row['NbBesoin'] ; ?></td>
							</tr>
						<?php 
									$total+=$row['NbBesoin'];
						...
						?>
							<tr bgcolor="3f93ef">
								<td><?php if($LangueAffichage=="FR"){echo "Total";}else{echo "Total";}?></td>
	...
	<td align="right" valign="top" width="25%">
		<table class="GeneralInfo" style="border-spacing:0; width:70%; align:center;box-shadow: 0 8px 10px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19);">
			...
			<tr><td height="4px"></td></tr>
			<tr>
				<td class="Libelle" width="10%"><?php if($LangueAffichage=="FR"){echo "Motif";}else{echo "Reason";}?> : </td>
			</tr>
			<tr>
				<td>
					<table width='100%'>
						<?php
							$checked="";
							if($_POST){
								if(isset($_POST['MotifNouveau'])){$checked="checked";}
							}
							else{$checked="checked";}
							
							echo "<tr><td>";
							echo "<input type='checkbox' class='checkType' name='MotifNouveau' Id='MotifNouveau' value='0' ".$checked." >";
							if($LangueAffichage=="FR"){echo "Nouveau";}else{echo "New";}
							echo "</td></tr>";
							
							$checked="";
							if($_POST){
								if(isset($_POST['MotifRenouvellement'])){$checked="checked";}
							}
							else{$checked="checked";}
							
							echo "<tr><td>";
							echo "<input type='checkbox' class='checkType' name='MotifRenouvellement' Id='MotifRenouvellement' value='1' ".$checked." >";
							if($LangueAffichage=="FR"){echo "Renouvellement";}else{echo "Renewal";}
							echo "</td></tr>";
						?>
					</table>
				</td>
			...
			<tr>
				<td>
					...
					<select name="Id_Formation" id="Id_Formation" style="width:200px">
						...
						<?php
						$laformation=$_SESSION['FiltreTabNbBesoins_Formation'];
