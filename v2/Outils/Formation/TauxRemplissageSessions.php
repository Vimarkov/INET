...
if($_POST){
	...
	$_SESSION['FiltreTauxRemplissageSessions_Plateforme']=$_POST['Id_Plateforme'];
	$_SESSION['FiltreTauxRemplissageSessions_DateDebut']=$_POST['DateDebut'];
	$_SESSION['FiltreTauxRemplissageSessions_DateFin']=$_POST['DateFin'];
	$_SESSION['FiltreTauxRemplissageSessions_Type']=$Id_Type;
	$_SESSION['FiltreTauxRemplissageSessions_ModeAffichage']=$_POST['ModeAffichage'];
	$_SESSION['FiltreTauxRemplissageSessions_Formation']=$_POST['formation'];
	$_SESSION['FiltreTauxRemplissageSessions_Categorie']=$Categorie;
	$_SESSION['FiltreTauxRemplissageSessions_Formateur']=$Id_Formateur;
} 
?>
				...
				if($_POST['DateDebut']<>"" && $_POST['DateFin']<>""){
					...
					if($dateFin>=$dateDebut){
						...
						while($leParcours<=$laFin){
							...
							$remplissage=TauxRemplissageSession($_POST['Id_Plateforme'],$Id_Type,$Id_Formateur,$Categorie,$_POST['formation'],$PremierJour,$DernierJour);

							$arrayTauxRemplissage[$i]=array("Abscisse" => $abscisse,"TauxRemplissage" => $remplissage);
						...
						$arrayTauxRemplissageCategorie=array();
						if($NbResult>0){
							$i=0;
							while($row=mysqli_fetch_array($Result))
							{
								$arrayTauxRemplissageCategorie[$i]=array("Abscisse" => utf8_encode($row['Categorie']),"TauxRemplissage" => $row['TauxRemplissage']);
								$i++;
							}
						}
				...
				?>
					<tr>
						<td height="350px;">
							<table class="TableCompetences" width="99%" height="100%" cellpadding="0" cellspacing="0">
								<tr>
									<td class="Libelle" height="5%" style="font-size:15px;"><?php if($_SESSION['Langue']=="FR"){echo "TAUX DE REMPLISSAGE DES SESSIONS";}else{echo "SESSION FILLING RATE";} ?></td>
								...
								<tr>
									<td valign="top">
										...
										<script>
											...
											// Add data
											chart.data = <?php echo json_encode($arrayTauxRemplissage); ?>;
											...
											series1.dataFields.valueY = "TauxRemplissage";
											series1.name = <?php echo json_encode($arrayLegendeTauxRemplissage[0]); ?>;
					...
					<tr>
						<td height="600px;">
							<table class="TableCompetences" width="99%" height="100%" cellpadding="0" cellspacing="0">
								<tr>
									<td class="Libelle" height="5%" style="font-size:15px;"><?php if($_SESSION['Langue']=="FR"){echo "TAUX DE REMPLISSAGE DES SESSIONS / CATEGORIE";}else{echo "SESSION FILLING RATE / CATEGORY";} ?></td>
									<td style="cursor:pointer;" align="right"></td>
								</tr>
								<tr><td style="border-bottom:2px solid #0b6acb;" colspan="2" height="4"></td></tr>
								<tr><td height="4"></td></tr>
								<tr>
									<td valign="top">
										...
										<script>
											...
											// Add data
											chart.data = <?php echo json_encode($arrayTauxRemplissageCategorie); ?>;
											...
											series1.columns.template.width = am4core.percent(80);
											series1.tooltipText = "{name}: {valueY.value}[/]%";
											series1.tooltipText = "[{categoryX}: bold]{valueY.value}[/]%";
											series1.dataFields.categoryX = "Abscisse";
											series1.dataFields.valueY = "TauxRemplissage";
											series1.name = <?php echo json_encode($arrayLegendeTauxRemplissage[0]); ?>;
											series1.stacked = false;
		...
		<table class="GeneralInfo" style="border-spacing:0; width:70%; align:center;box-shadow: 0 8px 10px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19);">
			...
			<tr>
				<td>
					...
					$selected="";
					if($_SESSION['FiltreTauxRemplissageSessions_ModeAffichage']=='Semaine'){$selected="selected";}
					...
					$selected="";
					if($_SESSION['FiltreTauxRemplissageSessions_ModeAffichage']=='Mois'){$selected="selected";}
					if($LangueAffichage=="FR"){echo "<option value='Mois' ".$selected.">Mois</option>";}
					else{echo "<option value='Mois' ".$selected.">Month</option>";}
					
					$selected="";
					if($_SESSION['FiltreTauxRemplissageSessions_ModeAffichage']=='Année'){$selected="selected";}
			...
			<tr>
				<td class="Libelle" width="10%"><input type="date" style="text-align:center;"id="DateDebut" name="DateDebut" size="10" value="<?php echo AfficheDateFR($_SESSION['FiltreTauxRemplissageSessions_DateDebut']); ?>"></td>
			</tr>
			<tr>
				<td class="Libelle" width="10%"><?php if($LangueAffichage=="FR"){echo "Date fin";}else{echo "End date";}?> : </td>
			</tr>
			<tr>
				<td class="Libelle" width="10%"><input type="date" style="text-align:center;"id="DateFin" name="DateFin" size="10" value="<?php echo AfficheDateFR($_SESSION['FiltreTauxRemplissageSessions_DateFin']); ?>"></td>
			</tr>
			<tr>
			...
			<tr>
				<td>
					...
					<select name="formation" id="formation" style="width:200px">
						...
						<?php
						$laformation=$_SESSION['FiltreTauxRemplissageSessions_Formation'];
