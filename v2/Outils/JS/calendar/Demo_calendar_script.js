...
$(function(){
    ...
    $(".calendar2_event").css({
        "width" : td_width2*0.98,
        "margin-left" : (td_width2-(td_width2*0.98))/2
    });
    ...
    $(".calendar_event").resizable({
        ...
        resize: function(event,ui){
            ...
            var height_css=object_drop.css("height");
            var height_css_value=parseInt(height_css.replace(".px",""));
            var duree_en_milli=Math.round(((((height_css_value/30)/4)*60)*60)*1000);
    ...
    $(".calendar_td").dblclick(function(e){
        ...
        var depart_en_sec=(((margin_top/30)/4-1)*60)*60;
        var depart_en_millisec=Math.round(depart_en_sec*1000);
        var height_css_value=parseInt(120);
        var duree_en_sec=(((height_css_value/30)/4)*60)*60;
        var duree_en_millisec=Math.round(duree_en_sec*1000);
        var fin_en_sec=depart_en_sec + duree_en_sec;
        var fin_en_millisec=depart_en_millisec + duree_en_millisec;
		...
		if(debut!=""){
			...
			duree_en_sec=(((height_css_value/30)/4)*60)*60;
        ...
        $("#gen_new_content").dialog({
			...
			buttons: {
				'Enregistrer': function() {
					...
					$.ajax({
						url : url_create,
						data : 'Id_Tache='+new_titre+'&Id_WP='+new_lieu+'&laDate='+day_choose+'&heureDebut='+heureDuDebut+'&heureFin='+heureDeFin+'&commentaire='+new_commentaire+'&Id_Prepa='+document.getElementById('Id_Prepa').value,
						async:false,
					});
					//Rechargement de la page
					if(document.getElementById('pagePHP').value=="Planning.php"){
        ...
        event.click(function(e){
            ...
            $("#dialog").dialog({
                ...
                open: function(event, ui) {
					...
					var i;
					var isElement = false;
					contenu +="<td><select class='lab' name='new_event_wp2' id='new_event_wp2' onchange=\"RechargerTache2('"+document.getElementById('langue').value+"')\">";
					for(i=0;i<Liste_WP.length;i++){
						$selected="";
						if($("#"+id_event+"_wp").html()==Liste_WP[i][0]){$selected="selected";}
						contenu += "<option value='"+Liste_WP[i][0]+"' "+$selected+">"+Liste_WP[i][1]+"</option>";
						isElement = true;
					}
                ...
                buttons: {
                    ...
                    'Modifier': function() {
						...
						$.ajax({
							url : url_create,
							data : 'Id='+id_event+'&Id_Tache='+new_titre+'&Id_WP='+new_lieu+'&Commentaire='+new_commentaire,
							async:false,
						});
