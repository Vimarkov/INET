		...
		if($nbQuestionnaire){
			while($rowQuestionnaire=mysqli_fetch_array($result)){
				...
				if(isset($_POST['btnEnregistrer'])){
					$req="UPDATE soda_surveillance SET MAJManuelle=1, SignatureSurveillant=1, SignatureSurveille=1, Resultat=".$resultat.", NumActionTracker='".$_POST["numActionTraker_".$rowQuestionnaire['Id_Questionnaire']]."', Commentaire='".$_POST["commentaire_".$rowQuestionnaire['Id_Questionnaire']]."', Etat='Clôturé', DateHeureRepondeur='".date("Y-m-d H:i:s")."' WHERE Id=".$rowQuestionnaire['Id']." ";
				}
				else{
					$req="UPDATE soda_surveillance SET MAJManuelle=1, Resultat=".$resultat.", NumActionTracker='".$_POST["numActionTraker_".$rowQuestionnaire['Id_Questionnaire']]."', Commentaire='".$_POST["commentaire_".$rowQuestionnaire['Id_Questionnaire']]."', Etat='Brouillon' WHERE Id=".$rowQuestionnaire['Id']." ";
				}
				$resultUpdt=mysqli_query($bdd,$req);
								...
								echo "<td >
										...
										<tr id="tr_<?php echo $rowQuestion['Id']; ?>">
											<td>
												...
												<input style="width:570px;" type="text" id="<?php echo "observation_".$rowQuestion['Id'];?>" name="<?php echo "observation_".$rowQuestion['Id'];?>" value="<?php echo stripslashes($observation);?>">
												<br>
