...
if($_SESSION['Langue']=="FR"){
	...
	$sheet->setCellValue('M1',utf8_encode('Cause de la non applicabilité'));
	$sheet->setCellValue('N1',utf8_encode('Type de non applicabilité'));
}
else{
	$sheet->setCellValue('A1',utf8_encode('N°'));
	...
	$sheet->setCellValue('L1',utf8_encode('Question'));
	$sheet->setCellValue('M1',utf8_encode('Cause of non-applicability'));
	$sheet->setCellValue('N1',utf8_encode('Type of non-applicability'));
}
	...
	if($nbSurveillance > 0){
		...
		while($rowSurveillance=mysqli_fetch_array($resultSurveillance)){
			...
			if($rowSurveillance['TypeNA']=="1"){
				if($_SESSION['Langue']=="FR"){
					$typeNA= "Liée aux circonstances de la surveillance (temporaire)";
				}
				else{
					$typeNA= "Related to circumstances of supervision (temporary)";
				}
			}
			elseif($rowSurveillance['TypeNA']=="1"){
				if($_SESSION['Langue']=="FR"){
					$typeNA= "Liée à la prestation (permanente)";
				}else{
					$typeNA= "Linked to the activity (permanent)";
				}
			}
			...
			$sheet->setCellValueByColumnAndRow(13,$ligne,utf8_encode(stripslashes($typeNA)));
...
header('Content-Disposition: attachment;filename="Export_NA.xlsx"'); 
header('Cache-Control: max-age=0'); 

$writer = PHPExcel_IOFactory::createWriter($workbook, 'Excel2007');

$chemin = '../../tmp/Export_NA.xlsx';
$writer->save($chemin);
readfile($chemin);
?>