...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
		{window.open("Ajout_FamilleMateriel.php?Mode="+Mode+"&Id="+Id,"PageToolsAjoutFamilleMateriel","status=no,menubar=no,width=675,height=250");}
</script>
		...
		<td>
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#67cff1;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Familles de matériel";}else{echo "Material families";}
					?>
					</td>
					<?php
					...
					<td>
						...
						<?php
                            $Requete="
                                SELECT
                                    Id,
									(SELECT Libelle FROM tools_typemateriel WHERE Id=Id_TypeMateriel) AS LIBELLE_TYPEMATERIEL,
                                    Libelle
                                FROM
