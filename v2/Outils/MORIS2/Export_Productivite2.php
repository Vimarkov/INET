...
if($_SESSION['FiltreRECORD_Vision']==1){
	...
	if($_SESSION['Langue']=="FR"){
		...
		$sheet->setCellValue('D1',utf8_encode('Temps vendu (h)'));
		$sheet->setCellValue('E1',utf8_encode('Temps objectif (h)'));
		$sheet->setCellValue('F1',utf8_encode('Temps passé (h)'));
		$sheet->setCellValue('G1',utf8_encode('Productivité Brute'));
		$sheet->setCellValue('H1',utf8_encode('Productivité Corrigée'));
		...
		$sheet->setCellValue('D1',utf8_encode('Time sold (h)'));
		$sheet->setCellValue('E1',utf8_encode('Temps objectif (h)'));
		$sheet->setCellValue('F1',utf8_encode('Time Spent (h)'));
		$sheet->setCellValue('G1',utf8_encode('Gross Productivity'));
		$sheet->setCellValue('H1',utf8_encode('Corrected Productivity'));
	...
	for($nbMois=1;$nbMois<=12;$nbMois++){
		...
		AND Suppr=0 	
		AND PasActivite=0
		AND (SELECT ProductiviteADesactive FROM new_competences_prestation WHERE Id=Id_Prestation)=0
		";
		if($listePrestation2<>""){
		...
		if($nbResultaMoisPrestaEC>0){
			while($LigneMoisPrestationEC=mysqli_fetch_array($resultEC)){
				...
				$productiviteBrut=0;
				if($LigneMoisPrestationEC['TempsPasse']>0){
				...
				$sheet->setCellValue('D'.$ligne,utf8_encode($LigneMoisPrestationEC['TempsAlloue']));
				$sheet->setCellValue('E'.$ligne,utf8_encode($LigneMoisPrestationEC['TempsObjectif']));
				$sheet->setCellValue('F'.$ligne,utf8_encode($LigneMoisPrestationEC['TempsPasse']));
				$sheet->setCellValue('G'.$ligne,utf8_encode($productiviteBrut));
				$sheet->setCellValue('H'.$ligne,utf8_encode($productiviteCorrigee));
				$sheet->setCellValue('I'.$ligne,utf8_encode(stripslashes($LigneMoisPrestationEC['CommentaireProductivite'])));
	...
	$req="SELECT DISTINCT new_competences_prestation.Id,
			...
			ON new_competences_prestation.Id_Plateforme=new_competences_plateforme.Id
			WHERE new_competences_prestation.UtiliseMORIS>0 
			AND new_competences_prestation.ProductiviteADesactive=0 ";
	if($_SESSION['FiltreRECORD_Prestation']<>""){
		$req.="AND new_competences_prestation.Id IN(".$_SESSION['FiltreRECORD_Prestation'].") ";
	}
		...
		while($rowPresta=mysqli_fetch_array($resultPresta)){
			...
			if($nbResultaMoisPrestaEC>0){
				while($LigneMoisPrestationEC=mysqli_fetch_array($resultEC)){
					...
					if($LigneMoisPrestationEC['TempsAlloue']>0 || $LigneMoisPrestationEC['TempsObjectif']>0 || $LigneMoisPrestationEC['TempsPasse']>0){					
						$sheet->setCellValue('A'.$ligne,utf8_encode($MoisLettre[$moisEC-1]." ".$anneeEC));
						$sheet->setCellValue('B'.$ligne,utf8_encode(stripslashes($LigneMoisPrestationEC['Prestation'])));
						$sheet->setCellValue('C'.$ligne,utf8_encode($LigneMoisPrestationEC['UER']));
						$sheet->setCellValue('D'.$ligne,utf8_encode($LigneMoisPrestationEC['TempsAlloue']));
