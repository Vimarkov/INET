...
if($_SESSION['Langue']=="FR"){
	...
	$sheet->setCellValue('C1',utf8_encode('Année'));
	$sheet->setCellValue('D1',utf8_encode('Mois'));
	$sheet->setCellValue('E1',utf8_encode('Données enregistrées'));
	$sheet->setCellValue('F1',utf8_encode('Données verrouillées'));
}
else{
	$sheet->setCellValue('A1',utf8_encode('Operating unit'));
	$sheet->setCellValue('B1',utf8_encode('Site'));
	$sheet->setCellValue('C1',utf8_encode('Year'));
	$sheet->setCellValue('D1',utf8_encode('Month'));
	$sheet->setCellValue('E1',utf8_encode('Recorded data'));
	$sheet->setCellValue('F1',utf8_encode('Locked data'));
}
$sheet->setCellValue('G1',utf8_encode('CHARGE / CAPA'));
$sheet->setCellValue('H1',utf8_encode('PRODUCTIVITE'));
$sheet->setCellValue('I1',utf8_encode('OTD'));
$sheet->setCellValue('J1',utf8_encode('OQD'));
$sheet->setCellValue('K1',utf8_encode('MANAGEMENT'));
$sheet->setCellValue('L1',utf8_encode('COMPETENCES (POLYV)'));
$sheet->setCellValue('M1',utf8_encode('COMPETENCES (QUALIF)'));
$sheet->setCellValue('N1',utf8_encode('SECURITE (PDV)'));
$sheet->setCellValue('O1',utf8_encode('SECURITE (AT)'));
$sheet->setCellValue('P1',utf8_encode('PRM & SATIS (PRM)'));
$sheet->setCellValue('Q1',utf8_encode('PRM & SATIS (SATIS)'));
$sheet->setCellValue('R1',utf8_encode('NC/RC'));
...
for($mois=1;$mois<=12;$mois++){
	...
	$req="SELECT Id,Libelle,Id_Plateforme,PlanPreventionADesactivite,ChargeADesactive,ProductiviteADesactive,PolyvalenceADesactive,
		...
		AND (
			...
			AND Suppr=0 
			AND CONCAT(YEAR(DateDebut),'_',IF(MONTH(DateDebut)<10,CONCAT('0',MONTH(DateDebut)),MONTH(DateDebut)))<='".date($_SESSION['MORIS_Annee2'].'_'.$leMois)."'
			AND (CONCAT(YEAR(DateFin),'_',IF(MONTH(DateFin)<10,CONCAT('0',MONTH(DateFin)),MONTH(DateFin)))>='".date($_SESSION['MORIS_Annee2'].'_'.$leMois)."' OR DateFin<='0001-01-01')
		)>0
	";
	...
	if ($nbResulta>0){
		...
		while($row=mysqli_fetch_array($result)){
			...
			if($row['SecuriteADesactive']==1 || ($row['SecuriteADesactive']==0 && $_SESSION['MORIS_Annee2'].'_'.$leMois>"2023_06")){
				$at="N/A";
			}
			...
			$sheet->setCellValue('C'.$ligne,utf8_encode($_SESSION['MORIS_Annee2']));
			$sheet->setCellValue('D'.$ligne,utf8_encode($mois));
			$sheet->setCellValue('E'.$ligne,utf8_encode($enregistre));
			if($enregistre=="X"){$sheet->getStyle('E'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($enregistre=="V"){$sheet->getStyle('E'.$ligne)->getFont()->getColor()->setRGB($vert);}
			
			$sheet->setCellValue('F'.$ligne,utf8_encode($verrouille));
			if($verrouille=="X"){$sheet->getStyle('F'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($verrouille=="V"){$sheet->getStyle('F'.$ligne)->getFont()->getColor()->setRGB($vert);}
			
			$sheet->setCellValue('G'.$ligne,utf8_encode($charge));
			if($charge=="X"){$sheet->getStyle('G'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($charge=="V"){$sheet->getStyle('G'.$ligne)->getFont()->getColor()->setRGB($vert);}
			
			$sheet->setCellValue('H'.$ligne,utf8_encode($productivite));
			if($productivite=="X"){$sheet->getStyle('H'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($productivite=="V"){$sheet->getStyle('H'.$ligne)->getFont()->getColor()->setRGB($vert);}
			
			$sheet->setCellValue('I'.$ligne,utf8_encode($otd));
			if($otd=="X"){$sheet->getStyle('I'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($otd=="V"){$sheet->getStyle('I'.$ligne)->getFont()->getColor()->setRGB($vert);}
			elseif($otd=="P"){$sheet->getStyle('I'.$ligne)->getFont()->getColor()->setRGB($orange);}
			
			$sheet->setCellValue('J'.$ligne,utf8_encode($oqd));
			if($oqd=="X"){$sheet->getStyle('J'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($oqd=="V"){$sheet->getStyle('J'.$ligne)->getFont()->getColor()->setRGB($vert);}
			elseif($oqd=="P"){$sheet->getStyle('J'.$ligne)->getFont()->getColor()->setRGB($orange);}
			
			$sheet->setCellValue('K'.$ligne,utf8_encode($management));
			if($management=="X"){$sheet->getStyle('K'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($management=="V"){$sheet->getStyle('K'.$ligne)->getFont()->getColor()->setRGB($vert);}
			
			$sheet->setCellValue('L'.$ligne,utf8_encode($polyv));
			if($polyv=="X"){$sheet->getStyle('L'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($polyv=="V"){$sheet->getStyle('L'.$ligne)->getFont()->getColor()->setRGB($vert);}
			
			$sheet->setCellValue('M'.$ligne,utf8_encode($qualif));
			if($qualif=="X"){$sheet->getStyle('M'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($qualif=="V"){$sheet->getStyle('M'.$ligne)->getFont()->getColor()->setRGB($vert);}
			
			$sheet->setCellValue('N'.$ligne,utf8_encode($pdv));
			if($pdv=="X"){$sheet->getStyle('N'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($pdv=="V"){$sheet->getStyle('N'.$ligne)->getFont()->getColor()->setRGB($vert);}
			elseif($pdv=="P"){$sheet->getStyle('N'.$ligne)->getFont()->getColor()->setRGB($orange);}
			
			$sheet->setCellValue('O'.$ligne,utf8_encode($at));
			if($at=="X"){$sheet->getStyle('O'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($at=="V"){$sheet->getStyle('O'.$ligne)->getFont()->getColor()->setRGB($vert);}
			
			$sheet->setCellValue('P'.$ligne,utf8_encode($prm));
			if($prm=="X"){$sheet->getStyle('P'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($prm=="V"){$sheet->getStyle('P'.$ligne)->getFont()->getColor()->setRGB($vert);}
			elseif($prm=="P"){$sheet->getStyle('P'.$ligne)->getFont()->getColor()->setRGB($orange);}
			
			$sheet->setCellValue('Q'.$ligne,utf8_encode($satis));
			if($satis=="X"){$sheet->getStyle('Q'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($satis=="V"){$sheet->getStyle('Q'.$ligne)->getFont()->getColor()->setRGB($vert);}
			
			$sheet->setCellValue('R'.$ligne,utf8_encode($nc));
			if($nc=="X"){$sheet->getStyle('R'.$ligne)->getFont()->getColor()->setRGB($rouge);}
			elseif($nc=="V"){$sheet->getStyle('R'.$ligne)->getFont()->getColor()->setRGB($vert);}
