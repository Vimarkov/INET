...
if($_SESSION['FiltreRECORD_Vision']==1){
	...
	if($_SESSION['Langue']=="FR"){
		...
		$sheet->setCellValue('D1',utf8_encode('Vert'));
		$sheet->setCellValue('E1',utf8_encode('Orange'));
		$sheet->setCellValue('F1',utf8_encode('Rouge'));
		$sheet->setCellValue('G1',utf8_encode('Evènements/Irritants'));
	}
	else{
		...
		$sheet->setCellValue('D1',utf8_encode('Green'));
		$sheet->setCellValue('E1',utf8_encode('Orange'));
		$sheet->setCellValue('F1',utf8_encode('Red'));
		$sheet->setCellValue('G1',utf8_encode('Events / Irritants'));
	}
	...
	for($nbMois=1;$nbMois<=12;$nbMois++){
		...
		TendanceManagement,EvenementManagement
		...
		if($nbResultaMoisPrestaEC>0){
			while($LigneMoisPrestationEC=mysqli_fetch_array($resultEC)){
				...
				$Orange="";
				$Rouge="";
				if($LigneMoisPrestationEC['TendanceManagement']==0){$Vert="X";}
				elseif($LigneMoisPrestationEC['TendanceManagement']==1){$Orange="X";}
				elseif($LigneMoisPrestationEC['TendanceManagement']==2){$Rouge="X";}
				...
				$sheet->setCellValue('D'.$ligne,utf8_encode($Vert));
				$sheet->setCellValue('E'.$ligne,utf8_encode($Orange));
				$sheet->setCellValue('F'.$ligne,utf8_encode($Rouge));
				$sheet->setCellValue('G'.$ligne,utf8_encode(stripslashes($LigneMoisPrestationEC['EvenementManagement'])));
