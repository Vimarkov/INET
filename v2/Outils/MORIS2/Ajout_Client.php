...
if($_POST){
	if($_POST['nomClient']<>""){
		if($_POST['Mode']=="A"){
			$requete="INSERT INTO moris_client (Libelle) VALUES ('".addslashes($_POST['nomClient'])."') ";
			$result=mysqli_query($bdd,$requete);
		}
		elseif($_POST['Mode']=="M"){
			$requete="UPDATE moris_client 
...
?>
		...
		<form id="formulaire" method="POST" action="Ajout_Client.php" >
		...
		<table width="95%" align="center" class="TableCompetences">
			<tr class="TitreColsUsers">
				<td class="Libelle"><?php if($_SESSION['Langue']=="EN"){echo "Client name";}else{echo "Nom du client";} ?> </td>
				<td colspan="3">
...
<?php
	...
	//Mode suppression
	{
		$requete="UPDATE moris_client SET Suppr=1 WHERE Id=".$_GET['Id'];
		$result=mysqli_query($bdd,$requete);
		echo "<script>FermerEtRecharger();</script>";
	}
}
