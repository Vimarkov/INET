<script language="javascript">
	function OuvreFenetreAjout(){
		var w=window.open("Ajout_DivisionClient.php?Mode=A&Id=0","PageUtilisateur","status=no,menubar=no,scrollbars=yes,width=800,height=250");
		w.focus();
		}
	function OuvreFenetreSuppr(Id){
		if(window.confirm('Etes-vous sûr de vouloir supprimer l\'accès à cette prestation ?')){
			var w=window.open("Ajout_DivisionClient.php?Mode=S&Id="+Id,"PageUtilisateur","status=no,menubar=no,scrollbars=yes,width=60,height=40");
			w.focus();
			}
		}
	function OuvreFenetreModif(Id)
		{
			var w= window.open("Ajout_DivisionClient.php?Mode=M&Id="+Id,"PageLieu","status=no,menubar=no,width=800,height=250");
			w.focus();
		}
</script>
<?php
...
<table align="center" width="100%" cellpadding="0" cellspacing="0">
	<tr bgcolor="#6EB4CD">
		...
		if($_SESSION["Langue"]=="FR"){SousTitre("DIVISION CLIENT","Outils/MORIS2/TableauDeBord.php?Menu=13",true);}
		else{SousTitre("CUSTOMER DIVISION","Outils/MORIS2/TableauDeBord.php?Menu=13",true);}
	...
	<tr>
		<td align="center" colspan="11">
			<a style='text-decoration:none;' class='Bouton' href='javascript:OuvreFenetreAjout()'>&nbsp;<?php if($_SESSION['Langue']=="EN"){echo "Add a customer division";}else{echo "Ajouter une division client";} ?>&nbsp;</a>
		</td>
	</tr>
