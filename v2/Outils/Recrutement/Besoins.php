...
<?php
if(isset($_GET['Tri'])){
	$tab = array("Ref","Prestation","Plateforme","Domaine","Programme","DateDemande","Demandeur","Etat","Lieu","Metier","DateBesoin","Duree","Nombre","Horaire","Etat","Statut","Statut2","CreationPoste","DateRecrutementMAJ","DateRecrutement");
	foreach($tab as $tri){
		if($_GET['Tri']==$tri){
...
<table width="100%" cellpadding="0" cellspacing="0" align="center">
	...
	<table width="100%" cellpadding="0" cellspacing="0" align="center" class="GeneralInfo">
		...
		<tr>
			...
			<td width="8%" class="Libelle">
				...
				if(DroitsFormation1Plateforme(17,array($IdPosteResponsableRecrutement,$IdPosteRecrutement,$IdPosteAssistantRH,$IdPosteResponsableRH))){
					$requeteSite="
							SELECT DISTINCT Id_Prestation AS Id, LEFT(Libelle,7) AS Libelle
							FROM recrut_annonce
							LEFT JOIN new_competences_prestation
							ON recrut_annonce.Id_Prestation=new_competences_prestation.Id
							WHERE Id_Plateforme=".$PtfSelect."
							ORDER BY Libelle ASC";
				}
				else{
					$requeteSite="SELECT DISTINCT Id_Prestation AS Id, LEFT(Libelle,7) AS Libelle
							FROM recrut_annonce
							LEFT JOIN new_competences_prestation
							ON recrut_annonce.Id_Prestation=new_competences_prestation.Id
		...
		<tr>
			<td width="10%" class="Libelle">
				&nbsp;<?php if($_SESSION["Langue"]=="FR"){echo "Besoin/Offre :";}else{echo "Need/Offer :";} ?>
				<select id="etat" style="width:80px;" name="etat" onchange="submit();">
					<option value=""></option>
					<?php 
					...
					?>
					<option value="<?php if($_SESSION["Langue"]=="FR"){echo "BESOIN";}else{echo "NEED";} ?>" <?php if($_SESSION["Langue"]=="FR"){if($etat=="BESOIN"){echo "selected";}}else{if($etat=="NEED"){echo "selected";}} ?>><?php if($_SESSION["Langue"]=="FR"){echo "BESOIN";}else{echo "NEED";} ?></option>
					<option value="<?php if($_SESSION["Langue"]=="FR"){echo "OFFRE";}else{echo "OFFER";} ?>" <?php if($_SESSION["Langue"]=="FR"){if($etat=="OFFRE"){echo "selected";}}else{if($etat=="OFFER"){echo "selected";}} ?>><?php if($_SESSION["Langue"]=="FR"){echo "OFFRE";}else{echo "OFFER";} ?></option>
				</select>
			</td>
			<td width="10%" class="Libelle">
	...
	<?php
		...
		$requete2="SELECT Id,DateDemande,Id_Demandeur,Id_Prestation,Metier,Nombre,Lieu,RaisonRefus,Suppr,Division,
			...
			EtatApprobation,EtatRecrutement,DateRecrutementMAJ,EtatPoste,DateRecrutement,
			".$reqSuite."
		...
		<td>
			<table class="TableCompetences" align="center" width="100%">
				<tr>
					...
					<td class="EnTeteTableauCompetences" width="5%" style="text-decoration:none;color:#000000;font-weight:bold;" ><a style="text-decoration:none;color:#000000;font-weight:bold;" id="tri" href="Besoins.php?Tri=Etat"><?php if($_SESSION["Langue"]=="FR"){echo "Besoin/Offre";}else{echo "Need/Offer";} ?><?php if($_SESSION['TriRecrutBesoin_Etat']=="DESC"){echo "&uarr;";} elseif($_SESSION['TriRecrutBesoin_Etat']=="ASC"){echo "&darr;";}?></a></td>
					<td class="EnTeteTableauCompetences" width="10%" style="text-decoration:none;color:#000000;font-weight:bold;" ><a style="text-decoration:none;color:#000000;font-weight:bold;" id="tri" href="Besoins.php?Tri=Statut"><?php if($_SESSION["Langue"]=="FR"){echo "Etat";}else{echo "State";} ?><?php if($_SESSION['TriRecrutBesoin_Statut']=="DESC"){echo "&uarr;";} elseif($_SESSION['TriRecrutBesoin_Statut']=="ASC"){echo "&darr;";}?></a></td>
	...
	<?php
			if($nbResulta>0){
				while($row=mysqli_fetch_array($result))
				{
					...
					if($row['EtatValidation']==-1 && $row['RaisonRefus']<>""){
						$Hover=" id='leHover' ";
						$Etat.="<span>".stripslashes($row['RaisonRefus'])."</span>";
					}
					elseif($row['EtatApprobation']==-1 && $row['RaisonRefusApprobation']<>""){
						$Hover=" id='leHover' ";
						$Etat.="<span>".stripslashes($row['RaisonRefusApprobation'])."</span>";
					}
					elseif($row['EtatRecrutement']==-1 && $row['RaisonRefusRecrutement']<>""){
						$Hover=" id='leHover' ";
						$Etat.="<span>".stripslashes($row['RaisonRefusRecrutement'])."</span>";
					}
					
					if($row['EtatRecrutement']<>0 || ($row['EtatApprobation']==1 && $row['OuvertureAutresPlateformes']==0) ){$CouleurEtat="#ecf943";}
		...
		?>
					<tr bgcolor="<?php echo $couleur;?>">
						...
						?></td>
						<td <?php if($ActionAFaire==1){echo "bgcolor='#f4a6ba'; cellpadding='1' ";}?>><a style="color:#3e65fa;" href="javascript:OuvreFenetreModif(<?php echo $row['Id']; ?>)"><?php echo stripslashes($row['Ref']);?></a></td>
						...
						<td <?php echo $Hover; ?>><?php echo stripslashes($row['Statut']);?></td>
