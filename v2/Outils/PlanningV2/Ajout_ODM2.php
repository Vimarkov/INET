...
if($_POST){
	if(isset($_POST['btnEnregistrer2'])){
		...
		$req="INSERT INTO rh_personne_contrat (Id_ContratInitial,Id_Personne,Id_DODM,Id_TypeContrat,Id_Metier,DateDebut,DateFin,
			...
			FraisReel,PrimeResponsabilite,IndemniteOutillage,MajorationVSD,PrimeEquipe,PanierGrandeNuit,PanierVSD,Motif,Titre,DateSignatureSiege,DateSignatureSalarie,DateRetourSigneAuSiege,ChampsModifie) 
			VALUES 
				(".$_POST['Id_Contrat'].",".$_POST['Id_Personne'].",".$_POST['Id_DODM'].",".$_POST['id_typeContrat'].",".$_POST['metier'].",
				'".TrsfDate_($_POST['dateDebut'])."','".TrsfDate_($_POST['dateFin'])."',".$Id_Prestation.",".$Id_Pole.",'ODM','".date('Y-m-d')."',".$_SESSION['Id_Personne'].",
				".$_POST['client'].",".$_POST['responsableAAA'].",".$IPD.",".$repas.",".$IGD.",".$repasGD.",".$fraisReel.",".$primeResp.",".$indemniteOutillage.",
				".$vsd.",".$primeEquipe.",".$panierGD.",".$panierVSD.",'".addslashes($_POST['descriptionMission'])."','".addslashes($_POST['titre'])."','".TrsfDate_($_POST['dateSignatureSiege'])."','".TrsfDate_($_POST['dateSignatureSalarie'])."','".TrsfDate_($_POST['dateRetourSigne'])."','".$_POST['ChampsModifies']."')";
			...
			if ($nbResulta>0){
				while($row=mysqli_fetch_array($result)){
					if(isset($_POST['moyensCheck_'.$row['Id']])){
						...
						$requeteInsert.=" (".$IdCree.",".$row['Id'].",".$montant.",'".addslashes($_POST['periodicite_'.$row['Id']])."',
		...
		echo "<script>ODMExcel(".$IdCree.")</script>";
...
$requete2="SELECT rh_personne_petitdeplacement.Id_PrestationDeplacement AS Id_Prestation,rh_personne_petitdeplacement.Id_PoleDeplacement AS Id_Pole,
rh_personne_petitdeplacement.DateDebut,rh_personne_petitdeplacement.DateFin,
(SELECT Id_Plateforme FROM new_competences_prestation WHERE new_competences_prestation.Id=Id_PrestationDeplacement) AS Id_Plateforme 
FROM rh_personne_petitdeplacement
WHERE Id=".$_GET['Id_DODM'];
$resultDODM=mysqli_query($bdd,$requete2);
$rowDODM=mysqli_fetch_array($resultDODM);

$etoile="<img src='../../Images/etoile.png' width='8' height='8' border='0'>";
?>

<form id="formulaire" class="test" action="Ajout_ODM2.php" method="post">
<table width="100%" cellpadding="0" cellspacing="0" align="center">
	...
	<input type="hidden" name="Id_DODM" id="Id_DODM" value="<?php echo $_GET['Id_DODM']; ?>" />
		...
		<td colspan="5">
			<table style="width:100%; border-spacing:0;">
				<tr>
					...
					<?php 
						if($_SESSION["Langue"]=="FR"){echo "Nouvel ODM";}else{echo "New mission order";}
					?>
					</td>
					<td width="4"></td>
				</tr>
					...
					<table class="TableCompetences" width="90%" align="center" cellpadding="0" cellspacing="0">
						...
						<tr>
							...
							<td width="10%">
								...
								{
									$selected="";
									if($rowDODM['Id_Plateforme']==$row['Id']){$selected="selected";}
									echo "<option value='".$row['Id']."' ".$selected.">".str_replace("'"," ",$row['Libelle'])."</option>\n";
								}
								?>
								</select>
							...
							<td width="10%">
								...
								{
									$selected="";
									if($rowDODM['Id_Prestation']."_".$rowDODM['Id_Pole']."_".$rowDODM['Id_Plateforme']==$rowsite['Id']."_".$rowsite['Id_Pole']."_".$rowsite['Id_Plateforme']){$selected="selected";}
									$display="style='display:none;'";
									if($rowDODM['Id_Plateforme']==$rowsite['Id_Plateforme']){$display="";}
								...
								<input type="hidden" name="id_prestationPole" id="id_prestationPole" value="<?php echo $rowDODM['Id_Prestation']."_".$rowDODM['Id_Pole']."_".$rowDODM['Id_Plateforme']; ?>" />
							</td>
						</tr>
						<tr><td height="4"></td></tr>
						<tr>
						...
						<tr><td height="4"></td></tr>
						<tr>
							<td width="10%" class="Libelle" colspan="4" id="LibelleDescriptionMission"><?php if($_SESSION["Langue"]=="FR"){echo "Description de la mission";}else{echo "Description of the mission";} ?></td>
						</tr>
						<tr>
							<td colspan="4">
						...
						<tr>
							...
							<td width="10%">
								<input type="date" style="text-align:center;" onChange="ModifierCouleurChamps('id_dateDebut','dateDebut','LibelleDateDebut')" id="dateDebut" name="dateDebut" size="10" value="<?php echo AfficheDateFR($rowDODM['DateDebut']); ?>">
								<input type="hidden" name="id_dateDebut" id="id_dateDebut" value="<?php echo AfficheDateFR($rowDODM['DateDebut']); ?>" />
							</td>
							<td width="10%" class="Libelle" id="LibelleDateFin"><?php if($_SESSION["Langue"]=="FR"){echo "Date de fin :";}else{echo "End date :";} ?> </td>
							<td width="10%">
								<input type="date" style="text-align:center;"  onChange="ModifierCouleurChamps('id_dateFin','dateFin','LibelleDateFin')" id="dateFin" name="dateFin" size="10" value="<?php echo AfficheDateFR($rowDODM['DateFin']); ?>">
								<input type="hidden" name="id_dateFin" id="id_dateFin" value="<?php echo AfficheDateFR($rowDODM['DateFin']); ?>" />
							</td>
						</tr>
						<tr><td height="4"></td></tr>
						<tr><td height="4" colspan="8" style="border-top:2px dotted #cdbad2"></td></tr>
