...
if($_SESSION["Langue"]=="FR"){
	$sheet->setCellValue('B1',utf8_encode('Date début'));
	$sheet->setCellValue('B2',utf8_encode('Date fin'));
	$sheet->setCellValue('B3',utf8_encode('CODE SITE'));
}
else{
	$sheet->setCellValue('B1',utf8_encode('Start date'));
	$sheet->setCellValue('B2',utf8_encode('End date'));
...
$sheet->setCellValue('C1',utf8_encode(AfficheDateJJ_MM_AAAA($_SESSION['FiltreRHPlanning_DateDebut'])));
$sheet->setCellValue('C2',utf8_encode(AfficheDateJJ_MM_AAAA($_SESSION['FiltreRHPlanning_DateFin'])));
	...
	while($row=mysqli_fetch_array($resultPersonne)){
		...
		while ($tmpDate <= $dateFin)
		{
			...
			if(appartientPrestation($tmpDate,$row['Id'],$PrestationSelect,$PoleSelect)==1){
				...
				//Cellule finale
				if($nbHeure==0){$nbHeure="";}
				$sheet->setCellValueByColumnAndRow($colonne,$ligne,utf8_encode($nbHeure));
