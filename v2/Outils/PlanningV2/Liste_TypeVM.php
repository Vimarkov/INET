...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
	{
		...
		{
			var w= window.open("Ajout_TypeVM.php?Mode="+Mode+"&Id="+Id,"PageLieu","status=no,menubar=no,width=450,height=200");
			w.focus();
		}
	}
</script>
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#cdcc8d;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Visites médicales # Types de visite";}else{echo "Medical visits # Type of visit";}
					?>
					</td>
				</tr>
			</table>
		...
		<td align="left">
			...
			<?php
				$req="SELECT Id, 
					...
					FROM rh_typevisitemedicale WHERE Suppr=0 
					ORDER BY Libelle ";
