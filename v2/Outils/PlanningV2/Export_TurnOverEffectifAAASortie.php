...
if($_SESSION['Langue']=="FR"){
	...
	$sheet->setCellValue('C1',utf8_encode('Date de sortie'));
	$sheet->setCellValue('D1',utf8_encode('Motif de sortie'));
	...
	$sheet->setCellValue('C1',utf8_encode('Release date'));
	$sheet->setCellValue('D1',utf8_encode('Exit reason'));
	$sheet->setCellValue('E1',utf8_encode('Type of Contract'));
}
...
if($nbEntree>0){
	...
	while($rowpersonne=mysqli_fetch_array($resultEntree)){
		...
		if($DateSortie>'0001-01-01'){
			$date = explode("-",$DateSortie);
		...
		$sheet->setCellValue('D'.$ligne,utf8_encode($Motif));
		$sheet->setCellValue('E'.$ligne,utf8_encode($TypeContrat));
...
header('Content-Disposition: attachment;filename="Export_TurnOverEffectifAAASortie.xlsx"'); 
header('Cache-Control: max-age=0'); 

$writer = PHPExcel_IOFactory::createWriter($workbook, 'Excel2007');

$chemin = '../../tmp/Export_TurnOverEffectifAAASortie.xlsx';
$writer->save($chemin);
readfile($chemin);
?>