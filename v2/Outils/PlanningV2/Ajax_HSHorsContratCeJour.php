				...
				if(TravailCeJourDeSemaine($laDate,$Id_Personne)=="" && $nb>0){
					if($HorsContrat==""){
						if($_SESSION['Langue']=="FR"){
							$HorsContrat="<img width='25px' src='../../Images/attention.png'/>Impossible de déclarer des astreintes car les personnes suivantes ont déjà des heures supplémentaires ce jour-là : <br>";
						}
						else{
							$HorsContrat="<img width='25px' src='../../Images/attention.png'/>Can not declare on call because the following people already have overtime on this day : <br>";
						}
					}
					while($rowHS=mysqli_fetch_array($result)){
						$HorsContrat.= "- ".$rowHS['Personne']." : ".AfficheDateJJ_MM_AAAA($rowHS['DateHS'])."\n";
					}
				}
			}
		}
