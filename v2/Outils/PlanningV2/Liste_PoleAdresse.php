...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
	{
		...
		{
			var w= window.open("Ajout_PoleAdresse.php?Mode="+Mode+"&Id="+Id,"PageLieu","status=no,menubar=no,width=450,height=200");
			w.focus();
		}
	}
</script>
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#a988b2;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Pôle (adresse)";}else{echo "Pole (address)";}
					?>
					</td>
				</tr>
			</table>
	...
	<tr>
		...
		<td align="left">
			<table class="TableCompetences" width="60%">
				<tr>
					<td class="EnTeteTableauCompetences" width="60%"><?php if($_SESSION["Langue"]=="FR"){echo "Pôle";}else{echo "Pole";}?></td>
					<td class="EnTeteTableauCompetences" width="40%"><?php if($_SESSION["Langue"]=="FR"){echo "Adresse";}else{echo "Address";}?></td>
				</tr>
			<?php
				$req="SELECT new_competences_pole.Id, 
					new_competences_prestation.Libelle AS Prestation,
					new_competences_pole.Libelle, new_competences_pole.Adresse AS AdressePole,
					(SELECT Adresse FROM new_competences_plateforme WHERE Id=Id_Plateforme) AS AdressePlateforme,
					new_competences_prestation.Adresse AS AdressePrestation
			...
			?>
				<tr bgcolor="<?php echo $Couleur;?>">
					<td><?php echo stripslashes(substr($row['Prestation'],0,7).' - '.$row['Libelle']);?></td>
					<td><?php if($row['AdressePole']<>""){echo $row['AdressePole'];}else{echo $row['AdressePlateforme'];} ?></td>
					<td>
