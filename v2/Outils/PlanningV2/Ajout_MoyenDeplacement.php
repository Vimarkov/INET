...
<head>
	...
	<script type="text/javascript">
		...
		{
			window.opener.location="Liste_MoyenDeplacement.php?Menu="+Menu;
			window.close();
		}
	</script>
</head>
...
if($_POST)
{
	if($_POST['Mode']=="Ajout")
	{
		$requeteInsertUpdate="INSERT INTO rh_moyendeplacement (Libelle,LibelleEN)";
	...
	{		
		$requeteInsertUpdate="UPDATE rh_moyendeplacement SET";
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="Ajout" || $_GET['Mode']=="Modif")
	{
		...
		{
			$Modif=True;
			$result=mysqli_query($bdd,"SELECT Id, Libelle, LibelleEN FROM rh_moyendeplacement WHERE Id=".$_GET['Id']." AND Suppr=0");
			$row=mysqli_fetch_array($result);
		}
?>
		<form id="formulaire" enctype="multipart/form-data" method="POST" action="Ajout_MoyenDeplacement.php" onSubmit="return VerifChamps();">
...
<?php
	...
	//Mode suppression
	{
		$result=mysqli_query($bdd,"UPDATE rh_moyendeplacement SET Suppr=1,Id_Suppr=".$_SESSION['Id_Personne'].", DateSuppr='".date('Y-m-d')."' WHERE Id=".$_GET['Id']);
