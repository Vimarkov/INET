...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
	{
		...
		{
			var w= window.open("Ajout_MoyenDeplacement.php?Mode="+Mode+"&Id="+Id,"PageLieu","status=no,menubar=no,width=450,height=200");
			w.focus();
		}
	}
</script>
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#a988b2;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Contrats # Moyens de déplacement";}else{echo "Contracts # Means of displacement";}
					?>
					</td>
				</tr>
			</table>
		...
		<td align="left">
			...
			<?php
				$req="SELECT Id, 
					...
					FROM rh_moyendeplacement WHERE Suppr=0 
					ORDER BY Libelle ";
