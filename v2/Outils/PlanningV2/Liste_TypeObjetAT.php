...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
	{
		...
		{
			var w= window.open("Ajout_TypeObjetAT.php?Mode="+Mode+"&Id="+Id,"PageLieu","status=no,menubar=no,width=450,height=200");
			w.focus();
		}
	}
</script>
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#ff1111;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Accident de travail # Type d'objet";}else{echo "Work accident # Type of object";}
					?>
					</td>
				</tr>
			</table>
		...
		<td align="left">
			...
			<?php
				$req="SELECT Id, 
					...
					FROM rh_typeobjet_at WHERE Suppr=0 
					ORDER BY Libelle ";
