...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
	{
		...
		{
			var w= window.open("Ajout_SiegeLesionAT.php?Mode="+Mode+"&Id="+Id,"PageLieu","status=no,menubar=no,width=450,height=200");
			w.focus();
		}
	}
</script>
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#ff1111;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Accident de travail # Siège des lésions";}else{echo "Siege of lesions";}
					?>
					</td>
				</tr>
			</table>
	...
	<tr>
		...
		<td align="left">
			<table class="TableCompetences" width="60%">
				<tr>
					<td class="EnTeteTableauCompetences" width="40%"><?php if($_SESSION["Langue"]=="FR"){echo "Libellé";}else{echo "Wording";}?></td>
					<td class="EnTeteTableauCompetences" width="40%"><?php if($_SESSION["Langue"]=="FR"){echo "Libellé Anglais";}else{echo "Wording English";}?></td>
					<td class="EnTeteTableauCompetences" width="15%"><?php if($_SESSION["Langue"]=="FR"){echo "Concerne un côté";}else{echo "Side concerned";}?></td>
			...
			<?php
				$req="SELECT Id, 
					Libelle, LibelleEN, CoteGD, Suppr
					FROM rh_siege_lesion_at WHERE Suppr=0 
					ORDER BY Libelle ";
			...
			?>
				<tr bgcolor="<?php echo $Couleur;?>">
					...
					<td><?php if($row['CoteGD']==1){if($_SESSION["Langue"]=="FR"){echo "Oui";}else{echo "Yes";}}else{if($_SESSION["Langue"]=="FR"){echo "Non";}else{echo "No";}};?></td>
					<td>
