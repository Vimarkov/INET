...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
	{
		...
		{
			var w= window.open("Ajout_TypeBesoin.php?Mode="+Mode+"&Id="+Id,"PageLieu","status=no,menubar=no,width=450,height=200");
			w.focus();
		}
	}
</script>
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#a9e99d;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "DODM # Type de besoin";}else{echo "DODM # Type of need";}
					?>
					</td>
				</tr>
			</table>
		...
		<td align="left">
			<table class="TableCompetences" width="50%">
				<tr>
					...
					<td class="EnTeteTableauCompetences" width="60%"><?php if($_SESSION["Langue"]=="FR"){echo "Service concerné";}else{echo "Service concerned";}?></td>
			...
			<?php
				$req="SELECT Id, 
					Libelle, LibelleEN, ServiceConcerne , Suppr
					FROM rh_typebesoin WHERE Suppr=0 
					ORDER BY Libelle ";
					...
					<td>
						<?php 
							if($_SESSION["Langue"]=="FR"){
								echo stripslashes($row['ServiceConcerne']);
							}
							else{
								if($row['ServiceConcerne']=="Accueil"){echo "Welcome";}
								elseif($row['ServiceConcerne']=="Moyens généraux"){echo "General options";}
							}
						?>
					</td>
					<td>
