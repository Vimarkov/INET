...
$excel = $workbook->load('releve heure baseDSK.xlsx');
...
function completerReleve($sheet,$IdPersonne,$DateCalcul,$dateFin,$Id_Agence,$Id_Plateforme){
	...
	while ($DateCalcul  < $dateFin)
	{
		...
		if($Id_Plateforme==$_SESSION['FiltreRHRelevesHeures_Plateforme'] & $nbC>0){
			...
			if($nbHeureForm>0){
				if($formExterne==1){
					$sheet->getStyle($colonneF.$ligne)->applyFromArray(array('fill'=>array('type'=>PHPExcel_Style_Fill::FILL_SOLID,'color'=>array('argb'=>'5eb2f6'))));
				}
			}
