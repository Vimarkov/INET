...
<script language="javascript">
	function OuvreFenetreExcel()
		{window.open("Export_RepartitionInterim.php","PageExcel","status=no,menubar=no,width=90,height=90");}
	function Recharger(){
...
<form class="test" action="RepartitionInterim.php" method="post">
<table width="100%" cellpadding="0" cellspacing="0" align="center">
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#fa9426;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Réparitions Interim";}else{echo "Interim allocation";}
					?>
					</td>
				</tr>
			</table>
	...
	<?php
		...
		$req = "SELECT DISTINCT new_rh_etatcivil.Id, 
			...
			new_rh_etatcivil.MatriculeDSK,new_rh_etatcivil.CentreDeCout
				...
				while($row=mysqli_fetch_array($result))
				{
					...
					for($laDate=$dateDebut;$laDate<$dateFin;$laDate=date('Y-m-d',strtotime($laDate." +1 day"))){
						if(estInterim($laDate,$row['Id'])){
							$NbHeuresTotal=$NbHeuresTotal+NombreHeuresTotalJourneeRepartition($row['Id'],$laDate);
						}
					}
					...
					if($nbResultaPresta>0){
						while($rowPresta=mysqli_fetch_array($resultPresta))
						{
							...
							if($NbHeuresTotal>0){
								...
								$pourcentage=round(($NbHeures/$NbHeuresTotal)*100);
								...
								<tr bgcolor="<?php echo $couleur;?>">
									...
									<td width="10%"><?php echo $NbHeures;?></td>
									<td width="10%"><?php echo $NbHeuresTotal;?></td>
								</tr>
								<?php
							}
						}
