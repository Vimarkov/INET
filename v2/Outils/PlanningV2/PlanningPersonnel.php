<head>
	...
	<script language="javascript">
		function OuvreFenetrePointageIndividuelExport(mois,annee)
			{window.open("PointageIndividuel_ExportPerso.php?Mois="+mois+"&Annee="+annee,"PageExcel","status=no,menubar=no,width=100,height=100");}
	</script>
</head>
...
$Debut=date("Y-m-d",mktime(0,0,0,1,1,$_SESSION['FiltreRHPlanning_Annee']));
$Fin=date("Y-m-d",mktime(0,0,0,13,0,$_SESSION['FiltreRHPlanning_Annee']));

$tmpDate=date("Y-m-d",mktime(0,0,0,1,1,$_SESSION['FiltreRHPlanning_Annee']));
...
$reqConges="SELECT rh_absence.Id_Personne_DA,rh_absence.DateDebut,rh_absence.DateFin,
		...
		WHERE rh_personne_demandeabsence.Id_Personne=".$_SESSION['Id_Personne']." 
...
AND rh_personne_rapportastreinte.Id_Personne=".$_SESSION['Id_Personne']."
...
$req="SELECT Id_Vacation,Id_Prestation,Id_Pole,DateVacation,Divers,
	...
	AND rh_personne_vacation.Id_Personne=".$_SESSION['Id_Personne']."
		...
		WHERE
			...
			(
				...
				WHERE
					...
					AND form_session_personne.Id_Personne=".$_SESSION['Id_Personne']." 
	...
	if($i==1 || $i==4 || $i==7 || $i==10){echo "<tr>";}
		echo "<td align='center'>";
			echo "<table style='border:1px solid #787878;' width='85%' cellpadding='0' cellspacing='0'>";
				...
				$leDebut=date("Y-m-d",mktime(0,0,0,$mois,1,$_SESSION['FiltreRHPlanning_Annee']));
				$laFin=date("Y-m-d",mktime(0,0,0,$mois+1,0,$_SESSION['FiltreRHPlanning_Annee']));
				
				//Si la personne est cadre alors afficher l'excel 
				$req="SELECT Id_AgenceInterim,Id_TypeContrat,Id_TempsTravail
							FROM
							(SELECT DISTINCT Id_AgenceInterim,Id_TypeContrat,DateDebut,Id,Id_TempsTravail
							FROM rh_personne_contrat
							WHERE Suppr=0
							AND DateDebut<'".$laFin."'
							AND (DateFin>='".$leDebut."' OR DateFin<='0001-01-01' )
						...
						AND Id_TempsTravail=10
						";

				$resultContrat=mysqli_query($bdd,$req);
				$nb=mysqli_num_rows($resultContrat);
				if($nb>0){
					echo "<tr><td class='cEnTete' colspan='8' align='center'>".$MoisLettre[$mois-1]." ".$tabDate[0]."   <a href=\"javascript:OuvreFenetrePointageIndividuelExport('".$mois."','".$_SESSION['FiltreRHPlanning_Annee']."');\"><img src='../../Images/excel2.gif' border='0' alt='Excel'></a></td></tr>";
				}
				else{
				...
				for($ligne=1;$ligne<=6;$ligne++){
					...
					for($colonne=0;$colonne<=7;$colonne++){
						...
						if($colonne==0){
							...
							$bgcolor="";
							$Travail=0;
							$laCouleur=TravailCeJourDeSemaine($dateMois,$_SESSION['Id_Personne']);
							if($laCouleur<>""){
								$Travail=1;
								$type="J";
							...
							$VacParticuliere=0;
							$Id_PrestationPole=PrestationPole_Personne($dateMois,$_SESSION['Id_Personne']);
							...
							$tab=HorairesJournee($_SESSION['Id_Personne'],$dateMois);
							if(sizeof($tab)>0){
								...
								if($nbVM>0){
									...
									while($rowVM=mysqli_fetch_array($resultVM)){
										if($rowVM['DateVisite']==$dateMois){
											...
											if(estSalarie($dateMois,$_SESSION['Id_Personne'])==0){
							...
							if($jour==1){
								if($joursem[$jourSemaine]==$joursem2[$colonne-1] && $tabDate[1]==$tabDateMois[1]){
									...
									if($VacParticuliere==0){
										$jourFixe=estJour_Fixe($dateMois,$_SESSION['Id_Personne']);
										$Id_Contrat =IdContrat($_SESSION['Id_Personne'],$dateMois);
