...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
	{
		...
		{
			var w= window.open("Ajout_AgenceInterim.php?Mode="+Mode+"&Id="+Id,"PageLieu","status=no,menubar=no,width=450,height=150");
			w.focus();
		}
	}
</script>
		...
		<td colspan="5">
			<table class="GeneralPage" style="width:100%; border-spacing : 0; background-color:#a988b2;">
				<tr>
					...
					if($LangueAffichage=="FR"){echo "Contrats # Agence d'intérim";}else{echo "Contracts # Acting Agency";}
					?>
					</td>
				</tr>
			</table>
	...
	<tr>
		...
		<td align="left">
			<table class="TableCompetences" width="40%">
				<tr>
					<td class="EnTeteTableauCompetences" width="60%"><?php if($_SESSION["Langue"]=="FR"){echo "Libellé";}else{echo "Wording";}?></td>
					<td class="EnTeteTableauCompetences" width="40%"><?php if($_SESSION["Langue"]=="FR"){echo "Coeff. gestion";}else{echo "Coefficient of management";}?></td>
					<td class="EnTeteTableauCompetences" width="40%"><?php if($_SESSION["Langue"]=="FR"){echo "Coeff. délégation";}else{echo "Coefficient of delegation";}?></td>
			...
			<?php
				$req="SELECT Id, 
					Libelle, CoeffGestion, CoeffDelegation, Suppr
					FROM rh_agenceinterim WHERE Suppr=0 
					ORDER BY Libelle ";
			...
			?>
				<tr bgcolor="<?php echo $Couleur;?>">
					...
					<td><?php echo $row['CoeffGestion'];?></td>
					<td><?php echo $row['CoeffDelegation'];?></td>
					<td>
