...
foreach($tabListe as $releve){
	if($releve<>""){
		...
		if ($nbPers>0)
		{
			...
			completerReleve($sheet2,$Id_Personne,$laDateDebut,$laDateFin,$Id_Agence,$_SESSION['FiltreRHRelevesHeures_Plateforme']);
...
function completerReleve($sheet,$IdPersonne,$DateCalcul,$dateFin,$Id_Agence,$Id_Plateforme){
	global $bdd;
	$EnAttente="#ffbf03";
	$Automatique="#3d9538";
	$Validee="#6beb47";
	...
	if($nbConges>0){
		while($rowConges=mysqli_fetch_array($resultConges)){
			$tabConges[$i]=array('Id_Personne_DA' => $rowConges['Id_Personne_DA'],
				'DateDebut' => $rowConges['DateDebut'],'DateFin' => $rowConges['DateFin'],'Id_TypeAbsenceInitial' => $rowConges['Id_TypeAbsenceInitial'],'Id_TypeAbsenceDefinitif' => $rowConges['Id_TypeAbsenceDefinitif'],
				'NbHeureAbsJour' => $rowConges['NbHeureAbsJour'],'NbHeureAbsNuit' => $rowConges['NbHeureAbsNuit'],'EtatN1' => $rowConges['EtatN1'],'EtatN2' => $rowConges['EtatN2'],'EtatRH' => $rowConges['EtatRH'],
				'TypeAbsenceIni' => $rowConges['TypeAbsenceIni'],'TypeAbsenceDef' => $rowConges['TypeAbsenceDef'],'CouleurIni' => $rowConges['CouleurIni'],'CouleurDef' => $rowConges['CouleurDef'],
				'NecessiteJustifIni' => $rowConges['NecessiteJustifIni'],'NecessiteJustifDef' => $rowConges['NecessiteJustifDef']);
			$i++;
		}
	}
	...
	if($nbAbs>0){
		while($rowAbs=mysqli_fetch_array($resultAbs)){
			$tabAbs[$i]=array('Id_Personne_DA' => $rowAbs['Id_Personne_DA'],
				'DateDebut' => $rowAbs['DateDebut'],'DateFin' => $rowAbs['DateFin'],'Id_TypeAbsenceInitial' => $rowAbs['Id_TypeAbsenceInitial'],'Id_TypeAbsenceDefinitif' => $rowAbs['Id_TypeAbsenceDefinitif'],
				'NbHeureAbsJour' => $rowAbs['NbHeureAbsJour'],'NbHeureAbsNuit' => $rowAbs['NbHeureAbsNuit'],
				'TypeAbsenceIni' => $rowAbs['TypeAbsenceIni'],'TypeAbsenceDef' => $rowAbs['TypeAbsenceDef'],'CouleurIni' => $rowAbs['CouleurIni'],'CouleurDef' => $rowAbs['CouleurDef'],
				'NecessiteJustifIni' => $rowAbs['NecessiteJustifIni'],'NecessiteJustifDef' => $rowAbs['NecessiteJustifDef']);
			$i++;
		}
	}
	...
	if($nb2HS>0){
		while($rowHS=mysqli_fetch_array($resultHS)){
			$tabHS[$i]=array('Id' => $rowHS['Id'],
				'Nb_Heures_Jour' => $rowHS['Nb_Heures_Jour'],'Nb_Heures_Nuit' => $rowHS['Nb_Heures_Nuit'],'DateHS' => $rowHS['DateHS'],'DatePriseEnCompteRH' => $rowHS['DatePriseEnCompteRH'],
	...
	if($nbAst>0){
		while($rowAst=mysqli_fetch_array($resultAst)){
			$tabAst[$i]=array('DateAstreinte' => $rowAst['DateAstreinte'],
				'DatePriseEnCompte' => $rowAst['DatePriseEnCompte'],'DiffHeures1' => $rowAst['DiffHeures1'],'DiffHeures2' => $rowAst['DiffHeures2'],'DiffHeures3' => $rowAst['DiffHeures3'],
				'Montant' => $rowAst['Montant'],'Intervention' => $rowAst['Intervention']);
			$i++;
		}
	}
	...
	if($nbSession>0){
		while($rowSession=mysqli_fetch_array($resultSession)){
			$tabSession[$i]=array('DateSession' => $rowSession['DateSession'],
				'Heure_Debut' => $rowSession['Heure_Debut'],'Heure_Fin' => $rowSession['Heure_Fin'],'PauseRepas' => $rowSession['PauseRepas'],'HeureDebutPause' => $rowSession['HeureDebutPause'],
				'HeureFinPause' => $rowSession['HeureFinPause']);
			$i++;
		}
	}
	...
	if($nbVM>0){
		while($rowVM=mysqli_fetch_array($resultVM)){
			$tabVM[$i]=array('DateVisite' => $rowVM['DateVisite'],'HeureVisite' => $rowVM['HeureVisite'],'HeureFin' => $rowVM['HeureFin']);
			$i++;
		}
	}
	...
	while ($DateCalcul  < $dateFin)
	{
		...
		if($Id_Plateforme==$_SESSION['FiltreRHRelevesHeures_Plateforme'] & $nbC>0){
			...
			//Congés
			if($nbConges>0){
				foreach($tabConges as $rowConges){
	...
	//Enlever les formules 
	$valeur = $sheet->getCell("O10")->getCalculatedValue();
	$sheet->setCellValue('O10',$valeur);
	
	$valeur = $sheet->getCell("R45")->getCalculatedValue();
	$sheet->setCellValue('R45',$valeur);
	
	$valeur = $sheet->getCell("R46")->getCalculatedValue();
	$sheet->setCellValue('R46',$valeur);
	
	for($col="C";$col<>"J";$col++){
		$valeur = $sheet->getCell($col."22")->getCalculatedValue();
		$sheet->setCellValue($col."22",$valeur);
		
		$valeur = $sheet->getCell($col."30")->getCalculatedValue();
		$sheet->setCellValue($col."30",$valeur);
		
		$valeur = $sheet->getCell($col."38")->getCalculatedValue();
		$sheet->setCellValue($col."38",$valeur);
		
		$valeur = $sheet->getCell($col."40")->getCalculatedValue();
		$sheet->setCellValue($col."40",$valeur);
	}
	...
	for($li="15";$li<="35";$li++){
		/*$valeur = $sheet->getCell("B".$li)->getCalculatedValue();
		$sheet->setCellValue("B".$li,$valeur);
		
		$valeur = $sheet->getCell("L".$li)->getCalculatedValue();
		$sheet->setCellValue("L".$li,$valeur);*/
	}
	
	for($li="48";$li<="58";$li++){
		//$valeur = $sheet->getCell("C".$li)->getCalculatedValue();
		//$sheet->setCellValue("C".$li,$valeur);
	}
	
	for($li="45";$li<="58";$li++){
		//$valeur = $sheet->getCell("J".$li)->getCalculatedValue();
		//$sheet->setCellValue("J".$li,$valeur);
	}
}
