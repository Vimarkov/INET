...
<script type="text/javascript">
	function OuvreFenetreModif(Mode,Id)
		{window.open("Ajout_Diplome.php?Mode="+Mode+"&Id="+Id,"PageFichier","status=no,menubar=no,width=375,height=50");}
</script>
<?php
$Droits="Aucun";
		...
		<td>
			<table class="GeneralPage" style="width:100%; border-spacing:0;">
				<tr>
					...
					echo "</a>";
					echo "&nbsp;&nbsp;&nbsp;";
					if($LangueAffichage=="FR"){echo "Gestion des compétences # Gestion des diplômes";}else{echo "Competencies management # Diplomas management";}?></td>
					<?php
					...
					<td width="25">
						<a class="Modif" href="javascript:OuvreFenetreModif('Ajout','0');">
							<img src="../../Images/Ajout.gif" border="0" alt="Ajouter un diplôme">
						</a>
					</td>
					<?php
						}
			...
			<table>
				<tr>
					...
					<td>
						...
						<?php
							$result=mysqli_query($bdd,"SELECT new_competences_diplome.Id, new_competences_diplome.Libelle, (SELECT Libelle FROM new_competences_niveau_diplome WHERE Id=new_competences_diplome.Id_Niveau) AS Niveau FROM new_competences_diplome ORDER BY Niveau ASC, new_competences_diplome.Libelle ASC");
							$nbenreg=mysqli_num_rows($result);
							if($nbenreg>0)
							{
						?>
							<tr>
								<td width="150" class="EnTeteTableauCompetences"><?php if($LangueAffichage=="FR"){echo "Niveau";}else{echo "Grade";}?></td>
