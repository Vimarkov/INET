...
$result=mysqli_query($bdd,"SELECT Nom, Prenom, NumBadge, Date_Naissance FROM new_rh_etatcivil WHERE new_rh_etatcivil.Id=".$_GET['Id']);
...
WHERE
	form_besoin.Id_Personne=".$_GET['Id']."
	...
	WHERE new_competences_personne_formation.Id_Personne=".$_GET['Id']." 
...
if($nbenreg > 0)
{
	while($row=mysqli_fetch_array($result))
	{
		...
		if($Libelle != $row['Libelle'])
		{
			...
			$sheet->setCellValueByColumnAndRow(0,$i,utf8_encode($Libelle));
			if($row['DateSession']>'0001-01-01' && $row['DateSession']!='0001-01-01'){$sheet->setCellValueByColumnAndRow(12,$i,utf8_encode($row['DateSession']));}
