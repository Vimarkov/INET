...
if($_POST)
{
	if($_POST['Mode']=="Ajout")
	{
		$result=mysqli_query($bdd,"SELECT * FROM rh_domaine WHERE Libelle='".$_POST['Libelle']."' AND Id_Plateforme=".$_POST['Id_Plateforme']);
		if(mysqli_num_rows($result)==0)
		{
			$result=mysqli_query($bdd,"INSERT INTO rh_domaine (Id_Plateforme, Libelle) VALUES (".$_POST['Id_Plateforme'].",'".$_POST['Libelle']."')");
			echo "<script>FermerEtRecharger();</script>";
		}
		else{echo "<font class='Erreur'>Ce libellé existe déjà pour ce domaine.<br>Vous devez recommencer l'opération.</font>";}
	}
	elseif($_POST['Mode']=="Modif")
	{
		$result=mysqli_query($bdd,"SELECT * FROM rh_domaine WHERE Libelle='".$_POST['Libelle']."' AND Id!=".$_POST['Id']." AND Id_Plateforme=".$_POST['Id_Plateforme']);
		if(mysqli_num_rows($result)==0)
		{
			$result=mysqli_query($bdd,"UPDATE rh_domaine SET Id_Plateforme=".$_POST['Id_Plateforme'].", Libelle='".$_POST['Libelle']."' WHERE Id=".$_POST['Id']);
			echo "<script>FermerEtRecharger();</script>";
		}
		else{echo "<font class='Erreur'>Ce libellé existe déjà pour ce domaine.<br>Vous devez recommencer l'opération.</font>";}
	}
...
elseif($_GET)
{
	...
	if($_GET['Mode']=="Ajout" || $_GET['Mode']=="Modif")
	{
		if($_GET['Id']!='0')
		{
			$result=mysqli_query($bdd,"SELECT * FROM rh_domaine WHERE Id=".$_GET['Id']);
			$row=mysqli_fetch_array($result);
		}
?>
		<form id="formulaire" method="POST" action="Ajout_Domaine.php" onSubmit="return VerifChamps();">
...
<?php
	...
	//Mode suppression
	{
		...
		if(mysqli_num_rows($result)==0)
		{
			$result=mysqli_query($bdd,"DELETE FROM rh_domaine WHERE Id=".$_GET['Id']);
			echo "<script>FermerEtRecharger();</script>";
		}
