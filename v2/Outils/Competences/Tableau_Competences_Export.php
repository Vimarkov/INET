...
elseif($_GET['Type']=="Conglomeration")
{
	...
	if($nbenreg2>0)
	{
		...
		{
			$ListePresta.=$row2['Libelle']."\n";
		}
	}
}
elseif($_GET['Type']=="Pole")
{
	$Requetes_Titre="SELECT Libelle, 
			Code_Analytique, 
			Id_Plateforme, 
			AfficherBadgeStamp, 
			AfficherBadge, 
	...
	WHERE Id IN (SELECT Id_Prestation FROM new_competences_pole WHERE Id=".$_GET['Id'].")";
	$Result_Titre=mysqli_query($bdd,$Requetes_Titre);
	$Ligne_Titre=mysqli_fetch_array($Result_Titre);
	$ResultLogo=mysqli_query($bdd,"SELECT Logo FROM new_competences_plateforme WHERE Id=".$Ligne_Titre[2]);
	$LigneLogo=mysqli_fetch_array($ResultLogo);
...
if($nbenreg>0)
{
	...
	if($_GET['Type']=="Prestation"){
		...
		if($Filiale==0){
			...
			$sheet->setCellValue("A5",utf8_encode("Activity : ".$Nom));
		}
	}
	elseif($_GET['Type']=="Plateforme"){
		$Nom =$Ligne_Titre[0];
		if($Filiale==0){
			$sheet->setCellValue("A5",utf8_encode("Unité d'exploitation : ".$Nom));
		}
		else{
			$sheet->setCellValue("A5",utf8_encode("Operating unit : ".$Nom));
		}
	}
	elseif($_GET['Type']=="Conglomeration"){
		$Nom =$Ligne_Titre[0];
		if($Filiale==0){
			$sheet->setCellValue("A5",utf8_encode("Prestations : ".$ListePresta));
		}
		else{
			$sheet->setCellValue("A5",utf8_encode("Activities : ".$ListePresta));
		}
		$sheet->getStyle('A5')->getAlignment()->setWrapText(true);
	}
	elseif($_GET['Type']=="Pole"){
		$Nom ="Pôle : ".$Ligne_Titre[0]." # ".$Ligne_Libelle_Pole[0];
		if($Filiale==0){
			$sheet->setCellValue("A5",utf8_encode("Pôle : ".$Nom));
		}
		else{
			$sheet->setCellValue("A5",utf8_encode("Pole : ".$Nom));
		}
	}
	...
	if($AfficherEtatQualif==1){
		$sheet->getRowDimension('6')->setRowHeight(130);
		$objDrawing = new PHPExcel_Worksheet_Drawing();
		$objDrawing->setName('PHPExcel logo');
		$objDrawing->setDescription('PHPExcel statut');
		$objDrawing->setPath('../../Images/statutQualif2.png');
	...
	while($Ligne_Liste_Qualification=mysqli_fetch_array($Result_Liste_Qualification))
	{
		...
		if($AfficherEtatQualif==1){
			$sheet->setCellValueByColumnAndRow($colonne2,6,utf8_encode($Ligne_Liste_Qualification['Etat']));
		}
		$laCouleur="000000";
		...
		|| $Ligne_Liste_Qualification['Etat']=="ATP" || $Ligne_Liste_Qualification['Etat']=="ATP*"){$laCouleur="39b070";}
		elseif($Ligne_Liste_Qualification['Etat']=="QTBD" || $Ligne_Liste_Qualification['Etat']=="QTBD*"){$laCouleur="ffca00";}
		$sheet->getStyle($colonneL3.'6')->getFont()->getColor()->setRGB($laCouleur);
	...
	if($nbPersonne>0){
		while($Ligne_Liste_Personne=mysqli_fetch_array($Result_Liste_Personne)){
			...
			elseif($AfficherStamp==1){
				...
				{
					$sheet->setCellValueByColumnAndRow(4,$ligne,utf8_encode($PRESTATION));
				}
			}
			elseif($AfficherBadge==1){
				...
				if($_GET['Type']=="Plateforme")
				{
					$sheet->setCellValueByColumnAndRow(3,$ligne,utf8_encode($PRESTATION));
				}
			}
				...
				if (mysqli_num_rows($Result_Ligne_Qualifications) > 0)
				{
					...
					while($Ligne_Qualifications=mysqli_fetch_array($Result_Ligne_Qualifications))
					{
						if($Ligne_Qualifications[7] == $Ligne_Liste_Qualification[0])
						{
							...
							elseif($Ligne_Qualifications[1] == "X"){$bgcolor="0099FF";}
							elseif($Ligne_Qualifications[1] == "S"){$bgcolor="0099FF";$LettreCombineeHelico="Q";}
							elseif($Ligne_Qualifications[1] == "Q" || $Ligne_Qualifications[1] == "Q1" || $Ligne_Qualifications[1] == "Q2" || $Ligne_Qualifications[1] == "Q3"){$bgcolor="00FF00";$LettreCombineeHelico="S";}
							...
							//-------------------------------------------------------------------------------------------------
							$QualificationCombineeHelico=false;
							if($Ligne_Liste_Qualification[1]==116 && ($Lettre == "Q" || $Lettre == "S"))
							{
								...
								if(mysqli_num_rows($Result_Ligne_Qualifications2)>0){
									$QualificationCombineeHelico=true;
							...
							if($QualificationCombineeHelico)
							{
								if($Ligne_Qualifications[1] == "Q"){$sheet->setCellValueByColumnAndRow($colonne,$ligne,utf8_encode($Lettre."/S"));}
								elseif($Ligne_Qualifications[1] == "S"){$sheet->setCellValueByColumnAndRow($colonne,$ligne,utf8_encode($Lettre."/Q"));}
							}
							else
							{
		...
		{
			$sheet->getStyle('F7:'.$colonneCorpsL.$ligne)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$sheet->getStyle('A7:'.$colonneCorpsL.$ligne)->getBorders()->applyFromArray(array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN ,'color' => array('rgb' => '0066cc'))));
			$sheet->getStyle($colonneCorpsL.'7:'.$colonneCorpsL.$ligne)->getBorders()->applyFromArray(array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_NONE ,'color' => array('rgb' => '0066cc'))));
		}
		if($_GET['Type']=="Plateforme"){
			if($AfficherStamp==1 && $AfficherBadge==1){
				$sheet->getStyle('G7:G'.$ligne)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
			}
			else{
				$sheet->getStyle('F7:F'.$ligne)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
			}
		}
