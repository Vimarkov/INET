...
if($_POST)
{		    
	$result=mysqli_query($bdd,"UPDATE new_rh_etatcivil SET NumBadge='".$_POST['NumBadge']."' WHERE Id=".$_POST['Id_Personne']);
...
elseif($_GET)
{
	...
	$result=mysqli_query($bdd,"SELECT Id, NumBadge FROM new_rh_etatcivil WHERE Id=".$_GET['Id_Personne']);
	$row=mysqli_fetch_array($result);
?>
<form id="formulaire" method="POST" action="Ajout_Profil_EtatCivil.php">
	<input type="hidden" name="Id_Personne" value="<?php echo $_GET['Id_Personne'];?>">
	<table style="align:center;" class="TableCompetences">
		<tr class="TitreColsUsers">
			<td><?php if($LangueAffichage=="FR"){echo "N° du badge";}else{echo "Badge Number";}?> : </td>
			<td>
				<input name="NumBadge" size="15" type="text" value="<?php echo $row['NumBadge'];?>">
			</td>
